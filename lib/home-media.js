...
// MEMORY LEAK FIX: Shared HTTPS agent to prevent creating new agents on every request
// Creating new agents causes socket/memory leaks as connection pools accumulate
...
async function searchHomeMedia(homeMediaUrl, homeMediaApiKey, type, id, config) {
  try {
    ...
    // MEMORY LEAK FIX: Reuse shared HTTPS agent instead of creating new one per request
    if (homeMediaUrl.startsWith('https://')) {
      if (!sharedHttpsAgent) {
        sharedHttpsAgent = new https.Agent({
          rejectUnauthorized: false,
          keepAlive: false,  // Disable keepAlive to prevent socket accumulation
          maxSockets: 10,    // Limit concurrent sockets
          timeout: 30000     // 30 second socket timeout
        });
      }
      requestConfig.httpsAgent = sharedHttpsAgent;
    }
...
/**
 * List all available files from home media server for catalog
 * @param {string} homeMediaUrl - Home media server URL
 * @param {string} homeMediaApiKey - API key for authentication
 * @returns {Promise<Array>} - Array of all files
 */
async function listAllFiles(homeMediaUrl, homeMediaApiKey) {
  try {
    if (!homeMediaUrl) {
      console.log(`[${LOG_PREFIX}] No HomeMedia URL configured`);
      return [];
    }

    console.log(`[${LOG_PREFIX}] Listing all files from home media server...`);
    ...
    // Filter out incomplete and sample files, return formatted for catalog
    const validFiles = files.filter(file => {
      ...
      // Skip sample files and tiny files
      const lowerName = file.name.toLowerCase();
      if (lowerName.includes('sample') || lowerName.includes('trailer') || file.size < 100 * 1024 * 1024) {
        return false;
      }
    ...
    console.log(`[${LOG_PREFIX}] Filtered to ${validFiles.length} valid files for catalog`);

    // Format files for catalog
    return validFiles.map(file => {
      const parsed = PTT.parse(file.name);

      // Use folder name if filename looks like a hash
      const fileNameWithoutExt = file.name.replace(/\.(mkv|mp4|avi|mov|wmv|flv|webm|m4v|mpg|mpeg|ts|m2ts)$/i, '');
      const looksLikeHash = /^[a-zA-Z0-9]{8,32}$/.test(fileNameWithoutExt);
      ...
      return {
        id: file.flatPath || file.path,
        name: displayName,
        size: file.size,
        url: getStreamUrl(homeMediaUrl, homeMediaApiKey, file.flatPath || file.path),
        info: parsed,
        source: 'homemedia'
      };
    });
    ...
    console.error(`[${LOG_PREFIX}] Error listing files:`, error.message);
    return [];
  }
}
