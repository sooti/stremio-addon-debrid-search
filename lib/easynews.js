...
function isValidVideo(file, userConfig = {}) {
  ...
  const minSizeMB = userConfig.minSize || 50; // Default 50MB minimum
  const maxSizeMB = userConfig.maxSize || 0; // Default 0 = no limit (to include remux files)
  const sizeMB = size / 1024 / 1024;

  if (sizeMB < minSizeMB) {
    console.log(`[${LOG_PREFIX}] File too small: ${title} (${Math.round(sizeMB)}MB < ${minSizeMB}MB)`);
    return false;
  }

  if (maxSizeMB > 0 && sizeMB > maxSizeMB) {
    console.log(`[${LOG_PREFIX}] File too large: ${title} (${Math.round(sizeMB)}MB > ${maxSizeMB}MB)`);
    return false;
  }
