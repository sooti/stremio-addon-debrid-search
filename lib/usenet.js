...
async function searchUsenet(newznabUrl, newznabApiKey, type, id, config) {
  try {
    ...
    // Fetch from Newznab (SQLite cache doesn't store search results)
    const results = await Newznab.search(newznabUrl, newznabApiKey, query, {
      category,
      limit: 50,
      type
    });
...
async function waitForStreamingReady(sabnzbdUrl, sabnzbdApiKey, nzoId, minPercentage = 5, maxWaitTime = 300000) {
  ...
  const pollInterval = 1000; // Check every 1 second (optimized for faster streaming)
  let lastPercent = 0;
      ...
      if (status.percentComplete > 10 && status.percentComplete < 100) {
        ...
        if (percentGained > 0 && elapsedSeconds > 0) {
          ...
          if (estimatedSecondsToComplete < 20) {
            ...
            // If timeout, proceed with streaming - get fresh status first
            console.log(`[${LOG_PREFIX}] Completion wait timeout, proceeding with streaming...`);
            const freshStatus = await getDownloadProgress(sabnzbdUrl, sabnzbdApiKey, nzoId);
            console.log(`[${LOG_PREFIX}] Download ready for streaming: ${freshStatus.percentComplete.toFixed(1)}%`);
            return freshStatus;
          }
        }
      }
