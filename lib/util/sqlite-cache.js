...
export async function upsertCachedMagnets(records) {
  ...
  if (debug) {
    console.log(`[SQLITE CACHE] Starting bulk upsert for ${records.length} records`);
  }
  ...
  try {
    ...
    if (debug) {
      console.log(`[SQLITE CACHE] Bulk upsert completed in ${duration}ms for ${records.length} records`);
    }
    
    return true;
  } catch (error) {
    console.error(`[SQLITE CACHE] Error bulk upserting magnet records: ${error.message}`);
    return false;
  }
}
