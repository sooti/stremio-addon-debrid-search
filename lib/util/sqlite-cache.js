...
const HTTP_RESOLVE_SERVICE = 'httpresolve';

export function isEnabled() {
  return Boolean(config.SQLITE_CACHE_ENABLED);
}

// Ensure data directory exists (runs regardless of whether SQLite is enabled)
export async function ensureDataDirectory() {
  try {
    ...
    if (!fs.existsSync(dataDir)) {
      ...
      console.log(`[SQLITE CACHE] Created data directory: ${dataDir}`);
    } else {
      if (debug) console.log(`[SQLITE CACHE] Data directory already exists: ${dataDir}`);
    }
  } catch (error) {
    console.error(`[SQLITE CACHE] Failed to create data directory: ${error.message}`);
    throw error;
  }
}
...
export async function getCachedHttpResolveUrl(cacheKey) {
  if (!isEnabled() || !cacheKey) {
    if (debug) console.log(`[SQLITE CACHE] HTTP resolve cache disabled or missing key`);
    return null;
  }

  try {
    const record = await getCachedRecord(HTTP_RESOLVE_SERVICE, cacheKey);
    if (!record) {
      return null;
    }

    let url = null;
    if (record.data && typeof record.data === 'object' && typeof record.data.url === 'string') {
      url = record.data.url;
    } else if (typeof record.data === 'string') {
      url = record.data;
    } else if (typeof record.fileName === 'string' && record.fileName.startsWith('http')) {
      url = record.fileName;
    }

    if (!url) {
      if (debug) console.log(`[SQLITE CACHE] HTTP resolve cache record missing URL for key ${cacheKey}`);
      return null;
    }
    ...
    console.error(`[SQLITE CACHE] Error getting HTTP resolve cache for ${cacheKey}: ${error.message}`);
    return null;
  }
}

export async function setCachedHttpResolveUrl(cacheKey, url, ttlMs) {
  if (!isEnabled() || !cacheKey || !url) {
    if (debug) console.log(`[SQLITE CACHE] Skipping HTTP resolve cache write (enabled=${isEnabled()}, key=${cacheKey ? 'yes' : 'no'}, url=${url ? 'yes' : 'no'})`);
    return false;
  }

  try {
    return await upsertCachedMagnet({
      service: HTTP_RESOLVE_SERVICE,
      hash: cacheKey,
      data: { url }
    }, { ttlMs });
  } catch (error) {
    console.error(`[SQLITE CACHE] Error caching HTTP resolve URL for ${cacheKey}: ${error.message}`);
    return false;
  }
}
...
export default { 
  ...
  getCachedHttpResolveUrl,
  setCachedHttpResolveUrl
};
