...
function filterEpisode(torrentDetails, season, episode, cinemetaDetails) {
    ...
    // Note: Season packs should NOT pass through here automatically.
    // They must be inspected by batchCheckSeasonPacks() which creates
    // episode-specific results with episodeFileHint. Only accept results
    // that have been explicitly marked as coming from a pack inspection.
    //
    // IMPORTANT: Only reject if explicitly marked as NOT from pack (false).
    // Missing/undefined isFromPack is allowed for backward compatibility with old cache.
    if (isSeasonPack(torrentTitle, season) && torrentDetails.isFromPack === false) {
        return false;
    }
