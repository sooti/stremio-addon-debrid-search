...
async function defaultTryResumeCloud($, { origin, get, validate, log = console }) {
  ...
  try {
    ...
    // For /zfile/ pages, prioritize extracting direct workers.dev links with ::key format from HTML
    // These are more reliable than API-generated links
    if (resumeUrl.includes('/zfile/')) {
      log.log('[LinkResolver] Detected /zfile/ page, looking for direct workers.dev links with ::key format');
      ...
      log.log('[LinkResolver] No direct workers.dev links with ::key found on /zfile/ page');
    }
