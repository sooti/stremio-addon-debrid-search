...
.service-row.drag-over-top {
	border-top: 3px solid #64ffda !important;
	margin-top: 1em;
}

.service-row.drag-over-bottom {
...
function landingTemplate(manifest, config = {}) {
	...
	<form class="pure-form" id="mainForm">
		<div class="form-element">
			...
			<p style="opacity: 0.7; font-size: 0.9rem; margin-bottom: 1em;">Add one or more services. All services will be queried simultaneously. Drag the ⋮⋮ handle to reorder services.</p>
	...
	const createServiceRow = (provider = 'RealDebrid', apiKey = '', extraConfig = {}) => {
		...
		row.innerHTML = \`
			...
			<div style="flex: 1;">
				<select class="debrid-provider full-width" style="margin-bottom: 0.5em;">
					...
					<option value="PersonalCloud">Personal Cloud (debrider.app)</option>
		...
		const updateUsenetFields = () => {
			if (select.value === 'HomeMedia') {
				...
				// Add Personal Cloud URL field if it doesn't exist
				if (!configDiv.querySelector('.personalcloud-url')) {
				...
				if (!configDiv.querySelector('.debriderapp-newznab-url')) {
					const newznabUrlInput = document.createElement('input');
					newznabUrlInput.type = 'text';
		...
		row.addEventListener('dragover', (e) => {
			...
			// Clear previous indicators from all rows
			document.querySelectorAll('.drag-over-top, .drag-over-bottom').forEach(el => {
				if (el !== row) {
					el.classList.remove('drag-over-top', 'drag-over-bottom');
				}
			...
			// Determine insert position and actually move the element
			if (e.clientY < midpoint) {
				// Insert before current row
				row.classList.add('drag-over-top');
				row.classList.remove('drag-over-bottom');
				if (draggingElement.nextSibling !== row) {
					container.insertBefore(draggingElement, row);
				}
			} else {
				// Insert after current row
				row.classList.add('drag-over-bottom');
				row.classList.remove('drag-over-top');
				if (draggingElement.previousSibling !== row) {
					container.insertBefore(draggingElement, row.nextSibling);
				}
			}
		});
	...
	// Add drag-over handling to the container itself
