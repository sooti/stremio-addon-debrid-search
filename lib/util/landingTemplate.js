	...
	const getDebridServices = () => {
		...
		rows.forEach(row => {
			...
			if (provider === 'Usenet') {
				...
				const httpVidLinkPro = row.querySelector('.http-vidlinkpro')?.checked ?? true;
				services.push({
					provider,
					http4khdhub,
					httpStremsrc,
...
const createServiceRow = (provider = '${process.env.DEFAULT_DEBRID_SERVICE || 'RealDebrid'}', apiKey = '', extraConfig = {}) => {
		...
		const updateUsenetFields = () => {
			...
			if (select.value === 'HomeMedia') {
				...
				httpConfigDiv.innerHTML = \`<div style=\"font-weight: 600; margin-bottom: 0.5em; color: #64ffda; font-size: 0.9rem;\">HTTP Streaming Sources</div><div style=\"display: flex; flex-direction: column; gap: 0.5em;\"><label style=\"display: flex; align-items: center; font-size: 0.9rem; cursor: pointer;\"><input type=\"checkbox\" class=\"http-4khdhub\" checked style=\"margin-right: 8px;">4KHDHub</label><label style=\"display: flex; align-items: center; font-size: 0.9rem; cursor: pointer;\"><input type=\"checkbox\" class=\"http-uhdmovies\" checked style=\"margin-right: 8px;">UHDMovies</label><label style=\"display: flex; align-items: center; font-size: 0.9rem; cursor: pointer;\"><input type=\"checkbox\" class=\"http-moviesdrive\" checked style=\"margin-right: 8px;">MoviesDrive</label><label style=\"display: flex; align-items: center; font-size: 0.9rem; cursor: pointer;\"><input type=\"checkbox\" class=\"http-stremsrc\" checked style=\"margin-right: 8px;">stremsrc</label><label style=\"display: flex; align-items: center; font-size: 0.9rem; cursor: pointer;\"><input type=\"checkbox\" class=\"http-vidlinkpro\" checked style=\"margin-right: 8px;\">VidLink.pro</label></div>\`;
