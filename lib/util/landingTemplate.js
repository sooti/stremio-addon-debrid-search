	...
	<form class="pure-form" id="mainForm">
		...
		<div class="form-element" id="torrentScrapersSection" style="display: none;">
			<sl-select id="Scrapers" name="Scrapers" multiple clearable label="Torrent Scrapers (optional)" placeholder="Select torrent scrapers" help-text="More scrapers = more results but slower response times." hoist max-options-visible="3">
				<sl-option value="jackett">Jackett (Meta-Tracker)</sl-option>
				<sl-option value="1337x">1337x</sl-option>
				<sl-option value="torrent9">Torrent9</sl-option>
				<sl-option value="btdig">BTDigg</sl-option>
				<sl-option value="snowfl">Snowfl</sl-option>
				<sl-option value="magnetdl">MagnetDL</sl-option>
				<sl-option value="wolfmax4k">Wolfmax4K (Spanish)</sl-option>
				<sl-option value="bludv">BluDV (Portuguese)</sl-option>
				<sl-option value="bitmagnet">Bitmagnet</sl-option>
				<sl-option value="knaben">Knaben</sl-option>
			</sl-select>
		</div>
...
const createServiceRow = (provider = '${process.env.DEFAULT_DEBRID_SERVICE || 'RealDebrid'}', apiKey = '', extraConfig = {}) => {
	...
	Promise.all([
		...
		if (indexerScrapersSelect) {
			...
			// If no indexer scrapers were previously selected (empty config), enable Zilean by default if available
			if (indexerScrapers.length > 0) {
				indexerScrapersSelect.value = indexerScrapers;
			} else {
				// Check if Zilean option exists and select it by default
				const zileanOption = indexerScrapersSelect.querySelector('sl-option[value="zilean"]');
				if (zileanOption) {
					indexerScrapersSelect.value = ['zilean'];
				}
			}
