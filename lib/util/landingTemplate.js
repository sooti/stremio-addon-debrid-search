	...
	const getDebridServices = () => {
		...
		rows.forEach(row => {
			...
			if (provider === 'Usenet') {
				...
				const httpDizipall24 = row.querySelector('.http-dizipall24')?.checked ?? false;
				services.push({
					provider,
					http4khdhub,
					httpStremsrc,
				...
				const enablePersonalCloud = row.querySelector('.enable-personal-cloud')?.checked !== false;
...
const createServiceRow = (provider = '${process.env.DEFAULT_DEBRID_SERVICE || 'RealDebrid'}', apiKey = '', extraConfig = {}) => {
		...
		const updateUsenetFields = () => {
			...
			if (select.value === 'HomeMedia') {
				...
				// Use extraConfig values or defaults for checkbox states
				const check4khdhub = extraConfig.http4khdhub !== false ? 'checked' : '';
				const checkUHDMovies = extraConfig.httpUHDMovies !== false ? 'checked' : '';
				const checkMoviesDrive = extraConfig.httpMoviesDrive !== false ? 'checked' : '';
				const checkStremsrc = extraConfig.httpStremsrc !== false ? 'checked' : '';
				const checkDizipall24 = extraConfig.httpDizipall24 === true ? 'checked' : '';
				httpConfigDiv.innerHTML = \`<div style=\"font-weight: 600; margin-bottom: 0.5em; color: #64ffda; font-size: 0.9rem;\">HTTP Streaming Sources</div><div style=\"display: flex; flex-direction: column; gap: 0.5em;\"><label style=\"display: flex; align-items: center; font-size: 0.9rem; cursor: pointer;\"><input type=\"checkbox\" class=\"http-4khdhub\" \${check4khdhub} style=\"margin-right: 8px;\">4KHDHub</label><label style=\"display: flex; align-items: center; font-size: 0.9rem; cursor: pointer;\"><input type=\"checkbox\" class=\"http-uhdmovies\" \${checkUHDMovies} style=\"margin-right: 8px;\">UHDMovies</label><label style=\"display: flex; align-items: center; font-size: 0.9rem; cursor: pointer;\"><input type=\"checkbox\" class=\"http-moviesdrive\" \${checkMoviesDrive} style=\"margin-right: 8px;\">MoviesDrive</label><label style=\"display: flex; align-items: center; font-size: 0.9rem; cursor: pointer;\"><input type=\"checkbox\" class=\"http-stremsrc\" \${checkStremsrc} style=\"margin-right: 8px;\">stremsrc</label><label style="display: flex; align-items: center; font-size: 0.9rem; cursor: pointer;" title="Turkish streaming site - Movies title="Turkish streaming site - Series only" Series"><input type="checkbox" class="http-dizipall24" \${checkDizipall24} style="margin-right: 8px;">Dizipall24 🇹🇷 (Turkish)</label></div>\`;
	...
	existingServices.forEach(service => {
		...
		if (service.provider === 'Usenet') {
			...
			extraConfig = {
				http4khdhub: service.http4khdhub,
				httpStremsrc: service.httpStremsrc,
				httpUHDMovies: service.httpUHDMovies,
				httpMoviesDrive: service.httpMoviesDrive,
				httpDizipall24: service.httpDizipall24
			};
		} else {
