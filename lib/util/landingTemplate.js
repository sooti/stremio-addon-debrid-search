...
// Initialize with existing config or one empty service (default to TorBox instead of RealDebrid)
const existingServices = ${JSON.stringify(config.DebridServices || (config.DebridProvider ? [{ provider: config.DebridProvider, apiKey: config.DebridApiKey }] : [{ provider: 'TorBox', apiKey: '' }]))};
...
const createServiceRow = (provider = 'TorBox', apiKey = '', extraConfig = {}) => {
