...
export function filterStreamsByLanguage(streams, selectedLanguages) {
  ...
  return streams.filter(stream => {
    ...
    // Also consider page-extracted language information if available
    let combinedLanguages = [...detectedLanguages];
    if (stream.languageInfo && Array.isArray(stream.languageInfo) && stream.languageInfo.length > 0) {
      // Merge page-detected languages with filename-detected languages
      combinedLanguages = [...new Set([...combinedLanguages, ...stream.languageInfo])];
    }

    // If English only is selected and no languages detected, keep it
    if (englishOnly && combinedLanguages.length === 0) {
      return true;
    }

    // Check if any selected language is in the combined languages
    return selected.some(lang => combinedLanguages.includes(lang));
  });
}
