...
export async function getTorrentsByStremId(type, id, debridService, debridToken, cachedOnly = true) {
  ...
  try {
    ...
    const tokenObj = {
      stores: [{
        c: debridService.toLowerCase() === 'realdebrid' ? 'rd' : 
           debridService.toLowerCase() === 'alldebrid' ? 'ad' : debridService.toLowerCase(),
        t: debridToken
      }],
      cached: cachedOnly
    };
    ...
    // Don't log error if it's a 404 (endpoint not available), just return empty array
