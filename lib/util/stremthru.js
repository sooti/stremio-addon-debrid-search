      ...
      batches.map(async (batch, index) => {
        try {
          ...
          if (responseData?.data?.items) {
            ...
            const result = cached.map(item => item.hash.toLowerCase());
      ...
      batches.map(async (batch, index) => {
        try {
          ...
          if (responseData?.data?.items) {
            ...
            const result = { hashes: items.filter(item => item.status === 'downloaded' || item.status === 'cached').map(item => item.hash.toLowerCase()), files: {} };
            responseData = null; // Release reference to allow GC
            return result;
          } else if (responseData?.items) {
            // Alternative response format
