...
export function matchesCandidateTitle(candidate, ctx, opts = {}) {
  ...
  for (const raw of values) {
    ...
    // Allow for a small number of extra non-stopword tokens (like country codes)
    const unmatchedTokens = candCore.filter(t => !q.includes(t));
    const nonStopwordExtras = unmatchedTokens.filter(t => !STOPWORDS.has(t));
    if (nonStopwordExtras.length > 2) continue; // Allow up to 2 extra words
