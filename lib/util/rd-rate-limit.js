// Simple token-bucket + concurrency limiter for Real-Debrid API calls
// Defaults to 250 requests/min and 4 concurrent in-flight requests

class RdRateLimiter {
  constructor({ ratePerMinute = 250, concurrency = 4 } = {}) {
...
const limiter = new RdRateLimiter({
  ...
  concurrency: parseInt(process.env.RD_CONCURRENCY || '4', 10)
});
