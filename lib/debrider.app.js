...
async function searchWithPersonalCloud(apiKey, type, id, userConfig = {}, baseUrl = BASE_URL) {
    ...
    try {
        ...
        const personalCloudPromise = (async () => {
            try {
                ...
                console.error(`[${LOG_PREFIX}] Error fetching personal cloud tasks: ${error.message}`);
                return [];
            }
        })();

        const cachedSearchPromise = search(apiKey, type, id, userConfig);

        const newznabPromise = (async () => {
            if (!userConfig.newznabUrl || !userConfig.newznabApiKey) {
                return [];
            }

            console.log(`[${LOG_PREFIX}] Searching Newznab for NZBs in parallel...`);
            try {
                let query = '';
                let category = '';
        ...
        const [personalStreams, regularStreams, newznabStreams] = await Promise.all([
            personalCloudPromise,
            cachedSearchPromise,
            newznabPromise
        ]);
