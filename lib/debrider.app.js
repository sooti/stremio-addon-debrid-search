...
const addHashToMongo = (hash, fileName = null, size = null, data = null) => debridHelpers.addHashToMongo(hash, fileName, size, data, 'debriderapp');
...
async function search(apiKey, type, id, userConfig = {}) {
    ...
    try {
        ...
        let cachedHashesFromMongo = new Set();
        try {
            if (mongoCache?.isEnabled()) {
                const mongoHashes = await mongoCache.getCachedHashes('debriderapp', hashes);
                mongoHashes.forEach(h => cachedHashesFromMongo.add(h.toLowerCase()));
                console.log(`[${LOG_PREFIX}] Found ${cachedHashesFromMongo.size} hashes in MongoDB cache`);
            }
        } catch (error) {
        ...
        try {
            if (mongoCache?.isEnabled() && mergedTorrents.length > 0) {
                const upserts = [];
