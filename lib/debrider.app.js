...
async function search(apiKey, type, id, userConfig = {}) {
    ...
    try {
        ...
        const torrents = [].concat(...scraperResults);
        ...
        const mergedTorrents = cachedTorrents.map(cachedTorrent => {
            const originalTorrent = torrents.find(t => t.InfoHash === cachedTorrent.infoHash);
            const videoFile = cachedTorrent.files.find(file => isVideo(file.name));
            if (!videoFile) {
                return null;
            }
            return {
