...
async function checkCache(apiKey, hashes) {
    ...
    try {
        ...
        // Try different request formats based on common debrid API patterns
        const requestBody = { data: hashes };
        console.log(`[${LOG_PREFIX}] DEBUG - Request body format: { data: [hashes] }`);
        console.log(`[${LOG_PREFIX}] DEBUG - Sample body:`, JSON.stringify({ data: hashes.slice(0, 3) }, null, 2));

        const response = await axiosWithProxy.post(url, requestBody, {
            headers: {
        ...
        console.error(`[${LOG_PREFIX}] Invalid or empty cache response from debrider.app:`, JSON.stringify(response.data, null, 2));
        return [];
    } catch (error) {
        ...
        if (error.response) {
            ...
            console.error(`[${LOG_PREFIX}] Response statusText: ${error.response.statusText}`);
            console.error(`[${LOG_PREFIX}] Response headers:`, error.response.headers);
            console.error(`[${LOG_PREFIX}] Response data:`, JSON.stringify(error.response.data, null, 2));
        } else if (error.request) {
            console.error(`[${LOG_PREFIX}] No response received. Request:`, error.request);
        }
        return [];
    }
}
