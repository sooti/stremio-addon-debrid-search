...
async function checkCache(apiKey, hashes) {
    ...
    const requestFormats = [
        {
            name: 'Format 1: type=torrent, data=hashes',
            url: `${BASE_URL}/link/lookup`,
            body: { type: 'torrent', data: hashes },
            headers: { 'Authorization': `Bearer ${apiKey}`, 'Content-Type': 'application/json' }
        },
        {
            name: 'Format 2: type=magnet, data=hashes',
            url: `${BASE_URL}/link/lookup`,
            body: { type: 'magnet', data: hashes },
            headers: { 'Authorization': `Bearer ${apiKey}`, 'Content-Type': 'application/json' }
        },
        {
            name: 'Format 3: type=cache, data=hashes',
            url: `${BASE_URL}/link/lookup`,
            body: { type: 'cache', data: hashes },
            headers: { 'Authorization': `Bearer ${apiKey}`, 'Content-Type': 'application/json' }
        },
        {
            name: 'Format 4: type=lookup, data=hashes',
            url: `${BASE_URL}/link/lookup`,
            body: { type: 'lookup', data: hashes },
            headers: { 'Authorization': `Bearer ${apiKey}`, 'Content-Type': 'application/json' }
        },
        {
            name: 'Format 5: type=torrent, data=magnets',
            url: `${BASE_URL}/link/lookup`,
            body: { type: 'torrent', data: hashes.map(h => `magnet:?xt=urn:btih:${h}`) },
            headers: { 'Authorization': `Bearer ${apiKey}`, 'Content-Type': 'application/json' }
        },
        {
            name: 'Format 6: type=magnet, data=magnet-links',
            url: `${BASE_URL}/link/lookup`,
    ...
    for (const format of requestFormats) {
        try {
            ...
            console.log(`[${LOG_PREFIX}] DEBUG - Request URL: ${format.url}`);
            console.log(`[${LOG_PREFIX}] DEBUG - Sample body:`, JSON.stringify(
                Array.isArray(format.body)
                    ? format.body.slice(0, 3)
                    : format.body.hashes?.slice(0, 3) || format.body.data?.slice(0, 3) || format.body.magnets?.slice(0, 3) || format.body,
                null, 2
            ));

            const response = await axiosWithProxy.post(format.url, format.body, { headers: format.headers });
