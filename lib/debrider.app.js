...
async function search(apiKey, type, id, userConfig = {}) {
    ...
    console.log(`[${LOG_PREFIX}] Type: ${type}, ID: ${id}`);
    console.log(`[${LOG_PREFIX}] API Key present: ${!!apiKey}, length: ${apiKey?.length || 0}`);
    ...
    console.log(`[${LOG_PREFIX}] Cinemeta: ${cinemetaDetails.name} (${cinemetaDetails.year || 'no year'})`);
...
async function checkCache(apiKey, hashes) {
    ...
    console.log(`[${LOG_PREFIX}] DEBUG - Checking cache for ${hashes.length} hashes`);
    console.log(`[${LOG_PREFIX}] DEBUG - First 3 hashes: ${hashes.slice(0, 3).join(', ')}`);

    // Try multiple API formats and endpoints until we find one that works
    // Based on createNzbTask pattern, the API uses { type: 'xxx', data: ... } format
    const requestFormats = [
        {
            name: 'Format 1: GET /link/lookup with query params',
            method: 'GET',
            url: `${BASE_URL}/link/lookup?hashes=${hashes.join(',')}`,
            body: null,
            headers: { 'Authorization': `Bearer ${apiKey}` }
        },
        {
            name: 'Format 2: GET /cache/check with query params',
            method: 'GET',
            url: `${BASE_URL}/cache/check?hashes=${hashes.join(',')}`,
            body: null,
            headers: { 'Authorization': `Bearer ${apiKey}` }
        },
        {
            name: 'Format 3: type=torrent, data=single-magnet-string',
            method: 'POST',
            url: `${BASE_URL}/link/lookup`,
            body: { type: 'torrent', data: hashes.map(h => `magnet:?xt=urn:btih:${h}`).join('\n') },
            headers: { 'Authorization': `Bearer ${apiKey}`, 'Content-Type': 'application/json' }
        },
        {
            name: 'Format 4: Check first hash individually to find working format',
            method: 'POST',
            url: `${BASE_URL}/link/lookup`,
            body: { type: 'magnet', data: `magnet:?xt=urn:btih:${hashes[0]}` },
            headers: { 'Authorization': `Bearer ${apiKey}`, 'Content-Type': 'application/json' },
            singleHashTest: true
        },
        {
            name: 'Format 5: type=torrent, data=hashes-array',
            method: 'POST',
            url: `${BASE_URL}/link/lookup`,
            body: { type: 'torrent', data: hashes },
            headers: { 'Authorization': `Bearer ${apiKey}`, 'Content-Type': 'application/json' }
        },
        {
            name: 'Format 6: type=magnet, data=magnet-array',
            method: 'POST',
            url: `${BASE_URL}/link/lookup`,
            body: { type: 'magnet', data: hashes.map(h => `magnet:?xt=urn:btih:${h}`) },
            headers: { 'Authorization': `Bearer ${apiKey}`, 'Content-Type': 'application/json' }
        }
    ];
    ...
    for (const format of requestFormats) {
        try {
            console.log(`[${LOG_PREFIX}] DEBUG - Trying ${format.name}`);
            console.log(`[${LOG_PREFIX}] DEBUG - ${format.method} ${format.url}`);
            if (format.body) {
                console.log(`[${LOG_PREFIX}] DEBUG - Body:`, JSON.stringify(format.body, null, 2).substring(0, 500));
            }

            let response;
            if (format.method === 'GET') {
                response = await axiosWithProxy.get(format.url, { headers: format.headers });
            } else {
                response = await axiosWithProxy.post(format.url, format.body, { headers: format.headers });
            }

            // Special handling for single hash test
            if (format.singleHashTest) {
                console.log(`[${LOG_PREFIX}] DEBUG - ✅ Single hash test succeeded! This format works.`);
                console.log(`[${LOG_PREFIX}] DEBUG - Response:`, JSON.stringify(response.data, null, 2));
                console.log(`[${LOG_PREFIX}] DEBUG - Now checking all ${hashes.length} hashes individually...`);

                // If single hash test worked, check all hashes using this format
                const allResults = [];
                const batchSize = 10; // Check 10 at a time
                for (let i = 0; i < hashes.length; i += batchSize) {
                    const batch = hashes.slice(i, Math.min(i + batchSize, hashes.length));
                    const batchPromises = batch.map(hash =>
                        axiosWithProxy.post(format.url,
                            { type: 'magnet', data: `magnet:?xt=urn:btih:${hash}` },
                            { headers: format.headers }
                        ).then(res => ({ hash, response: res.data, success: true }))
                         .catch(err => ({ hash, error: err.message, success: false }))
                    );
                    const batchResults = await Promise.all(batchPromises);
                    allResults.push(...batchResults);
                    console.log(`[${LOG_PREFIX}] DEBUG - Checked ${Math.min(i + batchSize, hashes.length)}/${hashes.length} hashes`);
                }

                // Process results
                const cachedTorrents = allResults
                    .filter(r => r.success && r.response?.cached)
                    .map(r => ({
                        infoHash: r.hash,
                        ...r.response
                    }));

                console.log(`[${LOG_PREFIX}] Found ${cachedTorrents.length} cached torrents via individual checking`);
                return cachedTorrents;
            }

            console.log(`[${LOG_PREFIX}] DEBUG - ✅ ${format.name} succeeded! Response status: ${response.status}`);
            console.log(`[${LOG_PREFIX}] DEBUG - Response headers:`, response.headers);
            console.log(`[${LOG_PREFIX}] DEBUG - Full response data:`, JSON.stringify(response.data, null, 2));

            // Try different response structures
            let resultArray = [];

            if (response.data) {
                console.log(`[${LOG_PREFIX}] DEBUG - Response data keys:`, Object.keys(response.data));

                // Try parsing different response structures
                if (Array.isArray(response.data.result)) {
                    console.log(`[${LOG_PREFIX}] DEBUG - Found result array with ${response.data.result.length} items`);
                    resultArray = response.data.result;
                } else if (Array.isArray(response.data.data)) {
                    console.log(`[${LOG_PREFIX}] DEBUG - Found data array with ${response.data.data.length} items`);
                    resultArray = response.data.data;
                } else if (Array.isArray(response.data)) {
                    console.log(`[${LOG_PREFIX}] DEBUG - Response is direct array with ${response.data.length} items`);
                    resultArray = response.data;
                } else if (typeof response.data === 'object') {
                    // Maybe it's an object with hash keys like TorBox
                    console.log(`[${LOG_PREFIX}] DEBUG - Response is object, checking for hash keys`);
                    const keys = Object.keys(response.data);
                    if (keys.length > 0 && hashes.some(h => keys.includes(h.toLowerCase()) || keys.includes(h.toUpperCase()))) {
                        console.log(`[${LOG_PREFIX}] DEBUG - Response appears to be hash-keyed object`);
                        resultArray = hashes.map(hash => {
                            const key = keys.find(k => k.toLowerCase() === hash.toLowerCase());
                            if (key && response.data[key]) {
                                return { hash, cached: true, ...response.data[key] };
                            }
            ...
            if (resultArray.length > 0) {
                ...
                resultArray.forEach((item, index) => {
                    const itemHash = item.hash || item.infoHash || hashes[index];
                    const isCached = item.cached === true || item.cached === 1 || item.status === 'cached';

                    console.log(`[${LOG_PREFIX}] DEBUG - Item ${index}:`, JSON.stringify({
                ...
                console.log(`[${LOG_PREFIX}] DEBUG - Found ${cachedTorrents.length} cached torrents out of ${resultArray.length} checked`);
                return cachedTorrents;
            }

            console.log(`[${LOG_PREFIX}] DEBUG - ${format.name} returned empty results, but request succeeded`);
            return [];

        } catch (error) {
            lastError = error;
            console.log(`[${LOG_PREFIX}] DEBUG - ❌ ${format.name} failed: ${error.message}`);
            if (error.response) {
                console.log(`[${LOG_PREFIX}] DEBUG - Status: ${error.response.status}, Data:`, JSON.stringify(error.response.data, null, 2).substring(0, 200));
            }
    ...
    // All formats failed
    console.error(`[${LOG_PREFIX}] All request formats failed. Last error: ${lastError?.message}`);
    if (lastError?.response) {
        console.error(`[${LOG_PREFIX}] Last response status: ${lastError.response.status}`);
        console.error(`[${LOG_PREFIX}] Last response statusText: ${lastError.response.statusText}`);
        console.error(`[${LOG_PREFIX}] Last response data:`, JSON.stringify(lastError.response.data, null, 2));
    }
    return [];
}
