...
async function getUHDMoviesStreams(imdbId, tmdbId, mediaType = 'movie', season = null, episode = null, config = {}) {
  ...
  try {
    ...
    if (cachedLinks && cachedLinks.length > 0) {
      ...
      // Process link resolution in parallel for better performance
      const maxLinksToProcess = Math.min(10, downloadInfo.links.length); // Limit to avoid overwhelming
      const resolutionPromises = downloadInfo.links.slice(0, maxLinksToProcess).map(async (linkInfo) => {
        try {
    ...
    const streamPromises = cachedLinks.map(async (streamInfo) => {
      try {
        ...
        const title = `${cleanFileName}${renderLanguageFlags(detectedLanguages)}\n💾 ${streamInfo.size} | UHDMovies`;
