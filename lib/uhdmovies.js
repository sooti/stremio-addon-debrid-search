...
async function getUHDMoviesStreams(imdbId, tmdbId, mediaType = 'movie', season = null, episode = null, config = {}) {
  ...
  const cacheKey = `uhd_final_v20_${tmdbId}_${mediaType}${season ? `_s${season}e${episode}` : ''}`;
    ...
    const streamPromises = cachedLinks.map(async (linkInfo) => {
      try {
        ...
        if (finalUrl) {
          ...
          const encodedUrl = finalUrl ? encodeUrlForStreaming(finalUrl) : finalUrl;
        ...
        if (error.response && error.response.status === 410) {
          ...
          await MongoCache.getCollection().then(collection => collection.deleteOne({ _id: cacheKey }));
        }
        return null;
      }
    });
