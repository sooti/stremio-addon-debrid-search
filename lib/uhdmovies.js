  ...
  try {
    ...
    if (keys) {
      ...
      if (UHDMOVIES_PROXY_URL) {
        ...
        console.log(`[UHDMovies] Making legacy proxied POST request for Instant Download API to: ${apiUrl}`);
        ...
        console.log(`[UHDMovies] Making proxied POST request via debrid-proxy for Instant Download API to: ${apiUrl}`);
...
async function getFinalLink(redirectUrl) {
  try {
    ...
    const downloadMethods = [
      { name: 'Resume Cloud', func: (dom) => tryResumeCloud(dom, new URL(redirectUrl).origin) },
      { name: 'Instant Download', func: (dom) => tryInstantDownload(dom, redirectUrl) }
    ];
...
async function getUHDMoviesStreams(imdbId, tmdbId, mediaType = 'movie', season = null, episode = null, config = {}) {
  ...
  try {
      ...
      const extractTimerId = `[UHDMovies-${requestId}] extractDownloadLinks`;
