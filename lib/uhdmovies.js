...
async function getUHDMoviesStreams(imdbId, tmdbId, mediaType = 'movie', season = null, episode = null, config = {}) {
  ...
  try {
    ...
    if (cachedLinks && cachedLinks.length > 0) {
      ...
      console.time(`[UHDMovies] Resolving ${downloadInfo.links.length} SID links`);
      ...
      console.timeEnd(`[UHDMovies] Resolving ${downloadInfo.links.length} SID links`);
      console.log(`[UHDMovies] Cached links after resolution:`, cachedLinks);
    ...
    console.time(`[UHDMovies] Processing ${cachedLinks.length} cached driveleech redirect URLs`);
    ...
    const streamPromises = cachedLinks.map(async (linkInfo) => {
      try {
        ...
        if (finalUrl) {
          ...
          if (finalUrl.includes('video-leech.pro') || finalUrl.includes('cdn.video-leech.pro')) {
            try {
              console.log(`[UHDMovies] Post-processing video-leech link to extract Google URL: ${finalUrl.substring(0, 100)}...`);

              // Follow the redirect to get the final URL with ?url= parameter
              const redirectResponse = await makeRequest(finalUrl, {
                maxRedirects: 5,
                headers: {
                  'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,*/*;q=0.8',
                  'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36'
              ...
              // Check if the redirect went to video-seed.pro with ?url= parameter
              if (redirectResponse.request && redirectResponse.request.res && redirectResponse.request.res.responseUrl) {
                const redirectedUrl = redirectResponse.request.res.responseUrl;
                console.log(`[UHDMovies] Redirected to: ${redirectedUrl.substring(0, 150)}...`);
                ...
                if (redirectedUrl.includes('video-seed.pro') && redirectedUrl.includes('?url=')) {
                  try {
                    ...
                    const urlParam = urlObj.searchParams.get('url');
                    if (urlParam && urlParam.includes('googleusercontent.com')) {
                      console.log(`[UHDMovies] ✓ Extracted Google URL from video-seed.pro redirect: ${urlParam.substring(0, 100)}...`);
                      processedUrl = urlParam;
                    }
                  } catch (urlParseError) {
                    console.log(`[UHDMovies] Failed to parse redirect URL: ${urlParseError.message}`);
                  }
                }
              }
            } catch (postProcessError) {
              console.log(`[UHDMovies] Post-processing failed, using original URL: ${postProcessError.message}`);
            }
          }
          ...
          const encodedUrl = processedUrl ? encodeUrlForStreaming(processedUrl) : processedUrl;
        ...
        if (error.response && error.response.status === 410) {
          console.log(`[UHDMovies] Detected 410 error for ${linkInfo.driveleechRedirectUrl}. Clearing cache for ${cacheKey}.`);
          const collection = await MongoCache.getCollection();
          if (collection) {
    ...
    console.timeEnd(`[UHDMovies] Processing ${cachedLinks.length} cached driveleech redirect URLs`);
