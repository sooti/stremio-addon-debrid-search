...
const UHDMEMORY_CACHE = new Map();
const UHDMEMORY_CACHE_TTL = 5 * 60 * 1000; // 5 minutes in-memory cache

const getFromCache = async (key) => {
  // First check in-memory cache
  const memoryCacheKey = `memory_${key}`;
  const memoryCached = UHDMEMORY_CACHE.get(memoryCacheKey);
  if (memoryCached) {
    const now = Date.now();
    if (now - memoryCached.timestamp < UHDMEMORY_CACHE_TTL) {
      console.log(`[UHDMovies] Memory cache HIT for ${key}`);
      return memoryCached.data;
    } else {
      // Remove expired entry
      UHDMEMORY_CACHE.delete(memoryCached);
    }
  }
  ...
  // Add to in-memory cache if found in mongo
  if (result) {
    UHDMEMORY_CACHE.set(memoryCacheKey, {
      data: result,
      timestamp: Date.now()
    });
  }
...
function compareMedia(mediaInfo, searchResult) {
  ...
  const normalizeForWordMatching = (str) => String(str || '').toLowerCase()
    ...
    .replace(/\s+/g, ' ')
    .trim();
  const normalizeForTitleStartMatching = (str) => String(str || '').toLowerCase()
    .replace(/[^a-zA-Z0-9\s]/g, ' ')
    .replace(/\s+/g, ' ')
    .trim()
    .split(' ')
  ...
  if (mediaTitleWords.length > 0) {
    ...
    if (resultTitleWords.length >= mediaTitleWords.length) {
      ...
      if (startsWithTitle) {
        ...
        const cleanedResult = resultTextLower.replace(/^(download|watch|stream)\s+/i, '');
        ...
        if (cleanedResult.startsWith(mediaTitleForPattern + ' ') || 
            cleanedResult.startsWith(mediaTitleForPattern + '(') || 
            cleanedResult.startsWith(mediaTitleForPattern + '-') ||
            cleanedResult.startsWith(mediaTitleForPattern + ':') ||
        ...
        // For single words, also check if they appear as standalone words at the start
          ...
          if (cleanedResult.startsWith(mediaWord + ' ') || 
              cleanedResult.startsWith(mediaWord + '(') || 
              cleanedResult.startsWith(mediaWord + '-') || 
              cleanedResult.startsWith(mediaWord + ':') ||
...
async function getUHDMoviesStreams(imdbId, tmdbId, mediaType = 'movie', season = null, episode = null, config = {}) {
  ...
  const cacheKey = `uhd_final_v27_${tmdbId}_${mediaType}${season ? `_s${season}e${episode}` : ''}`;
  ...
  try {
    ...
    if (cachedLinks && cachedLinks.length > 0) {
      ...
      console.log(`[UHDMovies] Prepared ${cachedLinks.length} validated SID URLs for lazy resolution`);

      // 7. Cache the original SID URLs (not driveleech URLs - lazy resolution!)
      if (cachedLinks.length > 0) {
        console.log(`[UHDMovies] Caching ${cachedLinks.length} SID URLs for key: ${cacheKey}`);
        await saveToCache(cacheKey, cachedLinks);
      } else {
        console.log(`[UHDMovies] No SID URLs found. Not caching to allow retrying later.`);
        return [];
      }
    }
