  ...
  try {
    ...
    if (hasProxy) {
      const customAxios = axios.create({
        ...
        timeout: proxyConfig.timeout || 60000, // Use configured timeout or 60s default
        socketTimeout: proxyConfig.socketTimeout || 60000
      });

      // Pass the custom axios instance to the AllDebridClient
      return new AllDebridClient(apiKey, { ...options, axios: customAxios });
    } else {
      // No proxy needed, use default configuration with timeout
      const customAxios = axios.create({
        timeout: parseInt(process.env.DEBRID_REQUEST_TIMEOUT || '60000', 10),
        socketTimeout: parseInt(process.env.DEBRID_SOCKET_TIMEOUT || '60000', 10)
      });
      return new AllDebridClient(apiKey, { ...options, axios: customAxios });
    }
  } catch (error) {
