...
async function searchAllDebridTorrents(apiKey, type, id, userConfig = {}) {
  ...
  try {
    ...
    cachedResults = await processAndFilterTorrents(externalTorrents, adHandler, episodeInfo, externalOnlyQuotas);
    ...
    if (nonCachedTorrents.length > 0 && !hasEnoughHQResults()) {
      const topNonCached = nonCachedTorrents.sort((a, b) => (b.Seeders || 0) - (a.Seeders || 0)).slice(0, 10); // Increased from 5 to 10
