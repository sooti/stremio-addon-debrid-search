    ...
    const scraperResults = await searchCoordinator.executeSearch(
      ...
      async () => {
        ...
        const pushAllScrapers = (cfg) => {
          ...
          if (config.MAGNETDL_ENABLED) scraperPromises.push(scrapers.searchMagnetDL(key, signal, LOG_PREFIX, cfg));
          if (config.WOLFMAX4K_ENABLED) scraperPromises.push(scrapers.searchWolfmax4K(key, signal, LOG_PREFIX, cfg));
          if (config.BLUDV_ENABLED) scraperPromises.push(scrapers.searchBluDV(key, signal, LOG_PREFIX, cfg));
        };
