  ...
  try {
    ...
    if (stremThru.isEnabled() && apiKey) {
      try {
        ...
        stremthruCachedHashes = await stremThru.checkInstantAvailability(allExternalHashes, 'alldebrid', apiKey, clientIp);
    ...
    if (uncachedExternalTorrents.length > 0) {
      ...
      const adHandler = {
          ...
          if (stremThru?.isEnabled() && apiKey && cached.size < newHashes.length) {
            try {
              ...
              if (hashesToCheck.length > 0) {
                const stremthruResults = await stremThru.checkInstantAvailability(hashesToCheck, 'alldebrid', apiKey, clientIp);
        ...
        cleanup: async () => {
          ...
          if (magnetIdsToDelete.size > 0) cleanupTemporaryMagnets(AD, Array.from(magnetIdsToDelete), apiKey);
        }
      };
