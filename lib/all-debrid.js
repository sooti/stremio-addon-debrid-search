...
function createAllDebridClient(apiKey, options = {}) {
  ...
  return new AllDebridClient(apiKey, clientOptions);
}
...
async function getMagnetFiles(AD, magnetIds) {
  ...
  try {
    // Use the internal _post method from AllDebridClient
...
async function searchAllDebridTorrents(apiKey, type, id, userConfig = {}) {
  ...
  try {
    ...
    // Phase 1.5: SQLite counts are for scraper optimization only, NOT for quota satisfaction
    const releaseKey = makeReleaseKey(type, imdbId, episodeInfo?.season, episodeInfo?.episode);

    // ONLY personal files count toward quotas (not SQLite hash metadata)
    // SQLite cache is used to avoid API calls, but cached hashes still need to be returned as streams
    ...
    // Use doubled RealDebrid limits for AllDebrid to allow more results
    const adDefaultMax = parseInt(process.env.MAX_RESULTS_PER_QUALITY, 10) || 2;
    const adQualityLimits = {
      'Remux': (parseInt(process.env.MAX_RESULTS_REMUX, 10) || adDefaultMax) * 2,
      'BluRay': (parseInt(process.env.MAX_RESULTS_BLURAY, 10) || adDefaultMax) * 2,
      'WEB/WEB-DL': (parseInt(process.env.MAX_RESULTS_WEBDL, 10) || adDefaultMax) * 2,
      'BRRip/WEBRip': (parseInt(process.env.MAX_RESULTS_WEBRIP, 10) || 1) * 2,
      'Audio-Focused': (parseInt(process.env.MAX_RESULTS_AUDIO, 10) || 1) * 2,
      'Other': (parseInt(process.env.MAX_RESULTS_OTHER, 10) || 10) * 2
    };
    ...
    const personalHighResSatisfied = HQ_CATEGORIES.every(cat => {
      const limit = adQualityLimits[cat];
    ...
    const adHandler = {
      ...
      checkCachedHashes: async (hashes) => {
        ...
        // Use SQLite cache when available
        if (sqliteCache?.isEnabled()) {
            try {
                const sqliteHashes = await sqliteCache.getCachedHashes('alldebrid', hashes);
                sqliteHashes.forEach(h => cached.add(h.toLowerCase()));
      ...
      liveCheckHash: async (hash) => {
        ...
        try {
          ...
          console.log(`[AD CACHE-CHECK] Live check for ${hash.substring(0,16)}... failed: ${error.message}`);
          // Don't return false right away - may still be able to use the file if we have it cached
        }
        return false;
      },
      batchCheckSeasonPacks: async (hashes, season, episode) => {
        // Enable pack inspection for AllDebrid using available APIs with doubled limit
        const MAX_PACKS_TO_INSPECT = (config.MAX_PACKS_TO_INSPECT || 3) * 2; // Double the pack inspection limit
        ...
        for (const hash of hashes) {
            ...
            try {
                ...
                console.error(`[AD PACK INSPECT] Error inspecting pack ${hash.substring(0,8)}: ${error.message}`);
                // Clean up any temporary magnet on error - but we can't do this here since we don't have the magnet ID in error scope
            }
        }
    ...
    // For immediate response, check SQLite cache for already cached results
    // The detailed cache checking will happen in background
    cachedResults = [];
    nonCachedTorrents = [];

    // Define externalOnlyQuotas here so it's accessible in both sync and async contexts
    const externalOnlyQuotas = { byCategory: {}, byCategoryResolution: {} };

    // Start background processing to update cache
    setImmediate(async () => {
        try {
            ...
            // For external torrent processing: Respect quotas for non-personal files
            // Personal files are user-specific, but external torrents should still respect quality quotas
            const backgroundCachedResults = await processAndFilterTorrents(externalTorrents, adHandler, episodeInfo, externalOnlyQuotas, false);
            const backgroundNonCachedTorrents = externalTorrents.filter(t => !backgroundCachedResults.some(c => c.InfoHash === t.InfoHash));
            ...
            const hasEnoughHQResults = () => {
              ...
              return HQ_CATEGORIES.some(category => {
                ...
                const results1080p = [...personalFiles, ...backgroundCachedResults].filter(r =>
                  r.category === category && r.resolution === '1080p'
                ).length;
                const results2160p = [...personalFiles, ...backgroundCachedResults].filter(r =>
            ...
            console.log(`[${LOG_PREFIX}] Background cache check completed for ${externalTorrents.length} external torrents`);
        } catch (error) {
    ...
    // Meanwhile, check the SQLite cache synchronously for the most recent cached results
    // This allows us to return cached results immediately while the live checking runs in background
    try {
        const cachedHashes = new Set();
        if (sqliteCache?.isEnabled()) {
            const externalHashes = externalTorrents.map(t => (t.InfoHash || t.hash || '').toLowerCase()).filter(Boolean);
            if (externalHashes.length > 0) {
                const results = await sqliteCache.getCachedHashes('alldebrid', externalHashes);
        ...
        // Filter cached results to respect quotas
        const cachedExternalTorrents = externalTorrents.filter(t => cachedHashes.has((t.InfoHash || t.hash || '').toLowerCase()));
        
        // Apply quota filtering to cached external results to ensure they respect quality/resolution limits
        const quotasAppliedResults = await processAndFilterTorrents(cachedExternalTorrents, adHandler, episodeInfo, externalOnlyQuotas, false);
        
        // Return only the cached external results that pass quota checks, along with personal files
        cachedResults = quotasAppliedResults;
        nonCachedTorrents = externalTorrents.filter(t => !cachedHashes.has((t.InfoHash || t.hash || '').toLowerCase()));
    } catch (error) {
        console.error(`[${LOG_PREFIX}] Error getting cached results:`, error.message);
        // Fallback to returning just personal files if there's an error
        cachedResults = [];
