  ...
  try {
    ...
    const adHandler = {
      ...
      checkCachedHashes: async (hashes) => {
        ...
        // Use MongoDB cache when available
        if (mongoCache?.isEnabled()) {
            try {
                const mongoHashes = await mongoCache.getCachedHashes('alldebrid', hashes);
                mongoHashes.forEach(h => cached.add(h.toLowerCase()));
                if (mongoHashes.length > 0) {
                    console.log(`[AD CACHE-CHECK] MongoDB returned ${mongoHashes.length}/${hashes.length} cached hashes`);
                }
            } catch (error) {
          ...
          if (magnetId) {
            ...
            if (hasVideo && hasValidSize) {
              ...
              try {
                ...
                await addHashToMongo(hash, largestVideo?.path || null, largestVideo?.bytes || null, { status: 'cached', magnetId: magnetId });
              } catch {}
    ...
    // For immediate response, check MongoDB cache for already cached results
    ...
    // Meanwhile, check the MongoDB cache synchronously for the most recent cached results
    ...
    try {
        ...
        if (mongoCache?.isEnabled()) {
            ...
            if (externalHashes.length > 0) {
                const results = await mongoCache.getCachedHashes('alldebrid', externalHashes);
