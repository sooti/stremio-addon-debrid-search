...
import { getCachedHashes as mongoGetCachedHashes, upsertCachedMagnet as mongoUpsert, upsertCachedMagnets as mongoUpsertMany, getReleaseCounts as mongoGetReleaseCounts, default as mongoCache } from './common/mongo-cache.js';
...
function deferMongoUpserts(payloads = []) {
  try {
    ...
    setImmediate(() => {
      mongoUpsertMany(payloads).catch(err => {
        console.error(`[AD MONGO] Background bulk upsert failed: ${err.message}`);
      });
    });
  } catch (err) {
    console.error(`[AD MONGO] Error deferring mongo upserts: ${err.message}`);
  }
}
  ...
  try {
    ...
    const adHandler = {
      ...
      checkCachedHashes: async (hashes) => {
        ...
        if (mongoCache?.isEnabled()) {
            try {
                const mongoHashes = await mongoGetCachedHashes('alldebrid', hashes);
        ...
        // If not all hashes are found in Mongo, check AllDebrid for the remaining ones
        const remainingHashes = hashes.filter(h => !cached.has(h.toLowerCase()));
        if (remainingHashes.length > 0) {
            try {
                const magnets = remainingHashes.map(h => `magnet:?xt=urn:btih:${h}`);
                ...
                for (const entry of list) {
                    ...
                    if (h && instant) {
                        cached.add(h);
                        // Add the newly found hash to Mongo for future checks
                        addHashToMongo(h);
                    }
                }
            } catch (error) {
                console.error(`[AD API] Error checking instant availability: ${error.message}`);
            }
        }
