...
const adCall = (fn, apiKey) => AdLimiter.schedule(fn, 'ad-call', apiKey);
...
function createAllDebridClient(apiKey, options = {}) {
  // Get proxy agent for alldebrid service (all-debrid-api uses 'request' library, not axios)
  const proxyAgent = debridProxyManager.getProxyAgent('alldebrid');

  // Log if proxy is being used
  if (proxyAgent) {
    const proxyUrl = debridProxyManager.getProxyUrl('alldebrid');
    console.log(`[AD] Proxy configured for AllDebrid API calls: ${proxyUrl}`);
  } else {
    console.log('[AD] No proxy configured for AllDebrid API calls');
  }

  try {
    // all-debrid-api uses the 'request' library, which accepts 'agent' option
    const clientOptions = {
      ...options,
      timeout: parseInt(process.env.DEBRID_REQUEST_TIMEOUT || '60000', 10)
    };

    // If proxy agent exists, pass it for both HTTP and HTTPS
    if (proxyAgent) {
    ...
    console.warn('[AD] Failed to create AllDebrid client with proxy agent, trying default:', error.message);
 ...
 * @param {string} apiKey - The API key for rate limiting
 * @returns {Promise<Object>} Response with magnets array containing file trees
 */
async function getMagnetFiles(AD, magnetIds, apiKey) {
...
async function cleanupTemporaryMagnets(AD, magnetIds, apiKey) {
  ...
  for (const id of magnetIds) {
    try {
      await adCall(() => AD.magnet.delete(id), apiKey);
    } catch (deleteError) {
...
async function searchAllDebridTorrents(apiKey, type, id, userConfig = {}, clientIp = null, isBackgroundRefresh = false) {
  ...
  try {
    ...
    if (userConfig.enablePersonalCloud !== false) {
      personalFiles = await searchPersonalFiles(apiKey, searchKey, 0.3);
    } else {
    ...
    if (stremThru.isEnabled() && apiKey) {
      try {
        ...
        stremthruCachedHashes = await stremThru.checkInstantAvailability(allExternalHashes, 'alldebrid', apiKey, clientIp, sid);
    ...
    if (uncachedExternalTorrents.length > 0) {
      ...
      const adHandler = {
        ...
        checkCachedHashes: async (hashes) => {
          ...
          console.log(`[AD CACHE-CHECK] Checking ${newHashes.length} new hashes not checked in foreground`);

          let cached = new Set();
          // First check SQLite cache for existing results
          if (sqliteCache?.isEnabled()) {
              try {
                  const sqliteHashes = await sqliteCache.getCachedHashes('alldebrid', newHashes);
          ...
          if (stremThru?.isEnabled() && apiKey && cached.size < newHashes.length) {
            try {
              ...
              if (hashesToCheck.length > 0) {
                const stremthruResults = await stremThru.checkInstantAvailability(hashesToCheck, 'alldebrid', apiKey, clientIp, sid);
        ...
        liveCheckHash: async (hash) => {
          ...
          try {
            ...
            const response = await adCall(() => AD.magnet.upload(magnet), apiKey);
            ...
            if (magnetId) {
              ...
              const filesResponse = await getMagnetFiles(AD, magnetId, apiKey);
        ...
        cleanup: async () => {
          ...
          if (magnetIdsToDelete.size > 0) cleanupTemporaryMagnets(AD, magnetIdsToDelete, apiKey);
        }
      };
      ...
      setImmediate(async () => {
        try {
          ...
          const backgroundCachedResults = await processAndFilterTorrents(uncachedExternalTorrents, adHandler, episodeInfo, { byCategory: {}, byCategoryResolution: {} }, bypassQuotas);
    ...
    await saveHashCache();
    cleanupTemporaryMagnets(createAllDebridClient(apiKey), magnetIdsToDelete, apiKey);
  }
}
...
async function searchPersonalFiles(apiKey, searchKey, threshold = 0.3) {
  ...
  try {
    ...
    if (!searchKey) {
      console.log(`[${LOG_PREFIX}] Personal files search called with empty searchKey, returning empty results`);
      return [];
    }

    const existingDownloads = await getAllDownloads(AD, apiKey).catch(() => []);
...
async function processMagnetAlternative(apiKey, magnetUrl, clientIp) {
  ...
  try {
    ...
    const up = await adCall(() => AD.magnet.upload(magnetUrl), apiKey);
    const created = extractADMagnet(up) || {};
    const magnetId = created.id;
    if (!magnetId) return null;

    // Use magnet/files API to get file list
    const filesResp = await getMagnetFiles(AD, magnetId, apiKey);
...
async function unrestrictUrl(apiKey, hostUrl, clientIp) {
  ...
  try {
    ...
    if (hostUrl.startsWith('alldebrid:')) {
      ...
      const response = await adCall(() => AD.link.unlock(link), apiKey);
      ...
      const response = await adCall(() => AD.link.unlock(hostUrl), apiKey);
...
async function getAllDownloads(AD, apiKey) {
  try {
    const response = await adCall(() => AD.user.recentLinks(), apiKey);
...
async function listTorrents(apiKey, skip = 0) {
  const AD = createAllDebridClient(apiKey);
  try {
    const downloads = await getAllDownloads(AD, apiKey);
...
async function checkAndProcessCache(apiKey, externalTorrents) {
    ...
    checkCachedHashes: async (hashes) => {
      ...
      if (stremThru?.isEnabled() && apiKey && cached.size < hashes.length) {
        try {
          ...
          if (hashesToCheck.length > 0) {
            const stremthruResults = await stremThru.checkInstantAvailability(hashesToCheck, 'alldebrid', apiKey, null);
            for (const hash of stremthruResults) {
              cached.add(hash.toLowerCase());
            }
          }
  ...
  let cachedResults = await processAndFilterTorrents(externalTorrents, adHandler, null, {}, hasStremThruResults);
