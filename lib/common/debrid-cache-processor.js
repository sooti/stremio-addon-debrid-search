...
export async function processAndFilterTorrents(torrents, handler, episodeInfo = null) {
    ...
    try {
        ...
        if (preCachedHashes.size > 0 && handler && typeof handler.onHashChecked === 'function') {
            try { for (const h of preCachedHashes) handler.onHashChecked(h, true, { from: 'Pre-check' }); } catch {}
        }
    } catch (err) {
            ...
            if (seasonPackHashesToInspect.size > 0) {
                ...
                try {
                    ...
                    for (const [k, v] of foundMap.entries()) {
                        ...
                        try { handler?.onHashChecked && handler.onHashChecked(String(k).toLowerCase(), true, { from: 'Batch Pack Inspection' }); } catch {}
                    }
    ...
    const checkTorrentsInTiers = async (tier) => {
        ...
        for (const torrent of tier.torrents) {
            ...
            // notify handler about the decision for this hash
            try { handler?.onHashChecked && handler.onHashChecked(InfoHash, isCached, { from }); } catch {}
    ...
    try {
        ...
        const checkLimitsAndExit = () => {
            ...
            const webdlCount = cachedResults.filter(r => r.category === 'WEB/WEB-DL' && (r.resolution === '2160p' || r.resolution === '1080p')).length;
