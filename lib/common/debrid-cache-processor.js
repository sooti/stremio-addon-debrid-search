 ...
 */
export async function processAndFilterTorrents(torrents, handler, episodeInfo = null, satisfiedQuotas = {}, bypassQuotas = false) {
    ...
    const shouldBypassQuotas = bypassQuotas || (handler && typeof handler.bypassQuotas === 'boolean' && handler.bypassQuotas);
    ...
    const checkTorrentsInTiers = async (tier) => {
        ...
        // Pre-filter torrents that don't need live checking (already pre-cached or don't meet criteria)
        const prefilteredTorrents = [];
        for (const torrent of tier.torrents) {
            ...
            // Add to prefiltered list to process in parallel
            prefilteredTorrents.push(torrent);
        }
        ...
        // Process live checks in parallel with configurable concurrency limit
        if (prefilteredTorrents.length > 0 && handler && typeof handler.liveCheckHash === 'function') {
            const checkTorrent = async (torrent) => {
                const { category, resolution, InfoHash } = torrent;
                let isCached = false;
                let from = '';
                ...
                return { torrent, isCached, from };
            };

            // Process all prefiltered torrents in parallel with concurrency limit
            const results = await parallelLimit(prefilteredTorrents, checkTorrent, CONCURRENCY_LIMIT);

            // Process results and add to cachedResults
            for (const { torrent, isCached, from } of results) {
                const { category, resolution } = torrent;
                ...
                if (isCached) {
                    ...
                    // Check if we've satisfied the quota for this specific category/resolution
                    if (!shouldBypassQuotas) {
                        const newTotalInThisRes = categoryResolutionTracker[category][resolution] || 0;
                        const neededForThisCategoryRes = Math.max(0, (qualityLimits[category] || defaultMax) - getSatisfiedByCategoryRes(category, resolution));
            ...
            // Check early exit conditions after processing the entire tier (only if not bypassing quotas)
            if (!shouldBypassQuotas) {
            ...
            // Check if handler has been aborted after processing all results in this tier
            if (handler && typeof handler.isAborted === 'function' && handler.isAborted()) {
                console.log(`[${LOG_PREFIX} CACHE] ⛔ Search aborted by handler - stopping cache checks`);
                return true; // Early exit
            }
        } else {
            // Handle torrents that were already pre-cached or don't need live checking
            for (const torrent of prefilteredTorrents) {
                const { category, resolution, InfoHash } = torrent;
                
                if (preCachedHashes.has(InfoHash)) {
                    cachedResults.push({ ...torrent, source: LOG_PREFIX.toLowerCase(), isCached: true, from: 'API Batch' });
                    ...
                    console.log(`[${LOG_PREFIX} CACHE] ✅ CACHED [${category} ${resolution}] via API Batch: "${torrent.name.substring(0, 50)}"`);
                }
            }
            
            // Check if handler has been aborted after processing this tier
            if (handler && typeof handler.isAborted === 'function' && handler.isAborted()) {
                console.log(`[${LOG_PREFIX} CACHE] ⛔ Search aborted by handler - stopping cache checks`);
                return true; // Early exit
            }
