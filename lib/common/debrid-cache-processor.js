...
export async function processAndFilterTorrents(torrents, handler, episodeInfo = null, satisfiedQuotas = {}) {
    ...
    // Filter to only include torrents that are cached to avoid unnecessary processing
    const cachedEnrichedTorrents = enrichedTorrents.filter(t => preCachedHashes.has(t.InfoHash));
    
    if (debugLogsEnabled) console.log(`[${LOG_PREFIX} CACHE] Grouping and prioritizing ${cachedEnrichedTorrents.length} potential cached torrents (out of ${enrichedTorrents.length} total).`);

    const groupedTorrents = cachedEnrichedTorrents.reduce((groups, torrent) => {
    ...
    const inspectAllSeasonPacks = async () => {
        ...
        for (const pack of uniqueSeasonPacks) {
            if (validSeasonPacks.has(pack.InfoHash)) {
                ...
                const inspectionResult = validSeasonPacks.get(pack.InfoHash);
                let resultToPush;
