...
export async function processAndFilterTorrents(torrents, handler, episodeInfo = null, satisfiedQuotas = {}) {
    ...
    if (debugLogsEnabled) console.log(`[${LOG_PREFIX} CACHE] Grouping and prioritizing ${enrichedTorrents.length} potential torrents.`);
    ...
    const inspectAllSeasonPacks = async () => {
        ...
        for (const pack of uniqueSeasonPacks) {
            if (validSeasonPackHashes.has(pack.InfoHash)) {
                ...
                if (debugLogsEnabled) console.log(`[${LOG_PREFIX} CACHE] ✅ CACHED PACK [${category} ${resolution}] via Batch Pack Inspection: "${name.substring(0, 50)}"`);
            }
        }
    };
    ...
    try {
        ...
        const checkLimitsAndExit = () => {
            ...
            if (remuxCount >= remuxLimit || blurayCount >= blurayLimit || webdlCount >= webdlLimit) {
                if (debugLogsEnabled) console.log(`[${LOG_PREFIX} CACHE] ✅ Sufficient high-quality results found (Remux: ${remuxCount}/${remuxLimit}, BluRay: ${blurayCount}/${blurayLimit}, WEB/WEB-DL: ${webdlCount}/${webdlLimit}). Skipping packs and lower resolutions.`);
                return true;
            }
            return false;
        };
        ...
        if (hasHQ1080pOrHigher) {
            if (debugLogsEnabled) console.log(`[${LOG_PREFIX} CACHE] ✅ Found at least one 1080p+ WEB/WEB-DL/BluRay/Remux (incl. DB). Skipping 720p and lower tiers.`);
        } else {
