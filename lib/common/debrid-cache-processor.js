...
export async function processAndFilterTorrents(torrents, handler, episodeInfo = null) {
    ...
    try {
        for (const tier of allTiers) {
            // New logic: Before processing a 720p tier, check if we have enough high-quality results.
            if (tier.name.includes('720p')) {
                ...
                // If we have met the quota for EITHER Remux or BluRay in high resolutions, stop.
                if (remuxCount >= remuxLimit || blurayCount >= blurayLimit) {
                     console.log(`[${LOG_PREFIX} CACHE] ✅ Sufficient high-quality results found (Remux: ${remuxCount}/${remuxLimit}, BluRay: ${blurayCount}/${blurayLimit}). Skipping 720p and lower.`);
                     break;
                }
            }
