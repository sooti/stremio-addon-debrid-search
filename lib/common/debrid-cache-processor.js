...
export async function processAndFilterTorrents(torrents, handler, episodeInfo = null) {
    ...
    const codecTracker = {}; 
    // Tracks total accepted results per resolution across all categories
    ...
    if (episodeInfo && handler.batchCheckSeasonPacks) {
        ...
        if (allSeasonPacks.length > 0) {
            ...
            // Inspect season packs in prioritized order (quality/resolution/size),
            // proceeding in rounds until we reach the configured pack limit
            // or exhaust the inspection budget.
            const MAX_ROUNDS = parseInt(process.env.MAX_PACK_ROUNDS || '3', 10);
            let round = 0;
            let offset = 0;
            
            // Target is to collect up to MAX_PACKS_TO_INSPECT valid packs overall
    ...
    const checkTorrentsInTiers = async (tier) => {
        ...
        for (const torrent of tier.torrents) {
            ...
            if (episodeInfo && episodeInfo.season && episodeInfo.episode) {
                ...
                if (isSeasonPack) {
                    if (validSeasonPackHashes.has(InfoHash)) {
                        // Enforce the same rules/filters and quotas for packs
            ...
            // Enforce per-resolution total cap before doing cache checks
            const resolutionCap = parseInt(config.TARGET_CODEC_COUNT, 10) || 0;
            if (resolutionCap > 0) {
                const resTotal = resolutionTotalTracker[resolution] || 0;
                if (resTotal >= resolutionCap) {
