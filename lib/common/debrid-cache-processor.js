...
// The problematic getQualityCategory import is removed from here
...
export async function processAndFilterTorrents(torrents, handler, episodeInfo = null) {
    ...
    const inspectAllSeasonPacks = async () => {
        ...
        // Add valid packs to results, respecting limits
        for (const pack of uniqueSeasonPacks) {
            if (validSeasonPackHashes.has(pack.InfoHash)) {
                // This logic is simplified from checkTorrentsInTiers for packs
                const { category, resolution, name } = pack;
                const limit = qualityLimits[category] || defaultMax;
                if ((categoryResolutionTracker[category]?.[resolution] || 0) >= limit) continue;
    ...
    const checkTorrentsInTiers = async (tier) => {
        ...
        for (const torrent of tier.torrents) {
            ...
            if (episodeInfo) continue; // Individual file checks are skipped for episodes if packs are handled
    ...
    try {
        ...
        // Phase 1: Check all high-resolution individual files
        ...
        // Decision Point: Check if we have enough high-quality results to skip packs and low-res files
        const checkLimitsAndExit = () => {
            const remuxCount = cachedResults.filter(r => r.category === 'Remux' && (r.resolution === '2160p' || r.resolution === '1080p')).length;
            const blurayCount = cachedResults.filter(r => r.category === 'BluRay' && (r.resolution === '2160p' || r.resolution === '1080p')).length;
            const remuxLimit = qualityLimits['Remux'] || defaultMax;
            const blurayLimit = qualityLimits['BluRay'] || defaultMax;
            if (remuxCount >= remuxLimit || blurayCount >= blurayLimit) {
                console.log(`[${LOG_PREFIX} CACHE] ✅ Sufficient high-quality results found (Remux: ${remuxCount}/${remuxLimit}, BluRay: ${blurayCount}/${blurayLimit}). Skipping packs and lower resolutions.`);
                return true;
            }
            return false;
        };
        ...
        // Phase 2: If needed, inspect season packs
        await inspectAllSeasonPacks();

        // Decision Point 2: Check limits again after packs have been added
        if (checkLimitsAndExit()) return cachedResults;

        // Phase 3: If still needed, check low-resolution individual files
