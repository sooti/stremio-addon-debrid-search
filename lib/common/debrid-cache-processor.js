...
export async function processAndFilterTorrents(torrents, handler, episodeInfo = null) {
    ...
    let validSeasonPackHashes = new Set();
    if (episodeInfo && handler.batchCheckSeasonPacks) {
        ...
        if (allSeasonPacks.length > 0) {
            ...
            if (seasonPackHashesToInspect.size > 0) {
                console.log(`[${LOG_PREFIX} CACHE] Prioritized and selected top ${seasonPackHashesToInspect.size} (max ${MAX_PACKS_TO_INSPECT}) unique season packs for inspection...`);
                validSeasonPackHashes = await handler.batchCheckSeasonPacks(seasonPackHashesToInspect, season, episode);
                console.log(`[${LOG_PREFIX} CACHE] Found ${validSeasonPackHashes.size} valid packs containing the episode.`);
            }
        }
    }
            ...
            if (episodeInfo && episodeInfo.season && episodeInfo.episode) {
                ...
                if (isSeasonPack) {
                    if (validSeasonPackHashes.has(InfoHash)) {
                        cachedResults.push({ ...torrent, source: LOG_PREFIX.toLowerCase(), isCached: true, from: 'Batch Pack Inspection' });
