...
export async function searchStremthru(mediaType, mediaId, signal, season, episode, logPrefix, config) {
  const res = await base.searchStremthru(mediaType, mediaId, signal, season, episode, logPrefix, config);
  return applyPornFilter(res);
}
...
export async function searchWolfmax4K(query, signal, logPrefix, config) {
  const res = await base.searchWolfmax4K(query, signal, logPrefix, config);
  return applyPornFilter(res);
}
