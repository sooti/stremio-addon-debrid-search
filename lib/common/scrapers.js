...
import * as mongoCache from './mongo-cache.js';
...
export async function searchKnaben(query, signal, logPrefix, config) {
    ...
    try {
        ...
        const base = ((config?.KNABEN_URL || ENV.KNABEN_URL) || 'https://api.knaben.org').replace(/\/$/, '');
        const timeout = config?.SCRAPER_TIMEOUT ?? ENV.SCRAPER_TIMEOUT;

        // Prepare the search parameters based on the AIOStreams implementation
        const requestBody = {
            query: query,
            size: limit,
            hideUnsafe: false,
        ...
        if (config?.MEDIA_TYPE === 'movie') {
            requestBody.categories = [3000000]; // Movies category
        } else if (config?.MEDIA_TYPE === 'series') {
            requestBody.categories = [2000000]; // TV category
        } else if (config?.IS_ANIME) {
            requestBody.categories = [6000000]; // Anime category
        }

        console.log(`[${logPrefix} SCRAPER] ${scraperName} searching with query: ${query}`);

        const response = await axiosWithProxy.post(`${base}/v1`, requestBody, {
            timeout,
            signal,
            headers: {
        ...
        // Cache results in scraper cache
        scraperCache.set(scraperName, query, config, processedResults);
        ...
        if (stremThru.isEnabled() && config?.DEBRID_TOKEN && config?.DEBRID_SERVICE) {
            ...
            if (allHashes.length > 0) {
                try {
                    // Get cached hashes from MongoDB first
                    const cachedHashesFromMongo = await mongoCache.getCachedHashes(config.DEBRID_SERVICE, allHashes);
                    ...
                    if (config.STREMTHRU_ENABLED) {
                        ...
                        // Combine MongoDB cached and StremThru cached hashes
                        const allCachedHashes = new Set([...cachedHashesFromMongo, ...stremthruCachedHashes]);
                        ...
                        if (immediateResults.length > 0) {
                            console.log(`[${logPrefix} SCRAPER] ${scraperName} returning ${immediateResults.length} cached results immediately via StremThru/MongoDB.`);
                        }
                        ...
                        // Only MongoDB cache available
                        const immediateResults = processedResults.filter(r => cachedHashesFromMongo.has(r.InfoHash.toLowerCase()));
                        
                        if (immediateResults.length > 0) {
                            console.log(`[${logPrefix} SCRAPER] ${scraperName} returning ${immediateResults.length} cached results from MongoDB immediately.`);
                        }
                        
                        // Perform background cache checking for remaining results
                        const nonCachedResults = processedResults.filter(r => !cachedHashesFromMongo.has(r.InfoHash.toLowerCase()));
                        if (nonCachedResults.length > 0) {
                            performBackgroundCacheCheck(nonCachedResults, config, logPrefix);
                        }
...
async function performBackgroundCacheCheck(results, config, logPrefix) {
    try {
        ...
        // to check the availability of these hashes and cache the results in MongoDB
