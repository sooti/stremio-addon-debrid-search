...
function generateScraperCacheKey(scraperName, query, config) {
...
export async function searchTorrentio(mediaType, mediaId, signal, logPrefix, config) {
    ...
    try {
        ...
        // Store results in SQLite cache
        await SqliteCache.upsertCachedMagnet({
            service: 'scraper',
            hash: cacheKey,
            size: processedResults.length,
            data: { data: processedResults, resultCount: processedResults.length }
        });
