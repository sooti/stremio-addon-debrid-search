    ...
    try {
        ...
        for (let batchStart = 0; batchStart < pageUrls.length; batchStart += batchSize) {
            ...
            const batchPromises = batchUrls.map(async (url, batchIndex) => {
                ...
                // Each request in batch waits index * 300ms (0ms, 300ms, 600ms, 900ms, 1200ms)
                if (batchIndex > 0) {
                ...
                const curlCmd = index === 0
                    ...
                    : `curl -s -L -b "${cookieFile}" -c "${cookieFile}" -H "User-Agent: ${userAgent}" -H "Accept: text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8" -H "Accept-Language: en-US,en;q=0.5" -H "Accept-Encoding: gzip, deflate, br, zstd" -H "DNT: 1" -H "Connection: keep-alive" -H "Upgrade-Insecure-Requests: 1" -H "Sec-Fetch-Dest: document" -H "Sec-Fetch-Mode: navigate" -H "Sec-Fetch-Site: same-origin" -H "Sec-Fetch-User: ?1" -H "Referer: ${prevPageReferer}" -H "Priority: u=0, i" -H "TE: trailers" --compressed "${url}"`;
                ...
                return execPromise(curlCmd, execOptions)
                    ...
                    .catch(error => {
                        ...
                        console.log(`[${logPrefix} SCRAPER] ${scraperName} page ${index + 1} failed (exit code: ${exitCode}): ${stderr || error.message}`);
