...
export async function searchTorrentGalaxy(searchKey, signal, logPrefix) {
    ...
    try {
        ...
        const base = (config.TORRENTGALAXY_URL || 'https://torrentgalaxy.to').replace(/\/$/, '');
...
export async function searchSnowfl(query, signal, logPrefix) {
    ...
    console.time(`[${logPrefix} TIMER] ${scraperName}`);
    try {
        const base = config.SNOWFL_URL || 'https://snowfl.com';

        // --- MODIFIED LOGIC AS REQUESTED ---
        // Tokens are now hardcoded from the provided URL.
        const token = 'qHCQznJRpRsjkgFjDVfUGsGNfiphTFBuAmJYqz';
        const sortToken = 'x93wmebv';

        // The URL is constructed with the hardcoded tokens and the raw query.
        // Note: Axios will automatically handle basic encoding like spaces (%20).
        const url = `${base}/${token}/${query}/${sortToken}/0/NONE/NONE/1`;
        // --- END OF MODIFICATION ---

        const response = await axios.get(url, {
            params: { '_': Date.now() }, // Cache buster timestamp
        ...
        const results = resultsArray.map(r => {
            ...
            const infoHash = getHashFromMagnet(r.magnet);
            if (!infoHash) return null;

            return {
                Title: r.name,
                InfoHash: infoHash,
                Size: sizeToBytes(r.size || '0 MB'),
                Seeders: parseInt(r.seeder) || 0,
                Tracker: `${scraperName} | ${r.site}`,
            };
        }).filter(Boolean);
