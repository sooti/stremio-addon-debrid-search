...
export async function initMongo() {
  ...
  initPromise = (async () => {
    try {
      ...
      await col.createIndex({ service: 1, releaseKey: 1 }, { sparse: true });
      await col.createIndex({ service: 1, releaseKey: 1, category: 1, resolution: 1 }, { sparse: true });

      // Add compound indexes for faster search cache lookups
      await col.createIndex({ service: 1, releaseKey: 1, expiresAt: 1 }, { sparse: true }); // For time-based queries
