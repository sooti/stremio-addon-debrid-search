...
// --- Porn detection helper ---
// Heuristic detection of adult content by common markers and site names.
// Intentionally conservative to avoid false positives, but includes explicit "xxx" token.
export function isPornTitle(title) {
    if (!title || typeof title !== 'string') return false;
    const s = title.toLowerCase();

    // Common site/brand markers
    const brands = [
        'brazzers','pornhub','xvideos','xhamster','realitykings','teamskeet','bangbros','mofos','vixen','blacked',
        'tushy','naughtyamerica','evilangel','wicked','joymii','babes','adulttime','digitalplayground','girlsway',
        'mylf','pervcity','nubiles','sexart','metart','x-art','porntrex','faphouse','onlyfans','fansly','jav','hentai'
    ];
    if (brands.some(b => s.includes(b))) return true;

    // Explicit XXX token (lowercase) surrounded by non-alphanumerics or boundaries
    // Note: This may filter the film "xXx" in some cases; acceptable trade-off per request.
    if (/(^|[^a-z0-9])xxx([^a-z0-9]|$)/.test(s)) return true;

    // Age/explicit indicators
    if (/(^|[^a-z0-9])(18\+|nsfw|porn|pr0n|amateur|camgirl|webcam)([^a-z0-9]|$)/.test(s)) return true;
