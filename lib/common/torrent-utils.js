...
export function isValidVideo(fileName, fileSize = 0, minSize = 50 * 1024 * 1024, logPrefix = 'UTIL') {
    ...
    // Try to decode URI, but fallback to original if it fails
    let decoded;
    try {
        decoded = decodeURIComponent(fileName).toLowerCase();
    } catch (e) {
        // If decodeURIComponent fails, use the filename as-is
...
export function isValidTorrentTitle(title, logPrefix = 'UTIL') {
    ...
    // Check if video indicator filter is disabled via config
    if (config.DISABLE_VIDEO_INDICATOR_FILTER) {
        console.log(`[${logPrefix}] Video indicator filter disabled, allowing: ${title}`);
        return true;
    }
...
export function getHashFromMagnet(magnetLink) {
    ...
    try {
        // Match either 40-char hex hash or 32-char base32 hash
        const match = magnetLink.match(/btih:([a-zA-Z0-9]{32,40})(?:&|$)/i);
...
export const resolutionOrder = {
    '2160p': 4,
    '4k': 4,      // 4K is same as 2160p
    '1440p': 3.5, // Between 4K and 1080p
    '1080p': 3,
    '720p': 2,
    '480p': 1,
    'auto': 0.5,  // Auto resolution, ranked below 480p
    'other': 0
};
...
/**
 * Obfuscates sensitive data in strings (API keys, tokens, passwords)
 * @param {string} str - String that may contain sensitive data
 * @param {string|Array<string>} sensitiveValues - API key(s) or other sensitive values to obfuscate
 * @returns {string} - String with sensitive data replaced by '***'
 */
export function obfuscateSensitive(str, sensitiveValues) {
    if (!str || typeof str !== 'string') return str;
    if (!sensitiveValues) return str;
    ...
    for (const value of values) {
        if (value && typeof value === 'string' && value.length > 0) {
            // Replace the sensitive value with '***' but keep first and last 2 chars if long enough
            const replacement = value.length > 8 ? `${value.substring(0, 2)}***${value.substring(value.length - 2)}` : '***';
