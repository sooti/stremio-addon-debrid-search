...
async function searchRealDebridTorrents(apiKey, type, id, userConfig = {}) {
  ...
  try {
    ...
    if (personalHighResSatisfied && !skipPersonalEarlyExit) {
      ...
      // Opportunistically record personal files to SQLite for future runs
      try {
    ...
    const rdHandler = {
        ...
        checkCachedHashes: async (hashes) => {
            ...
            // Prefer SQLite cache when enabled
            try {
              if (sqliteCache?.isEnabled()) {
                return await sqliteCache.getCachedHashes('realdebrid', hashes);
              }
            } catch {}
    ...
    // Create empty satisfiedQuotas for external torrents since personal files don't count toward external quotas
    const externalOnlyQuotas = { byCategory: {}, byCategoryResolution: {} };

    // Record personal files to SQLite (release-aware)
    try {
      if (sqliteCache?.isEnabled()) {
        const upserts = [];
        for (const file of enrichedPersonalFiles) {
    ...
    if (shouldProcessExternalTorrents) {
        ...
        // Process external torrents with quotas respected (do not bypass quotas)
        const processExternalTorrents = async () => {
            try {
                // Process external torrents respecting quotas instead of bypassing them
                const backgroundCachedResults = await processAndFilterTorrents(externalTorrents, rdHandler, episodeInfo, externalOnlyQuotas, false);
                ...
                console.log(`[${LOG_PREFIX}] Background cache check completed for ${externalTorrents.length} external torrents with quotas respected`);
                return { backgroundCachedResults, backgroundNonCachedTorrents };
            } catch (error) {
                console.error(`[${LOG_PREFIX}] Background cache check failed:`, error.message);
                return { backgroundCachedResults: [], backgroundNonCachedTorrents: externalTorrents };
            } finally {
                try {
        ...
        // Await the processing of external torrents to get live cache results
        const { backgroundCachedResults, backgroundNonCachedTorrents } = await processExternalTorrents();
        cachedResults = backgroundCachedResults;
        nonCachedTorrents = backgroundNonCachedTorrents;
    } else {
    ...
    // For immediate response, we already returned cached results from SQLite + personal files
...
async function searchTorrents(apiKey, searchKey = null, threshold = 0.3) {
  ...
  let cachedResults = await processAndFilterTorrents(uniqueExternalTorrents, rdHandler, null, {}, false);
