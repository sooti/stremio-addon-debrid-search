...
function formatCachedResult(torrent, isCached) {
  ...
  // If there's an episode hint, it's from a pack, and its details are the source of truth.
  ...
  // Use searchableName for the original pack title if available, otherwise use the definitive title
...
async function searchRealDebridTorrents(apiKey, type, id) {
  ...
  try {
          ...
          try {
            ...
            for (const file of info.data.files) {
              ...
              if (fileSeason === season && fileEpisode === episode) {
                ...
                matchingFiles.push({ ...file, parsedInfo: parsed });
              }
            }
            ...
            if (matchingFiles.length > 0) {
              ...
              const episodeResult = {
                ...
                isFromPack: true,
                packHash: hash,
                searchableName: info.data.filename
              };
              
              successfulPackResults.set(hash.toLowerCase(), episodeResult);
              packResults.set(hash, [episodeResult]); // Signal success to processor
              inspectedCount++;
    ...
    // After processing, substitute any malformed pack results with our perfect, saved results.
    if (successfulPackResults.size > 0) {
        cachedResults = cachedResults.map(torrent => {
            const hash = (torrent.InfoHash || '').toLowerCase();
            if (torrent.episodeFileHint && successfulPackResults.has(hash)) {
                return successfulPackResults.get(hash); // Perform the substitution
            }
            return torrent;
        });
    }
    
    // Remove any packs that failed inspection entirely.
    if (failedPackHashes.size > 0) {
        const beforeCount = externalTorrents.length;
        externalTorrents = externalTorrents.filter(t => !failedPackHashes.has((t.InfoHash || '').toLowerCase()));
        console.log(`[${LOG_PREFIX}] Discarded ${beforeCount - externalTorrents.length} packs that failed inspection.`);
    }

    // Always include some non-cached magnets too (top by seeders)
    if (externalTorrents.length > 0) {
      let nonCachedTorrents = externalTorrents.filter(t => !t.isCached);

      if (episodeInfo) {
        nonCachedTorrents = nonCachedTorrents.filter(t => {
          try {
            const parsed = PTT.parse(t.Title || t.name || '');
            const isPack = parsed.season !== undefined && (parsed.episode === undefined || Array.isArray(parsed.episode));
            return !isPack;
          } catch (e) {
            return true;
          }
      ...
      for (const file of videoFiles) {
        ...
        if (fileReference && fileReference !== 'undefined') {
          allVideoFiles.push({
            ...
            source: 'realdebrid',
            isPersonal: true,
            isCached: true, // Personal files are always considered cached
            tracker: 'Personal',
...
function formatDownloadFile(download) {
  return {
    ...
    source: 'realdebrid',
    isPersonal: true,
    isCached: true, // Personal downloads are always considered cached
    tracker: 'Personal'
  };
}
