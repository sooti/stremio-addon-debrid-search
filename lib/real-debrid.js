...
async function validatePersonalStreams(apiKey, streams) {
    ...
    const personalStreams = streams.filter(s => s.isPersonal || (s.tracker === 'Personal'));
    if (personalStreams.length === 0) {
        return streams;
    }

    console.log(`[RD VALIDATE] Validating ${personalStreams.length} personal streams...`);

    try {
        const RD = new RealDebridClient(apiKey);
        const [userTorrents, userDownloads] = await Promise.all([
            getAllTorrents(RD).catch(() => []),
            getAllDownloads(RD).catch(() => [])
        ]);

        const userHashes = new Set(userTorrents.map(t => t.hash.toLowerCase()));
        const userDownloadIds = new Set(userDownloads.map(d => d.id));

        let validatedCount = 0;
        const updatedStreams = streams.map(stream => {
            if (stream.isPersonal || (stream.tracker === 'Personal')) {
                const streamHash = stream.hash;
                if (streamHash) { // It's from a torrent
                    if (!userHashes.has(streamHash)) {
                        validatedCount++;
                        return { ...stream, isPersonal: false, tracker: 'Cached' };
                    }
                } else { // It might be from a download
                    if (stream.id && !String(stream.id).includes(':')) {
                         if (!userDownloadIds.has(stream.id)) {
                             validatedCount++;
                             return { ...stream, isPersonal: false, tracker: 'Cached' };
                         }
                    }
