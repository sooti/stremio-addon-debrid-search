...
async function checkAndProcessCache(apiKey, externalTorrents, searchType, searchId) {
    ...
    const requiredKeywords = [
        ...
        'dvdrip', 'dvdr', 'hdtv', 'hdrip', 'bluray rip'
    ];
    ...
    const maxPerQuality = parseInt(process.env.MAX_RESULTS_PER_QUALITY, 10) || 3;
    ...
    try {
        for (let pass = 0; pass < MAX_PASSES; pass++) {
            ...
            for (let i = 0; i < torrentsToCheck.length; i++) {
                ...
                if (currentCount >= maxPerQuality) {
                    console.log(`[RD CACHE] 🚫 SKIPPING [${category} ${resolution}] - category/resolution limit (${maxPerQuality}) reached.`);
                    continue;
                }
                ...
                // This is the new rule for skipping Audio-Focused torrents
                if (category === 'Audio-Focused') {
                    const hasBluRay = categoryResolutionTracker['BluRay'] && Object.keys(categoryResolutionTracker['BluRay']).length > 0;
                    const hasWebDL = categoryResolutionTracker['WEB/WEB-DL'] && Object.keys(categoryResolutionTracker['WEB/WEB-DL']).length > 0;
                    if (hasBluRay || hasWebDL) {
                        console.log(`[RD CACHE] 🚫 SKIPPING [Audio-Focused] because a superior BluRay or WEB-DL source was found.`);
                        continue;
                    }
                }
                ...
                const handleCachedResult = (torrent, from) => {
                    ...
                    const newCount = categoryResolutionTracker[category][resolution];

                    foundResolutions.add(resolution);
                    console.log(`[RD CACHE] ✅ ADDED (${from}) [${category} ${resolution}] - Total: ${newCount}/${maxPerQuality}`);
                };
