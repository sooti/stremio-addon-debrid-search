...
const ZILEAN_LIMIT = parseInt(process.env.ZILEAN_LIMIT) || 75; // Renamed this line
...
async function searchTorrents(apiKey, searchKey = null, threshold = 0.3) {
    ...
    try {
        ...
        if (BITMAGNET_ENABLED) scraperPromises.push(searchBitmagnet(searchKey, signal));
	...
	if (ZILEAN_ENABLED) scraperPromises.push(searchZilean(searchKey, null, null, signal));
        ...
        if (BT4G_ENABLED) scraperPromises.push(searchBt4g(searchKey, signal));
...
async function searchRealDebridTorrents(apiKey, type, id) {
    ...
    if (ZILEAN_ENABLED) scraperPromises.push(searchZilean(searchKey, season, episode, signal));
    ...
    if (BT4G_ENABLED) scraperPromises.push(searchBt4g(specificSearchKey, signal));
    ...
    try {
        ...
        // Sort by resolution (desc) and then by size (desc)
        allResults.sort((a, b) => {
            ...
            if (rankA !== rankB) {
                return rankB - rankA; // Sort by resolution descending
            }

            const sizeA = a.size || a.Size || 0;
            const sizeB = b.size || b.Size || 0;
            return sizeB - sizeA; // Then by size descending
        });

        console.log(`[RD] Comprehensive total: ${allResults.length} streams (sorted by resolution then size)`);
        return allResults;
...
async function searchBitmagnet(query, signal) {
    console.time('[RD TIMER] Bitmagnet');
    try {
        ...
        console.timeEnd('[RD TIMER] Bitmagnet');
        ...
        console.timeEnd('[RD TIMER] Bitmagnet');
        if (!axios.isCancel(error)) console.error(`[RD SCRAPER] Bitmagnet search failed: ${error.message}`);
        return [];
    }
}
    ...
    try {
        ...
        if (episode) {
            ...
            results = results.filter(result => {
                // A season pack is a valid result for any episode within it.
                // We identify a season pack if the episodes array is empty or 'complete' is true.
                const isSeasonPack = result.episodes.length === 0 || result.complete;
                if (isSeasonPack) {
                    return true;
                }

                // Otherwise, it must be a result that explicitly contains the target episode.
                return result.episodes.includes(targetEpisode);
            });
        }
        // --- End of New Filtering Logic ---

        return results.slice(0, ZILEAN_LIMIT).map(result => ({
            Title: result.raw_title,
            InfoHash: result.info_hash,
            Size: parseInt(result.size),
            Seeders: null, // Zilean API doesn't provide seeder counts
            Tracker: 'Zilean | DMM'
        }));
    } catch (error) {
        console.timeEnd('[RD] Zilean');
...
async function searchBt4g(query, signal) {
    ...
    try {
        ...
        console.timeEnd('[RD TIMER] BT4G');
        return results;

    } catch (error) {
        console.timeEnd('[RD TIMER] BT4G');
        if (!axios.isCancel(error)) console.error(`[RD SCRAPER] BT4G search failed: ${error.message}`);
        return [];
    }
}
