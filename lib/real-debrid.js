...
async function searchRealDebridTorrents(apiKey, type, id, userConfig = {}) {
  ...
  try {
    ...
    // Always run scrapers even if personal files satisfy quota (for cache warming)
    // But mark that personal files already satisfied quota for optimization
    const skipPersonalEarlyExit = true; // Force scraper execution for cache warming

    if (personalHighResSatisfied && !skipPersonalEarlyExit) {
    ...
    // Log if we're running scrapers despite personal files satisfying quota
    if (personalHighResSatisfied && skipPersonalEarlyExit) {
      console.log(`[${LOG_PREFIX}] Personal quotas already satisfied, but running scrapers for cache warming.`);
    }
    ...
    // For external torrent processing: DON'T count personal files toward quotas
    // This ensures external torrents are always cached for sharing across users
    // Personal files are user-specific and shouldn't prevent external caching
    ...
    if (shouldProcessExternalTorrents) {
        ...
        // Run processAndFilterTorrents in the background using setImmediate to not block the response
        // but still process the external torrents for future caching
        const processExternalTorrents = async () => {
            try {
                const backgroundCachedResults = await processAndFilterTorrents(externalTorrents, rdHandler, episodeInfo, externalOnlyQuotas);
                ...
                const hasEnoughHQResults = () => {
                  ...
                  return HQ_CATEGORIES.some(category => {
                    ...
                    const results1080p = [...enrichedPersonalFiles, ...backgroundCachedResults].filter(r =>
                      r.category === category && r.resolution === '1080p'
                    ).length;
                    const results2160p = [...enrichedPersonalFiles, ...backgroundCachedResults].filter(r =>
                ...
                if (backgroundNonCachedTorrents.length > 0 && !hasEnoughHQResults()) {
                  const topNonCached = backgroundNonCachedTorrents
                  ...
                  console.log(`[RD] ✅ Already have sufficient HQ results - skipping non-cached torrent inspection (background)`);
                }
        ...
        setImmediate(processExternalTorrents);
        
        // Meanwhile, check the MongoDB cache synchronously for the most recent cached results
        // This allows us to return cached results immediately while the live cache checking runs in background
        try {
            const cachedHashes = new Set();
            if (mongoCache?.isEnabled()) {
                const externalHashes = externalTorrents.map(t => (t.InfoHash || t.hash || '').toLowerCase()).filter(Boolean);
                if (externalHashes.length > 0) {
                    const results = await mongoCache.getCachedHashes('realdebrid', externalHashes);
            ...
            // Return only the already cached external results (from MongoDB) along with personal files
            // This ensures we return immediate results while the live checking happens in background
            cachedResults = externalTorrents.filter(t => cachedHashes.has((t.InfoHash || t.hash || '').toLowerCase()));
    ...
    // For immediate response, we already returned cached results from MongoDB + personal files
    // The non-cached verification happens in background, so we skip it here

    if (cachedResults.length > 0) {
      // These are already cached, so they should be persisted to ensure they're counted for future requests
      try {
        if (mongoCache?.isEnabled()) {
          const upserts = [];
    ...
    console.log(`[${LOG_PREFIX}] Comprehensive total: ${allResults.length} streams (sorted) - external live cache check running in background`);
    return allResults;
    ...
    await saveHashCache();
    cleanupTemporaryTorrents(createRealDebridClient(apiKey), torrentIdsToDelete);
  }
}
...
async function searchTorrents(apiKey, searchKey = null, threshold = 0.3) {
  ...
  let cachedResults = await processAndFilterTorrents(uniqueExternalTorrents, rdHandler, null);
  if (uniqueExternalTorrents.length > 0) {
      const nonCached = uniqueExternalTorrents.filter(t => !cachedResults.some(c => c.InfoHash === t.InfoHash));
...
async function resolveMagnetUrl(apiKey, magnetUrl, clientIp) {
  ...
  try {
    ...
    if (filesWithLinks.length === 0) {
        console.log(`[${LOG_PREFIX}] Error: No files with valid links found for torrent ${torrentId}`);
        return null;
    }
    ...
    if (!selected) {
        console.log(`[${LOG_PREFIX}] Error: No valid video file found in torrent ${torrentId}`);
        return null;
    }
    ...
    console.error(`[${LOG_PREFIX}] resolveMagnetUrl error: ${error.message}`);
    return null;
  }
}
