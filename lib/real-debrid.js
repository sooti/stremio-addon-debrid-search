...
function calculateTorrentPriority(torrent) {
    ...
    if (config.PRIORITY_PENALTY_AAC_OPUS_ENABLED && (name.includes(' aac') || name.includes('.aac') || name.includes(' opus') || name.includes('.opus'))) {
        priorityScore -= 50;
    }
...
async function checkAndProcessCache(apiKey, externalTorrents, searchType, searchId) {
    ...
    const torrentsWithPriority = validTorrents.map(torrent => ({
        ...
        resolution: getResolutionFromName(torrent.Title || torrent.title),
    ...
    try {
        for (let i = 0; i < torrentsToCheck.length; i++) {
            ...
            if (config.PRIORITY_SKIP_WEBRIP_ENABLED) {
            ...
            if (config.PRIORITY_SKIP_AAC_OPUS_ENABLED) {
            ...
            if (config.DIVERSIFY_CODECS_ENABLED && codec !== 'unknown') {
                const otherCodec = codec === 'h265' ? 'h264' : 'h265';
                if (counts[codec] >= config.TARGET_CODEC_COUNT && counts[otherCodec] < config.TARGET_CODEC_COUNT) {
...
function formatExternalResult(result) {
    ...
    if (result.Size && result.Size < 200 * 1024 * 1024) {
        console.log(`[${LOG_PREFIX}] Filtering small torrent: ${result.Title} (${formatSize(result.Size)})`);
        return null;
    }
    return {
        name: result.Title,
...
async function searchRealDebridTorrents(apiKey, type, id) {
    ...
    try {
        ...
        console.log(`[${LOG_PREFIX}] Comprehensive total: ${allResults.length} streams (sorted by resolution then size)`);
        return allResults;
