              ...
              try {
                if (sqliteCache?.isEnabled()) {
                  ...
                  for (const t of backgroundCachedResults) {
                    ...
                    // Store full torrent data to preserve important fields like isFromPack, episodeFileHint
                    // This ensures that pack inspection results can be properly identified when loaded from cache
                    const fullData = {
                      ...t,
                      // Ensure these critical fields are preserved
                      source: t.isPersonal ? 'personal' : (t.isCached ? 'cached' : 'checked'),
                      isFromPack: t.isFromPack || false,
                      episodeFileHint: t.episodeFileHint || null,
