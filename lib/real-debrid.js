...
function createRealDebridClient(apiKey, options = {}) {
  ...
  // Add response size limits to prevent memory leaks from large responses
  // Default to 50MB max response size (can be overridden with env var)
  const maxResponseSize = parseInt(process.env.RD_MAX_RESPONSE_SIZE || '52428800', 10); // 50MB default
  clientOptions.maxContentLength = maxResponseSize;
  clientOptions.maxBodyLength = maxResponseSize;
...
async function searchRealDebridTorrents(apiKey, type, id, userConfig = {}, clientIp = null, isBackgroundRefresh = false) {
  ...
  try {
    ...
    let personalFiles = await searchPersonalFiles(apiKey, searchKey, 0.3, clientIp);
          ...
          liveCheckHash: async (hash) => {
              ...
              try {
                  ...
                  if (!['downloaded', 'finished'].includes(status)) {
                    ...
                    // Clean up response
                    if (torrentInfo) torrentInfo.data = null;
                    return false;
                  }
                  ...
                  // Clean up large response after processing
                  if (torrentInfo) torrentInfo.data = null;
                  return true;
              } catch (e) {
          ...
          batchCheckSeasonPacks: async (hashes, season, episode) => {
              ...
              for (const hash of hashes) {
                  ...
                  try {
                      ...
                      if (!info?.data?.files) {
                          // Clean up response before continuing
                      ...
                      if (matchingFiles.length > 0) {
                          ...
                          const filename = info.data.filename; // Extract before cleanup
                          const episodeResult = {
                              ...
                              isCached: true, isFromPack: true, packHash: hash, searchableName: filename
                          };
  ...
  const rdHandler = {
    ...
    liveCheckHash: async (hash) => {
      ...
      try {
        ...
        // Clean up large response after successful check
        if (torrentInfo) torrentInfo.data = null;
        return true;
      } catch (e) {
        const status = e?.response?.status || e?.status;
...
async function getAllTorrents(RD, apiKey) {
  ...
  try {
    for (let page = 1; page <= 2; page++) {
      ...
      // Extract only the data we need to reduce memory footprint
      allTorrents.push(...torrents);

      // Explicitly null out the response to help GC
      response.data = null;
  ...
  for (const torrent of torrents.slice(0, 3)) {
    try {
      ...
      if (!info?.data?.files || !info.data.links) {
        // Clean up response object before continuing
        if (info) {
          info.data = null;
        }
        continue;
