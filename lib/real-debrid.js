...
function combineAndMarkResults(apiKey, personalFiles, externalSources, specificSearchKey) {
    ...
    const fuse = new Fuse(newExternalTorrents, { keys: ['Title', 'name'], threshold: 0.3, distance: 200, ignoreLocation: true, minMatchCharLength: 2 });
...
async function inspectAndFilterNonCached(torrents, rdHandler) {
    ...
    for (const torrent of torrents) {
        ...
        if (isValid) {
            ...
            console.log(`[RD] -> REJECTED (junk files or inspection failed): ${torrent.Title}`);
        }
    }
...
async function searchRealDebridTorrents(apiKey, type, id) {
  ...
  try {
    ...
    const isLikelyEpisode = (t) => {
        const rawCandidates = [(t.Title || t.name || '').toString(), (t.path || '').toString()].filter(Boolean);
        ...
        for (const hash of hashes) {
          ...
          try {
            ...
            const matchingFiles = info.data.files.filter(file => {
              if (!isValidVideo(file.path, file.bytes, 50 * 1024 * 1024, LOG_PREFIX)) return false;
    ...
    let nonCachedTorrents = externalTorrents.filter(t => !cachedResults.some(c => c.InfoHash === t.InfoHash));
