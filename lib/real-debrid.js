...
const { isValidVideo, getResolutionFromName, resolutionOrder, delay, filterByYear, smarterSanityCheck, formatSize } = torrentUtils;
const LOG_PREFIX = 'RD';
...
function formatStreamObject(torrent) {
    const displayName = torrent.fileName || torrent.name;
    const displaySize = torrent.fileSize || torrent.size;
    
    const tracker = torrent.Tracker || (torrent.isPersonal ? 'Personal' : (torrent.from || 'Cached'));

    const titleParts = [
        displayName,
        `💾 ${formatSize(displaySize || 0)}`,
        `👤 ${torrent.Seeders || 0}`,
        `⚙️ ${tracker}`
    ];
    const title = titleParts.join(' | ');
    const hash = (torrent.InfoHash || torrent.hash || '').toLowerCase();

    const stream = {
        title: title,
        infoHash: hash,
        behaviorHints: {
            bingeGroup: `realdebrid|${hash}`
        }
    };

    if (torrent.fileIdx !== undefined) {
        stream.fileIdx = torrent.fileIdx;
    }

    if (torrent.isPersonal && torrent.url) {
        stream.url = torrent.url;
    }

    return stream;
}

function combineAndMarkResults(apiKey, personalFiles, externalSources, specificSearchKey, cinemetaDetails) {
    ...
    let saneResults = Array.from(uniqueExternalTorrents.values()).filter(t => t.InfoHash && !personalHashes.has(t.InfoHash.toLowerCase()));

    saneResults = smarterSanityCheck(saneResults, cinemetaDetails, LOG_PREFIX);
...
async function searchTorrents(apiKey, searchKey = null, threshold = 0.3) {
    ...
    try {
        ...
        const combinedResults = await combineAndMarkResults(apiKey, personalFiles, scraperResults, searchKey, null);
        if (combinedResults.length === 0) return personalFiles.map(formatStreamObject);

        const externalTorrents = combinedResults.filter(t => !t.isPersonal);
        if (externalTorrents.length === 0) return personalFiles.map(formatStreamObject);
        
        const rdHandler = {
            getIdentifier: () => LOG_PREFIX,
            checkCachedHashes: async (hashes) => new Set(hashes.filter(isHashInCache)),
        };

        const cachedResults = await processAndFilterTorrents(externalTorrents, rdHandler);
        
        const finalResults = [...personalFiles, ...cachedResults].map(formatStreamObject);
        console.log(`[${LOG_PREFIX}] Final: ${finalResults.length} streams`);
        return finalResults;
...
async function searchRealDebridTorrents(apiKey, type, id) {
    ...
    try {
        ...
        const combinedResults = await combineAndMarkResults(apiKey, personalFiles, scraperResults, specificSearchKey, cinemetaDetails);
        ...
        const rdHandler = {
            ...
            batchCheckSeasonPacks: async (hashes, season, episode) => {
                ...
                const inspectPack = async (hash) => {
                    let torrentId = null;
                    if (debugLogsEnabled) {
                        console.log(`[${LOG_PREFIX} DBG] Pack Inspect: Starting check for hash ${hash}`);
                    }
                    try {
                        const magnet = `magnet:?xt=urn:btih:${hash}`;
                        const addResponse = await RD.torrents.addMagnet(magnet).catch(() => null);
                        ...
                        if (files && files.length > 0) {
                            ...
                            if (debugLogsEnabled) {
                                console.log(`[${LOG_PREFIX} DBG] Pack Inspect: Found ${files.length} files in ${hash}. Checking for S${paddedSeason}E${paddedEpisode}...`);
                                files.slice(0, 5).forEach(f => console.log(`[${LOG_PREFIX} DBG] -> File: ${f.path}`));
                                if (files.length > 5) console.log(`[${LOG_PREFIX} DBG] -> ...and ${files.length - 5} more files.`);
                            }
                            ...
                            if (debugLogsEnabled) {
                                console.log(`[${LOG_PREFIX} DBG] Pack Inspect: Episode ${fileIndex !== -1 ? `found at index ${fileIndex}` : 'NOT found'} in ${hash}.`);
                            }
                            ...
                            console.log(`[${LOG_PREFIX} DBG] Pack Inspect: No files found inside torrent ${hash}.`);
                        }
                    } catch (error) {
                ...
                return new Map(results.filter(Boolean).map(r => [r.hash, { fileIdx: r.fileIdx, fileName: r.fileName, fileSize: r.fileSize }]));
            },
            cleanup: async () => {
                await saveHashCache();
        ...
        let allResults = combined.map(torrent => formatStreamObject(torrent));
