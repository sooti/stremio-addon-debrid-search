...
function formatCachedResult(torrent, isCached) {
    ...
    // Prefer the actual episode file size (when inspecting season packs) instead of the whole pack size
...
async function searchTorrents(apiKey, searchKey = null, threshold = 0.3) {
    ...
    try {
        ...
        if (config.TORRENT_GALAXY_ENABLED) scraperPromises.push(scrapers.searchTorrentGalaxyg(searchKey, signal, LOG_PREFIX));
  ...
  try {
    ...
    if (cinemetaDetails?.name) {
      ...
      externalTorrents = externalTorrents.filter(t => {
        // primary check (object-aware)
        if (matchesSeriesTitle(t, cinemetaDetails.name)) return true;

        // also try the common name/title fields prefixed with a leading slash
        ...
        // if there's a path field that starts with a slash, check that explicitly
        if (t.path && matchesSeriesTitle(t.path, cinemetaDetails.name)) return true;
        if (t.path && matchesSeriesTitle('/' + t.path, cinemetaDetails.name)) return true;
      ...
      if (type === 'series' && episodeInfo) {
        ...
        const isLikelyEpisode = (t) => {
          ...
          // also include slash-prefixed variants
          const variants = [];
          for (const r of rawCandidates) {
            variants.push(r);
            variants.push(r.replace(/^\/+/, '')); // strip leading slashes
            variants.push('/' + r.replace(/^\/+/, '')); // ensure single leading slash
          }
    ...
    const rdHandler = {
      ...
      checkCachedHashes: async (hashes) => {
        ...
        hashes.forEach(hash => {
          if (isHashInCache(hash)) cached.add(hash);
        });
        return cached;
      },
      ...
      batchCheckSeasonPacks: async (hashes, season, episode) => {
        ...
        const inspectPack = async (hash) => {
          ...
          try {
            ...
            torrentId = addResponse.data.id;
            try { await rdCall(() => RD.torrents.selectFiles(torrentId, 'all')); } catch (_) {}

            let torrentInfo = null;
            let files = null;
            const maxTries = 5;
            for (let attempt = 0; attempt < maxTries; attempt++) {
              torrentInfo = await rdCall(() => RD.torrents.info(torrentId).catch(() => null));
            ...
            if (files && files.length > 0) {
              ...
              const episodePatterns = [
                `[sS][\\W_]*${paddedSeason}[\\W_]*[eE][\\W_]*${paddedEpisode}`,
                `[sS][\\W_]*${season}[\\W_]*[eE][\\W_]*${episode}`,
                `\\b${season}[\\W_]*x[\\W_]*${paddedEpisode}\\b`,
                `\\b${paddedSeason}[\\W_]*x[\\W_]*${paddedEpisode}\\b`,
                `\\b[eE]p?\\.?\\s*${paddedEpisode}\\b`,
              ...
              for (let i = 0; i < files.length; i++) {
                ...
                if (combinedPattern.test(f.path) && isValidVideo(f.path, f.bytes, 50 * 1024 * 1024, LOG_PREFIX)) {
                  matchingEpisodeFiles.push({ id: f.id, index: i, path: f.path, bytes: f.bytes });
                }
              }

              if (matchingEpisodeFiles.length > 0) {
              ...
              if (seasonFiles.length > 0) {
                const episodeNumberPattern = new RegExp([`\\b${episode}\\b`, `\\b${paddedEpisode}\\b`, `\\b[eE]p?${episode}\\b`, `\\b[eE]p?${paddedEpisode}\\b`].join('|'), 'i');
                const matchingSeasonFiles = seasonFiles.filter(file => episodeNumberPattern.test(file.path));
                if (matchingSeasonFiles.length > 0) {
                  const validEpisodeInSeason = matchingSeasonFiles.some(file => isValidVideo(file.path, file.bytes, 50 * 1024 * 1024, LOG_PREFIX));
        ...
        if (isHashInCache(hash)) {
            ...
            try {
                ...
                if (matchingTorrent) {
                    ...
                    // Get torrent details
                    const torrentInfo = await rdCall(() => RD.torrents.info(matchingTorrent.id));
...
async function processTorrents(RD, torrents) {
    ...
    for (const torrent of torrents.slice(0, 3)) {
        try {
            const torrentDetails = await rdCall(() => RD.torrents.info(torrent.id));
