...
async function searchRealDebridTorrents(apiKey, type, id, userConfig = {}, clientIp = null) {
  ...
  try {
    ...
    if (stremThru.isEnabled() && apiKey) {
      try {
        ...
        stremthruCachedHashes = await stremThru.checkInstantAvailability(allExternalHashes, 'realdebrid', apiKey, clientIp);
    ...
    if (uncachedExternalTorrents.length > 0) {
              ...
              if (stremThru?.isEnabled() && apiKey && cached.size < hashes.length) {
                try {
                  ...
                  if (hashesToCheck.length > 0) {
                    const stremthruResults = await stremThru.checkInstantAvailability(hashesToCheck, 'realdebrid', apiKey, clientIp);
      ...
      setImmediate(async () => {
        try {
            ...
            const backgroundCachedResults = await processAndFilterTorrents(uncachedExternalTorrents, rdHandler, episodeInfo, { byCategory: {}, byCategoryResolution: {} }, false);
