...
function formatCachedResult(torrent, isCached) {
  ...
  // Preserve existing URL if provided (especially for personal RD files)
  let url = (typeof torrent.url === 'string' && torrent.url !== 'undefined') ? torrent.url : null;
  const episodeHint = torrent.episodeFileHint || null;

  if (!url) {
    if (torrent.isPersonal) {
      // Prefer RD file reference when we have IDs; fall back to magnet if only hash is known
...
async function searchRealDebridTorrents(apiKey, type, id) {
  ...
  try {
    ...
    if (type === 'movie') {
      // Drop TV-like patterns for movie searches
      const episodeLike = (name) => {
        if (!name) return false;
        const s = String(name).toLowerCase();
        return /s\d{1,2}\s*e\d{1,2}|\b\d{1,2}\s*x\s*\d{1,2}\b|\bseason\s*\d+\b|\bep(?:isode)?\s*\d+\b/.test(s);
      };
      const beforeType = externalTorrents.length;
      externalTorrents = externalTorrents.filter(t => {
        const n = (t?.name || t?.Title || t?.title || '').toString();
        try { const info = PTT.parse(n) || {}; if (info.season || info.episode) return false; } catch {}
        if (episodeLike(n)) return false;
        return true;
      });
      if (beforeType !== externalTorrents.length) {
        console.log(`[${LOG_PREFIX}] Movie-mode: removed ${beforeType - externalTorrents.length} TV-like results.`);
      }
    ...
    const rdHandler = {
      ...
      checkCachedHashes: async (hashes) => {
        ...
        lowered.forEach(hash => { if (isHashInCache(hash)) fromFile.add(hash); });
        const fromDb = await cacheDb.checkHashesCached('realdebrid', lowered);
        return new Set([...fromFile, ...fromDb]);
      },
...
async function checkAndProcessCache(apiKey, externalTorrents) {
  ...
  const rdHandler = {
    ...
    onHashChecked: (hash, isCached) => {
      try { mongoStatuses.push({ hash, cached: !!isCached }); } catch {}
    },
    liveCheckHash: async (hash) => {
      let torrentId;
      try {
    ...
    cleanup: async () => {
      await saveHashCache();
      if (torrentIdsToDelete.size > 0) cleanupTemporaryTorrents(RD, Array.from(torrentIdsToDelete));
      if (mongoStatuses.length > 0) {
        try { await cacheDb.upsertHashes('realdebrid', mongoStatuses); } catch {}
      }
    }
  };
