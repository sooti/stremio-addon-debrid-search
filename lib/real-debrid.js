  ...
  try {
    ...
    if (stremThru.isEnabled() && apiKey) {
      try {
        ...
        stremthruCachedHashes = await stremThru.checkInstantAvailability(allExternalHashes, 'realdebrid', apiKey);
              ...
              if (stremThru?.isEnabled() && apiKey && cached.size < hashes.length) {
                try {
                  ...
                  if (hashesToCheck.length > 0) {
                    const stremthruResults = await stremThru.checkInstantAvailability(hashesToCheck, 'realdebrid', apiKey);
