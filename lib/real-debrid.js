...
async function searchRealDebridTorrents(apiKey, type, id, userConfig = {}) {
  ...
  try {
    ...
    if (shouldProcessExternalTorrents) {
        ...
        // Use externalOnlyQuotas (empty) so personal files don't prevent external torrent caching
        cachedResults = await processAndFilterTorrents(externalTorrents, rdHandler, episodeInfo, externalOnlyQuotas);
