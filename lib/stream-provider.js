    ...
    try {
        if (provider === "realdebrid") {
            ...
            if (hostUrl.startsWith('magnet:') || hostUrl.includes('||HINT||')) {
                ...
                // Extract optional episode hint encoded after the magnet with a delimiter
                ...
                try {
                    ...
                    // If we have a hint, try to pick the hinted file; otherwise, pick the largest
                    let chosen = null;
                    if (episodeHint) {
                        // Prefer by fileId if available
                        if (episodeHint.fileId != null) {
                            chosen = videoFiles.find(f => f.id === episodeHint.fileId) || null;
                        }
                        // Fallback to path match
                        if (!chosen && episodeHint.filePath) {
                            chosen = videoFiles.find(f => f.path === episodeHint.filePath) || null;
                        }
                        // Fallback to pattern match if season/episode provided
                    ...
                    if (!chosen) {
                        chosen = videoFiles.reduce((prev, current) => (prev.bytes > current.bytes) ? prev : current);
                    }
...
function filterDownloadEpisode(download, season, episode) {
    ...
    const fileName = (download.name || download.title || "").toLowerCase();
    const patterns = [
        new RegExp("[sS][\\W_]*" + paddedSeason + "[\\W_]*[eE][\\W_]*" + paddedEpisode, "i"),
        new RegExp("[sS][\\W_]*" + season + "[\\W_]*[eE][\\W_]*" + episode, "i"),
        new RegExp("\\b" + Number(season) + "[\\W_]*x[\\W_]*" + paddedEpisode + "\\b", "i"),
        new RegExp("\\b" + paddedSeason + "[\\W_]*x[\\W_]*" + paddedEpisode + "\\b", "i"),
        new RegExp("\\b[eE]p?\\.?\\s*" + paddedEpisode + "\\b", "i")
    ];
