import { get4KHDHubStreams, getStreamSrcStreams, getVidLinkProStreams } from './http-streams.js';
...
async function getCachedTorrents(provider, type, id, config, searchFn) {
  ...
  if (cached || torzResults.length > 0) {
...
async function getMovieStreams(config, type, id) {
  ...
  if (config.DebridServices && Array.isArray(config.DebridServices) && config.DebridServices.length > 0) {
    config.DebridServices.forEach(service => {
      if (service.provider === 'Usenet') {
        ...
        const useVidLinkPro = service.httpVidLinkPro !== false;  // Default to true if not specified
        ...
        if (useVidLinkPro) {
          allStreamsPromises.push(
            withTimeout(
              getCachedTorrents('httpstreaming', type, `${id}-vidlinkpro`, config, () =>
                getVidLinkProStreams(id, type, null, null, config)
              ),
              HTTP_STREAMING_TIMEOUT_MS,
              'VidLinkPro'
            )
  ...
  // Sort streams by resolution (highest to lowest), then by size (largest to smallest) within each resolution
...
async function getSeriesStreams(config, type, id) {
  console.log(`[STREAM-PROVIDER] DEBUG getSeriesStreams - config.Scrapers:`, config.Scrapers);
  console.log(`[STREAM-PROVIDER] DEBUG getSeriesStreams - config.IndexerScrapers:`, config.IndexerScrapers);
  const [imdbId, season, episode] = id.split(":");
    ...
    config.DebridServices.forEach(service => {
      if (service.provider === 'Usenet') {
        ...
        if (useVidLinkPro) {
          allStreamsPromises.push(
            withTimeout(
              getCachedTorrents('httpstreaming', type, `${imdbId}-vidlinkpro-${season}:${episode}`, config, () =>
                getVidLinkProStreams(imdbId, type, season, episode, config)
              ),
              HTTP_STREAMING_TIMEOUT_MS,
              'VidLinkPro'
            )
