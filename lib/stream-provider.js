...
async function getCachedTorrents(provider, type, id, config, searchFn) {
  ...
  if (fastCached && Date.now() < fastCached.expires) {
    ...
    // For debrid services with personal files: ALWAYS fetch fresh to get personal files + merge
    const debridServices = ['RealDebrid', 'AllDebrid', 'TorBox', 'Premiumize', 'OffCloud', 'DebriderApp'];
    if (debridServices.includes(provider)) {
      console.log(`[CACHE] Returning cached results immediately, fetching fresh personal files in background`);

      // Return cached results immediately
      const immediateResults = [...fastCached.data];

      // Background: Fetch fresh (includes personal files + scrapers with their own cache) and update cache
      setImmediate(async () => {
        try {
          ...
          console.log(`[CACHE] Background refresh complete: ${freshPersonal.length} personal + ${freshNonPersonal.length} non-personal`);
  ...
  if (cached) {
    ...
    // For debrid services with personal files: return cached immediately, refresh in background
    const debridServices = ['RealDebrid', 'AllDebrid', 'TorBox', 'Premiumize', 'OffCloud', 'DebriderApp'];
    if (debridServices.includes(provider)) {
      console.log(`[CACHE] Returning cached results immediately, fetching fresh personal files in background`);

      // Return cached results immediately
      const immediateResults = [...cached.data];
      ...
      setImmediate(async () => {
        try {
          ...
          if (freshNonPersonal.length > 0) {
            console.log(`[CACHE] WARMING: ${cacheKey} (updating with ${freshNonPersonal.length} fresh non-personal results)`);
            await storeCacheResults(collection, cacheKey, freshNonPersonal, type, provider, false);
          }
        } catch (err) {
          console.error(`[CACHE] Background refresh FAILED: ${cacheKey}:`, err.message);
        }
