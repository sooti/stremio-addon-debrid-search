...
async function getMovieStreams(config, type, id) {
    ...
    if (config.DebridLinkApiKey || config.DebridProvider == "DebridLink") {
        ...
        const torrents = await OffCloud.searchOffcloudTorrents(apiKey, searchKey, 0.5)
        if (torrents && torrents.length) {
            const streams = await Promise.all(
                torrents
                    .filter(torrent => filterYear(torrent, cinemetaDetails))
...
async function getSeriesStreams(config, type, id) {
    ...
    if (config.DebridLinkApiKey || config.DebridProvider == "DebridLink") {
        ...
        const torrents = await OffCloud.searchOffcloudTorrents(apiKey, searchKey, 0.1)
        ...
        const realEpisodesFull = realEpisodes.filter(torrent => videoFileExtensions.test(torrent.name))
        console.log(realEpisodesFull)
        if (torrents && torrents.length) {
            const streams = await Promise.all(
                realEpisodesFull
...
function toStream(details, type) {
    ...
    return {
        name,
        title,
        url: encodeFileNameInUrl(validUrl), // Use the first valid URL found
        behaviorHints: {
