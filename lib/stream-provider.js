...
function toStream(details, type, config) {
    ...
    if (details.url && !details.url.startsWith('magnet:')) {
        // This is a direct debrid link (e.g., personal file)
        urlComponent = details.url;
    } else {
        // This is a magnet link. Embed fileIdx if it exists.
    ...
    // This URL structure should match your server-side router, e.g., /resolve/:provider/:apiKey/:url
    const streamUrl = `${ADDON_HOST}/resolve/${debridProvider}/${encodedApiKey}/${encodedUrl}`;
...
async function resolveUrl(debridProvider, apiKey, encodedUrl, clientIp) {
    ...
    if (provider === "realdebrid") {
        ...
        let magnet = url;
        let fileIdx = 'best'; // Default to largest file
        ...
        console.log(`[RESOLVER] RD: Parsed infoHash | File index: ${fileIdx}`);
        ...
        try {
            ...
            console.log(`[RESOLVER] RD: Added magnet, received ID: ${torrentId}`);
