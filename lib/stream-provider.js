  ...
  if (config.DebridServices && Array.isArray(config.DebridServices) && config.DebridServices.length > 0) {
    config.DebridServices.forEach(service => {
      if (service.provider === 'Usenet') {
        ...
        if (use4KHDHub) {
          allStreamsPromises.push(
            getCachedTorrents('httpstreaming', type, `${id}-4khdhub:${(config.Languages || []).join(',')}`, config, () => 
        ...
        if (useUHDMovies) {
          allStreamsPromises.push(
            getCachedTorrents('httpstreaming', type, `${id}-uhdmovies:${(config.Languages || []).join(',')}`, config, () => 
        ...
        if (useStremsrc) {
          allStreamsPromises.push(
            getCachedTorrents('httpstreaming', type, `${id}-stremsrc:${(config.Languages || []).join(',')}`, config, () => 
  ...
  if (config.DebridServices && Array.isArray(config.DebridServices) && config.DebridServices.length > 0) {
    config.DebridServices.forEach(service => {
      if (service.provider === 'Usenet') {
        ...
        if (use4KHDHub) {
          allStreamsPromises.push(
            getCachedTorrents('httpstreaming', type, `${imdbId}-4khdhub-${season}:${episode}:${(config.Languages || []).join(',')}`, config, () => 
        ...
        if (useUHDMovies) {
          allStreamsPromises.push(
            getCachedTorrents('httpstreaming', type, `${imdbId}-uhdmovies-${season}:${episode}:${(config.Languages || []).join(',')}`, config, () => 
        ...
        if (useStremsrc) {
          allStreamsPromises.push(
            getCachedTorrents('httpstreaming', type, `${imdbId}-stremsrc-${season}:${episode}:${(config.Languages || []).join(',')}`, config, () => 
