...
const SCRAPER_CACHE_TTL_SERIES_MIN = 60; // 1 hour
const SCRAPER_CACHE_TTL_MOVIE_MIN = 360; // 6 hours

// Caches search results from DebriderApp to speed up subsequent requests.
...
// avoiding the need to re-run the scraper search.
// The cache key is based on the media type, ID, and selected languages.
// Movies are cached for 6 hours, and series for 1 hour.
async function getDebriderAppTorrents(apiKey, type, id, config) {
  if (!MongoCache.isEnabled()) {
    return DebriderApp.search(apiKey, type, id, config);
  }
  ...
  const cacheKey = `debrider-search:${type}:${id}:${langKey}`;
  ...
  const results = await DebriderApp.search(apiKey, type, id, config);
...
async function getMovieStreams(config, type, id) {
  ...
  if (debridProvider == "DebridLink") {
    ...
    const torrents = await getDebriderAppTorrents(apiKey, type, id, config);
    if (torrents && torrents.length) {
      return torrents.sort(sortTorrents)
