...
async function getCachedTorrents(provider, type, id, config, searchFn) {
  ...
  if (fastCached && Date.now() < fastCached.expires) {
          console.log(`[SEARCH FAST CACHE] HIT: ${cacheKey} (${fastCached.data?.length || 0} non-personal results)`);
    
          // For debrid services with personal files: Return cached non-personal results + personal files immediately, fetch fresh in background
          const debridServices = ['RealDebrid', 'AllDebrid', 'TorBox', 'Premiumize', 'OffCloud', 'DebriderApp'];
          if (debridServices.includes(provider)) {
            console.log(`[CACHE] Debrid service cache hit. Fetching personal files and returning results immediately, fetching fresh in background.`);
            
            // Start fetching fresh results in background
            setImmediate(async () => {
              try {
                const freshResults = await searchFn();
                // personal files are not cached, so we need to get them fresh.
                // non-personal files are cached, but may be updated, so we get them fresh too.
                
                // Separate personal from non-personal in fresh results
                const freshPersonal = freshResults.filter(r => r.isPersonal);
                const freshNonPersonal = freshResults.filter(r => !r.isPersonal);
    
                console.log(`[CACHE] Background fresh fetch complete: ${freshPersonal.length} personal + ${freshNonPersonal.length} non-personal`);
    
                // Update cache with fresh non-personal results
                if (freshNonPersonal.length > 0) {
                  console.log(`[CACHE] WARMING: ${cacheKey} (updating with ${freshNonPersonal.length} fresh non-personal results)`);
                  await storeCacheResults(collection, cacheKey, freshNonPersonal, type, provider, true);
                }
              } catch (err) {
                console.error(`[CACHE] Background fetch failed: ${cacheKey}:`, err.message);
              }
            });
    
            // For immediate response, get personal files separately (without waiting for the full search)
            // This requires making a direct call to get personal files
            try {
              // Extract API key from config to fetch personal files
              ...
              if (apiKey && provider === 'RealDebrid') {
                ...
                // Get search key from Cinemeta using the ID
                ...
                // Filter personal files for series episodes like in searchRealDebridTorrents
                ...
                // Format personal files to match the expected stream format
                const formattedPersonal = filteredPersonalFiles.map(file => ({
                  ...file,
                  source: 'realdebrid',
                  isPersonal: true,
                  isCached: true // Personal files are considered cached from the user's account
                }));          
          // Combine cached non-personal with fresh personal files
          const combinedResults = [...fastCached.data, ...formattedPersonal];
          console.log(`[CACHE] Returning immediate results: ${fastCached.data?.length || 0} cached + ${formattedPersonal.length} personal`);
          return combinedResults;
                  ...
                  // Filter personal files for series episodes like in searchTorboxTorrents
                  let filteredPersonalFiles = personalFiles;
                  if (type === 'series' && id.includes(':')) {
                    const [, season, episode] = id.split(':');
                    if (season && episode) {
        ...
        : `${searchKey} ${cinemetaDetails.year || ''}`.trim();
          ...
          // Filter personal files for series episodes like in searchAllDebridTorrents
          let filteredPersonalFiles = personalFiles;
          if (type === 'series' && id.includes(':')) {
            const [, season, episode] = id.split(':');
            if (season && episode) {
          ...
          // If we can't get personal files, just return cached results
          console.log(`[CACHE] Returning cached results only: ${fastCached.data?.length || 0} items`);
          return fastCached.data;
        }
      } catch (err) {
        console.error(`[CACHE] Error fetching personal files for immediate response: ${err.message}`);
        // Fallback to just cached results if personal files fetch fails
        return fastCached.data;
      }
    } else {
      // For other providers (HTTP streams, etc): return cached + background refresh
      setImmediate(async () => {
        try {
          const freshResults = await searchFn();
          if (freshResults && freshResults.length > 0) {
            console.log(`[CACHE] WARMED: ${cacheKey} (new: ${freshResults.length}, old: ${fastCached.data?.length || 0})`);
            await storeCacheResults(collection, cacheKey, freshResults, type, provider, true);
          }
        } catch (err) {
  ...
  console.log(`[CACHE] Fast cache miss, checking MongoDB...`);
    ...
    if (debridServices.includes(provider)) {
      ...
      try {
        ...
        if (apiKey && provider === 'RealDebrid') {
          ...
          // Combine cached non-personal with fresh personal files
          const combinedResults = [...cached.data, ...formattedPersonal];
          console.log(`[CACHE] Returning immediate results: ${cached.data?.length || 0} cached + ${formattedPersonal.length} personal`);
          return combinedResults;
        } else if (apiKey && provider === 'TorBox') {
          const TorBox = (await import('./torbox.js')).default;
          // Get search key from Cinemeta using the ID
        ...
        : `${searchKey} ${cinemetaDetails.year || ''}`.trim();
          ...
          // If we can't get personal files, just return cached results
          console.log(`[CACHE] Returning cached results only: ${cached.data?.length || 0} items`);
          return cached.data;
        }
      } catch (err) {
        console.error(`[CACHE] Error fetching personal files for immediate response: ${err.message}`);
      ...
      // For other providers: return cached + background refresh
      setImmediate(async () => {
        try {
          const freshResults = await searchFn();
          if (freshResults && freshResults.length > 0) {
  ...
  // Store results (will filter out personal files)
  await storeCacheResults(collection, cacheKey, results, type, provider, true);

  // For RealDebrid: If no non-personal results to cache, store empty marker so next request triggers background scraping
  if (provider === 'RealDebrid') {
    const hasNonPersonal = results.some(r => !r.isPersonal);
    if (!hasNonPersonal && results.length > 0) {
      console.log(`[CACHE] Storing empty marker for personal-only results (${results.length} personal files found)`);
      ...
      try {
        await collection.replaceOne({ _id: cacheKey }, emptyMarker, { upsert: true });

        // Update fast cache with empty marker
        SEARCH_FAST_CACHE.set(cacheKey, {
          data: [],
          expires: Date.now() + SEARCH_FAST_CACHE_TTL
        });
        ...
        console.error(`[CACHE] Failed to store empty marker: ${e.message}`);
      }
    }
  }
