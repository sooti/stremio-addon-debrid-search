...
async function getCachedTorrents(provider, type, id, config, searchFn) {
  ...
  if (results && results.length > 0) {
    // Filter out any items that are already fully-formed stream objects with resolver or direct URLs.
    // This prevents storing URLs in the shared cache. Magnet links are not filtered.
