...
async function getCachedTorrents(provider, type, id, config, searchFn) {
  ...
  if (results && results.length > 0) {
    // Filter out any items that are already fully-formed stream objects with resolver URLs.
    // This prevents storing URLs in the shared cache.
    const cacheableData = results.filter(item => !(item && typeof item.url === 'string' && item.url.includes('/resolve/')));
...
async function getMovieStreams(config, type, id) {
  ...
  if (debridProvider == "DebridLink") {
    ...
    const files = await getCachedTorrents(debridProvider, type, id, config, () => Premiumize.searchFiles(apiKey, searchKey, 0.1));
...
async function getSeriesStreams(config, type, id) {
  ...
  if (debridProvider == "DebridLink") {
    ...
    const torrents = await getCachedTorrents(debridProvider, type, id, config, () => Premiumize.searchFiles(apiKey, searchKey, 0.1));
    if (torrents && torrents.length) {
      const streams = await Promise.all(
