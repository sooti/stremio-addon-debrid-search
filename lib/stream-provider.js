...
const SCRAPER_CACHE_TTL_SERIES_MIN = process.env.SCRAPER_CACHE_TTL_SERIES_MIN || 15;
...
async function getCachedTorrents(provider, type, id, config, searchFn) {
  ...
  if (cached) {
    ...
    // ALWAYS refresh cache in background to keep results fresh
    // This ensures users get instant cached results, but cache is constantly updated with latest data
    ...
    setImmediate(async () => {
      try {
        ...
        // ALWAYS update cache with fresh results, regardless of count
        // Fresh data is always better than stale data
        if (freshResults && freshResults.length > 0) {
          ...
          console.log(`[CACHE] WARMING: ${cacheKey} got 0 results, keeping existing cache`);
        }
      } catch (err) {
