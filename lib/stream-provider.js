...
async function getMovieStreams(config, type, id) {
  ...
  if (config.DebridServices && Array.isArray(config.DebridServices) && config.DebridServices.length > 0) {
    const allStreams = await Promise.all(
      config.DebridServices.map(service =>
  ...
  return getMovieStreamsFromProvider(debridProvider, apiKey, type, id, config, cinemetaDetails, searchKey);
}
...
async function getSeriesStreams(config, type, id) {
  ...
  return getSeriesStreamsFromProvider(debridProvider, apiKey, type, id, config, cinemetaDetails, searchKey, season, episode);
}
