...
async function getCachedTorrents(provider, type, id, config, searchFn) {
  ...
  // SKIP personal files for cached responses to avoid blocking/hanging
  // Personal files will be included in background refresh
  if (cached) {
    // Skip personal files fetch for cache hits to ensure fast response
    console.log(`[CACHE] Skipping personal files for cache hit (will be included in background refresh)`);

    // ALWAYS check Torz API if StremThru is enabled (fast, ~500ms)
    // This ensures we get the latest confirmed cached results from Torz immediately
    if ((provider === 'RealDebrid' || provider === 'AllDebrid') && config.IndexerScrapers?.includes('stremthru')) {
      try {
        const stremThru = await import('./util/stremthru.js');
        const debridService = provider === 'RealDebrid' ? 'realdebrid' : 'alldebrid';
        const apiKey = config.DebridApiKey || config.DebridServices?.find(s => s.provider === provider)?.apiKey;

        if (apiKey && stremThru.isEnabled()) {
          console.log(`[CACHE] Checking Torz API for fresh confirmed cached results...`);
          ...
          if (stremId && mediaType) {
            const torzResults = await stremThru.getCombinedTorrents(
              mediaType,
              stremId,
              debridService,
              apiKey,
            ...
            if (torzResults && torzResults.length > 0) {
              console.log(`[CACHE] Torz API returned ${torzResults.length} confirmed cached results`);
              // Mark as confirmed cached and add to combined results
              const formattedTorz = torzResults.map(t => ({
                ...t,
              ...
              // Deduplicate by hash (prefer Torz over cache)
              const existingHashes = new Set(combinedResults.map(r => (r.hash || r.infoHash || '').toLowerCase()));
              const newTorzResults = formattedTorz.filter(t => {
                const hash = (t.InfoHash || t.hash || '').toLowerCase();
                return hash && !existingHashes.has(hash);
              });

              combinedResults = [...combinedResults, ...newTorzResults];
              console.log(`[CACHE] Added ${newTorzResults.length} new Torz results to cache response`);
            } else {
              console.log(`[CACHE] Torz API returned 0 results`);
            }
          }
        }
      } catch (torzError) {
        console.error(`[CACHE] Error checking Torz API: ${torzError.message}`);
        // Continue with cached results only
      }
    }

    console.log(`[CACHE] Returning immediate results: ${combinedResults.length} total (${searchResults.length} from cache)`);
    ...
    if (searchResults.length > 0 || combinedResults.length > 0) {
      ...
      setImmediate(async () => {
        try {
          ...
          // Get fresh results with the search function (pass true to indicate background refresh)
          const freshResults = await searchFn(true);

          // Always update cache timestamp even if 0 results to avoid repeated searches
          if (freshResults && freshResults.length > 0) {
            ...
            if (nonPersonalFresh.length > 0) {
              ...
              // ALWAYS store ALL fresh results (not just new ones) to replace the entire cache
              // This ensures Torz results and other fresh data fully update the cache
              console.log(`[CACHE] Updating cache with all ${nonPersonalFresh.length} fresh results for ${cacheKey}`);
              await storeCacheResults(null, cacheKey, nonPersonalFresh, type, provider);
            } else {
              console.log(`[CACHE] Background refresh: no non-personal results for ${cacheKey}`);
              // Update cache timestamp even with no results
              await storeCacheResults(null, cacheKey, [], type, provider);
            }
          } else {
            console.log(`[CACHE] Background refresh: 0 results found for ${cacheKey}`);
            // Update cache timestamp even with 0 results to prevent repeated searches
            await storeCacheResults(null, cacheKey, [], type, provider);
          }
...
async function getMovieStreams(config, type, id) {
  ...
  if (config.DebridServices && Array.isArray(config.DebridServices) && config.DebridServices.length > 0) {
    config.DebridServices.forEach(service => {
      if (service.provider === 'Usenet') {
        ...
        const useMoviesDrive = service.httpMoviesDrive !== false;  // Default to true if not specified
          ...
          allStreamsPromises.push(
            withTimeout(
              getCachedTorrents('httpstreaming', type, `${id}-moviesdrive`, config, () =>
                ...
                return wrapHttpStreamsWithResolver(streams);
              }),
              HTTP_STREAMING_TIMEOUT_MS,
              'MoviesDrive'
            )
  ...
  // Use allSettled to get partial results even if some providers fail
  ...
  // Sort streams to have personal cloud results at the top, then by resolution, then by size
  flatStreams.sort((a, b) => {
    ...
    // Sort by size within same resolution
