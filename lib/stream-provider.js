...
async function getCachedTorrents(provider, type, id, config, searchFn) {
  ...
  if (fastCached && Date.now() < fastCached.expires) {
    console.log(`[SEARCH FAST CACHE] HIT: ${cacheKey} (${fastCached.data?.length || 0} results) - returning immediately`);
  ...
  if (cached) {
    ...
    // ALWAYS return cached results immediately to user
    console.log(`[CACHE] HIT: ${cacheKey} (${resultCount} results, age: ${cacheAgeMinutes}m) - returning immediately`);

    // Smart cache warming: Refresh in background to keep cache fresh
    // More aggressive thresholds to ensure users get fresh results
    const isHttpStreaming = provider === 'httpstreaming';
    const shouldWarmCache = (
      isHttpStreaming || // ALWAYS refresh http-streams (URLs can expire)
      cacheAgeMinutes > 5 || // Cache older than 5 minutes (was 15 for movies)
      resultCount < 10 // Very few results (likely incomplete)
    );

    if (shouldWarmCache) {
      console.log(`[CACHE] WARMING: ${cacheKey} (age: ${cacheAgeMinutes}m, results: ${resultCount}, provider: ${provider})`);
      ...
      setImmediate(async () => {
        try {
          ...
          if (freshResults && freshResults.length >= resultCount) {
            ...
            // Store the improved results (or equal results with potentially updated URLs)
            await storeCacheResults(collection, cacheKey, freshResults, type, provider, false);
          } else if (freshResults && freshResults.length > 0) {
            console.log(`[CACHE] WARMING: ${cacheKey} got fewer results (${freshResults.length} vs ${resultCount}), keeping old cache`);
          }
        } catch (err) {
      ...
      console.log(`[CACHE] No warming needed: ${cacheKey} (fresh enough: age=${cacheAgeMinutes}m)`);
    }
