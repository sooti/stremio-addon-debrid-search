...
async function getCachedTorrents(provider, type, id, config, searchFn) {
  ...
  if (cached) {
    ...
    if (debridServices.includes(provider)) {
      // For debrid services, we need to fetch personal files separately.
      // We do this in the background and return the cached results immediately.
      setImmediate(async () => {
        try {
      ...
      // For other providers, just return cached data and refresh in background
      setImmediate(async () => {
        try {
          const freshResults = await searchFn();
