    ...
    if (debridServices.includes(provider)) {
      ...
      setImmediate(async () => {
        try {
          ...
          if (apiKey && cached.data && cached.data.length > 0) {
            await verifyCachedTorrents(apiKey, provider, cached.data);
          }
          ...
          if (freshNonPersonal.length > 0) {
            ...
            await storeCacheResults(collection, cacheKey, freshNonPersonal, type, provider);
          }
        } catch (err) {
      ...
      setImmediate(async () => {
        try {
          ...
          if (cached.data && cached.data.length > 0) {
            await refreshHttpStreamLinks(cached.data);
          }
          ...
          if (freshResults && freshResults.length > 0) {
            ...
            await storeCacheResults(collection, cacheKey, freshResults, type, provider);
          }
        } catch (err) {
    ...
    setImmediate(async () => {
      try {
        if (results && results.length > 0) {
          ...
          await storeCacheResults(collection, cacheKey, results, type, provider);
        }
      } catch (err) {
