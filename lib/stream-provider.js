...
async function getCachedTorrents(provider, type, id, config, searchFn) {
  ...
  // ALWAYS check Torz API FIRST if StremThru is enabled (fast, ~500ms)
  ...
  if ((provider === 'RealDebrid' || provider === 'AllDebrid') && config.IndexerScrapers?.includes('stremthru')) {
    try {
      ...
      if (apiKey && stremThru.isEnabled()) {
        console.log(`[TORZ] Checking Torz API for fresh confirmed cached results BEFORE SQLite cache...`);
        ...
        if (stremId && mediaType) {
          ...
          if (rawTorzResults && rawTorzResults.length > 0) {
            ...
            torzResults = rawTorzResults.map(t => {
