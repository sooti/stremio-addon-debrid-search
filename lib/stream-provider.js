...
async function getUsenetStreams(config, type, id) {
  try {
    ...
    if (config.FileServerUrl) {
      try {
        ...
        if (response.data?.files && Array.isArray(response.data.files)) {
          personalFiles.push(...response.data.files);
          response.data.files.forEach(file => {
            personalFileNames.add(file.name);
          });
          console.log(`[USENET] ✓ Found ${personalFiles.length} files on server`);
          console.log(`[USENET] Sample files:`, personalFiles.slice(0, 2).map(f => f.name).join(', '));
        } else {
    ...
    const matchedPersonalFiles = personalFiles.filter(file =>
      matchesSearch(file.name, type, id, metadata)
    );
    ...
    const newznabStreams = results.slice(0, 50).map(result => {
      const resolution = getResolutionFromName(result.title);
      const resolutionLabel = (resolution === '2160p') ? '4k' : resolution;
      const configParam = encodeURIComponent(JSON.stringify(configData));
      const streamUrl = `${base}/usenet/stream/${encodeURIComponent(result.nzbUrl)}/${encodeURIComponent(result.title)}/${encodeURIComponent(type)}/${encodeURIComponent(id)}?config=${configParam}`;
      ...
      if (isInCloud) {
        console.log(`[USENET] ✓ Newznab result "${result.title}" matches personal file "${matchingFile.name}"`);
      }
    ...
    const personalOnlyStreams = matchedPersonalFiles
      ...
      .map(file => {
        ...
        // Use the file name as the release name
        const releaseName = file.name.replace(/\.(mkv|mp4|avi|mov|wmv|flv|webm|m4v|mpg|mpeg)$/i, '');
    ...
    // Combine: personal-only files first, then Newznab results
