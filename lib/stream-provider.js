    ...
    if (provider === "realdebrid") {
      ...
      return hostUrl;
        ...
        if (hostUrl.startsWith('magnet:')) {
            ...
            const transferId = await Premiumize.addMagnet(debridApiKey, hostUrl);
            if (!transferId) {
                throw new Error("Failed to add magnet to Premiumize.");
            }
            ...
            for (let i = 0; i < maxRetries; i++) {
                const transfers = await Premiumize.listTransfers(debridApiKey);
                const transfer = transfers.find(t => t.id === transferId);
                
                if (!transfer) {
                    // If transfer disappears, it might be a quick failure.
                    throw new Error("Transfer not found or failed immediately.");
                }

                if (transfer.status === 'finished') {
                    const itemId = transfer.folder_id || transfer.file_id;
                    if (!itemId) {
                        throw new Error("Finished transfer has no file or folder ID.");
                    }

                    let details = null;
                    const detailRetries = 3;
                    const detailRetryInterval = 2000;

                    for (let j = 0; j < detailRetries; j++) {
                        details = await Premiumize.getTorrentDetails(debridApiKey, itemId);
                        if (details) break; // Success, exit loop
                        if (j < detailRetries - 1) {
                            console.log(`[RESOLVER] PM: Failed to get details for itemId ${itemId} (attempt ${j + 1}/${detailRetries}). Retrying in ${detailRetryInterval}ms...`);
                            await new Promise(r => setTimeout(r, detailRetryInterval));
                        }
                    }

                    if (details && details.videos && details.videos.length > 0) {
                        let chosenVideo = null;
                        if (episodeHint && episodeHint.season && episodeHint.episode) {
                          const s = Number(episodeHint.season);
                          const e = Number(episodeHint.episode);
                          
                          chosenVideo = details.videos.find(f => {
                              const pttInfo = PTT.parse(f.name);
                              return pttInfo.season === s && pttInfo.episode === e;
                          });
                        }
                        if (!chosenVideo) {
                          chosenVideo = details.videos.reduce((a, b) => (a.size > b.size ? a : b));
                        }
                        
                        // The URL in details.videos is a /resolve/ URL, we need the raw stream link from it.
                        const urlParts = chosenVideo.url.split('/');
                        const encodedStreamLink = urlParts[urlParts.length - 1];
                        return decodeURIComponent(encodedStreamLink);
                    } else {
                        throw new Error("No video files found in completed transfer.");
                    }
                }

                if (transfer.status === 'error') {
                    throw new Error(`Transfer failed with error: ${transfer.message}`);
                }
                
                await new Promise(r => setTimeout(r, retryInterval));
            }

            throw new Error("Torrent not ready after multiple retries.");
        }
