...
// When a search is performed for a movie or series, the results are stored in SQLite.
...
// - Returns SQLite cached results quickly (single query)
...
async function getCachedTorrents(provider, type, id, config, searchFn) {
  ...
  // Try to get cached record using hash as the cache key (mirroring MongoDB approach but using SQLite)
  const cached = await SqliteCache.getCachedRecord('search', cacheKey);
  if (cached) {
    // Handle data structure from SQLite cache
    // cached.data may be an array or an object with data inside
    if (cached.data && typeof cached.data === 'object' && !Array.isArray(cached.data) && cached.data.data) {
      // Structure: { data: [...], resultCount: n }
      cached.data = cached.data.data;
    }
  }

  if (cached) {
    // Extract search results from cached data (handle both possible structures)
    let searchResults = [];
    let resultCount = 0;
    ...
    if (Array.isArray(cached.data)) {
      ...
      // Object format with nested data array (the format we store)
      searchResults = cached.data.data;
      resultCount = cached.data.resultCount || cached.data.data.length;
    } else {
      // If none of the above, try direct access
      searchResults = cached.data || [];
    ...
    if (debridServices.includes(provider)) {
      // For debrid services, run scrapers in the background to update cache
      setImmediate(async () => {
        try {
          // Verify cached torrents are still available in debrid service
          ...
          if (apiKey && searchResults && searchResults.length > 0) {
            await verifyCachedTorrents(apiKey, provider, searchResults);
          }
          
          // Update cache with fresh non-personal results in background
          ...
          if (freshNonPersonal.length > 0) {
            ...
            await storeCacheResults(null, cacheKey, freshNonPersonal, type, provider);
          }
        } catch (err) {
          console.error(`[CACHE] Background verification/fetch failed: ${cacheKey}:`, err.message);
        }
      });

      // Fetch personal files and combine with cached results for immediate response
      try {
        let apiKey = config.DebridApiKey;
        if (Array.isArray(config.DebridServices)) {
          const service = config.DebridServices.find(s => s.provider === provider);
        ...
        if (apiKey && provider === 'RealDebrid') {
          ...
          const combinedResults = [...searchResults, ...formattedPersonal];
      ...
      // For HTTP streaming providers, run scrapers in the background and refresh links
      setImmediate(async () => {
        try {
          // Refresh HTTP streaming links in background
          if (searchResults && searchResults.length > 0) {
            await refreshHttpStreamLinks(searchResults);
          }
          
          // Update cache with fresh results in background
          const freshResults = await searchFn();
          ...
          console.error(`[CACHE] HTTP streaming background processing failed: ${cacheKey}:`, err.message);
        }
      });
      
      return searchResults;
    } else {
      // For other providers, run scrapers in the background
      setImmediate(async () => {
        try {
          // Update cache with fresh results in background
          const freshResults = await searchFn();
    ...
    // Cache miss - run scrapers and return results, then update cache in background
    console.log(`[CACHE] MISS: ${cacheKey} - running scrapers to get fresh results`);
    
    // Get fresh results (this will block the response until complete)
    const results = await searchFn();
    
    // Store results in cache for next time in background
    setImmediate(async () => {
      try {
        if (results && results.length > 0) {
          console.log(`[CACHE] STORING: ${cacheKey} (${results.length} results for next request)`);
          await storeCacheResults(null, cacheKey, results, type, provider);
        }
      } catch (err) {
        console.error(`[CACHE] Cache storage failed for ${cacheKey}:`, err.message);
      }
    });
    
    // Return the fresh results for this request
    return results;
  }
}
