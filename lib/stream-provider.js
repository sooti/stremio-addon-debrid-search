import { get4KHDHubStreams, getStreamSrcStreams, getDizipall24Streams } from './http-streams.js';
...
async function getMovieStreams(config, type, id) {
  ...
  if (config.DebridServices && Array.isArray(config.DebridServices) && config.DebridServices.length > 0) {
    config.DebridServices.forEach(service => {
      if (service.provider === 'Usenet') {
        ...
        const useDizipall24 = service.httpDizipall24 === true;  // Default to false (opt-in)
        ...
        // Note: Dizipall24 is only for series, so skip for movies
        // (no promise added for movies, handled gracefully below)
  ...
  if (allStreamsPromises.length === 0) {
    console.log('[STREAM-PROVIDER] No stream providers configured or enabled for this content type');
    return [];
  }
    ...
    config.DebridServices.forEach(service => {
      if (service.provider === 'Usenet') {
        ...
        if (useDizipall24) {
          allStreamsPromises.push(
            withTimeout(
              getCachedTorrents('httpstreaming', type, `${imdbId}-dizipall24-${season}:${episode}`, config, () =>
                getDizipall24Streams(imdbId, type, season, episode, config)
              ).then(streams => wrapHttpStreamsWithResolver(streams)),
              HTTP_STREAMING_TIMEOUT_MS,
              'Dizipall24'
            )
