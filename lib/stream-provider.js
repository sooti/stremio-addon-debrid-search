...
async function getCachedTorrents(provider, type, id, config, searchFn) {
  ...
  // Run Torz API and SQLite cache check in PARALLEL for maximum performance
  // This ensures we get the fastest response time across all services
  const torzPromise = (async () => {
    // ONLY check Torz API for RealDebrid
    ...
    try {
      ...
      console.log(`[TORZ] Checking Torz API for RealDebrid in parallel with SQLite cache...`);
      ...
      // Convert Torz results to raw torrent format
      ...
      console.log(`[TORZ] Converted ${torzResults.length} Torz results to raw torrent format`);
  ...
  // Wait for BOTH Torz and SQLite cache to complete in parallel
  const [torzResults, cacheData] = await Promise.all([torzPromise, cachePromise]);
  const { searchResults, resultCount } = cacheData;
  ...
  // We have cached results - return them immediately
  // Personal files are included in the regular search flow on cache miss/expiry
  console.log(`[CACHE] Returning ${combinedResults.length} cached results for ${cacheKey}`);
