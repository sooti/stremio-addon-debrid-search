import { get4KHDHubStreams, getHDHub4uStreams } from './http-streams.js';
...
const HTTP_STREAMING_TIMEOUT_MS = parseInt(process.env.HTTP_STREAMING_TIMEOUT_MS) || 45000; // 45 seconds default for HTTP streams
  ...
  if (config.DebridServices && Array.isArray(config.DebridServices) && config.DebridServices.length > 0) {
    config.DebridServices.forEach(service => {
      if (service.provider === 'Usenet') {
        ...
        const useHDHub4u = service.httpHDHub4u !== false;
          ...
          allStreamsPromises.push(
            withTimeout(
              getCachedTorrents('httpstreaming', type, `${id}-hdhub4u`, config, () =>
                getHDHub4uStreams(id, type, null, null)
              ).then(streams => wrapHttpStreamsWithResolver(streams, config.host)),
              HTTP_STREAMING_TIMEOUT_MS,
              'HDHub4u'
            )
          ...
          allStreamsPromises.push(
            withTimeout(
              getCachedTorrents('httpstreaming', type, `${imdbId}-hdhub4u-${season}:${episode}`, config, () =>
                getHDHub4uStreams(imdbId, type, season, episode)
              ).then(streams => wrapHttpStreamsWithResolver(streams, config.host)),
              HTTP_STREAMING_TIMEOUT_MS,
              'HDHub4u'
            )
