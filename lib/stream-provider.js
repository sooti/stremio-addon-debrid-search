import { get4KHDHubStreams, getStreamSrcStreams, getHydraflixStreams } from './http-streams.js';
  ...
  if (config.DebridServices && Array.isArray(config.DebridServices) && config.DebridServices.length > 0) {
    config.DebridServices.forEach(service => {
      if (service.provider === 'Usenet') {
        ...
        const useHydraflix = service.httpHydraflix !== false;  // Default to true if not specified
        ...
        if (useHydraflix) {
          allStreamsPromises.push(
            withTimeout(
              getCachedTorrents('httpstreaming', type, `${id}-hydraflix`, config, () =>
                getHydraflixStreams(id, type, null, null, config)
              ).then(streams => wrapHttpStreamsWithResolver(streams)),
              HTTP_STREAMING_TIMEOUT_MS,
              'Hydraflix'
            )
    ...
    config.DebridServices.forEach(service => {
      if (service.provider === 'Usenet') {
        ...
        if (useHydraflix) {
          allStreamsPromises.push(
            withTimeout(
              getCachedTorrents('httpstreaming', type, `${imdbId}-hydraflix-${season}:${episode}`, config, () =>
                getHydraflixStreams(imdbId, type, season, episode, config)
              ).then(streams => wrapHttpStreamsWithResolver(streams)),
              HTTP_STREAMING_TIMEOUT_MS,
              'Hydraflix'
            )
