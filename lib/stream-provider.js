...
async function getCachedTorrents(provider, type, id, config, searchFn) {
  ...
  if (fastCached && Date.now() < fastCached.expires) {
    ...
    if (debridServices.includes(provider)) {
      console.log(`[CACHE] Debrid service cache hit. Fetching fresh results to include personal files.`);
      ...
      console.log(`[CACHE] Fresh fetch complete: ${freshPersonal.length} personal + ${freshNonPersonal.length} non-personal`);
