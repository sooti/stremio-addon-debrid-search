...
async function resolveUrl(debridProvider, debridApiKey, itemId, hostUrl, clientIp) {
    ...
    if (provider !== "realdebrid") {
        if (provider == "debridlink" || provider == "premiumize") { return hostUrl; }
        if (provider == "alldebrid") { return AllDebrid.unrestrictUrl(debridApiKey, hostUrl); }
        return Promise.reject(BadRequestError);
    }
        ...
        try {
            ...
            for (let i = 0; i < maxRetries; i++) {
                ...
                if (status === 'downloaded') {
                    ...
                    console.log('[RESOLVER DEBUG] Final torrentInfo.data:', JSON.stringify(torrentInfo.data, null, 2));
                    break; 
                }
            ...
            // First, create a list of ONLY the files that were actually selected.
            const selectedFiles = torrentInfo.data.files.filter(f => f.selected === 1);
            // Now, find the index of our video file WITHIN THAT NEW LIST.
