...
export async function processExtractorLinkWithAwait(link, linkNumber) {
    ...
    if (linkLower.includes('hblinks.dad')) {
        ...
        try {
            ...
            console.log(`Found ${extractedLinks.length} links in HBLinks page, processing first one...`);

            // Process the first available link recursively
            const firstLink = extractedLinks[0];
            return await processExtractorLinkWithAwait(firstLink, linkNumber);
        } catch (err) {
