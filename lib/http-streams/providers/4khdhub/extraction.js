    ...
    return makeRequest(url, { parseHTML: true })
        ...
        .then(response => {
            ...
            const buttonPromises = downloadButtons.get().map(async (button, index) => {
                ...
                if (buttonId === 's3' || buttonStyle.includes('#2d50e2') || text.includes('FSLv2') || text.includes('FSL V2')) {
                    serverType = 'FSL V2';
                    priority = 100;
            ...
            return Promise.all(buttonPromises)
                .then(results => {
                    ...
                    validResults.sort((a, b) => (b.priority || 0) - (a.priority || 0));
                ...
                .then(async (results) => {
                    ...
                    // FILTER OUT googleusercontent.com URLs COMPLETELY - user requested to never return these
                    ...
                    console.log(`After filtering googleusercontent: ${filteredResults.length} links remaining (${results.length - filteredResults.length} filtered out)`);
                    ...
                    // Validate remaining URLs (in batches of 4 for performance)
                    const validatedUrls = [];
                    for (let i = 0; i < filteredResults.length; i += 4) {
                        const batch = filteredResults.slice(i, i + 4);
                        const validationResults = await Promise.all(batch.map(async (result) => {
                            try {
                                console.log(`Validating ${result.serverType || 'Unknown'} server: ${result.url.substring(0, 80)}...`);
                                const validation = await validateSeekableUrl(result.url, { requirePartialContent: true });
                                if (validation.isValid) {
                                    console.log(`✓ ${result.serverType || 'Unknown'} server supports 206 (status: ${validation.statusCode})`);
                                    return result;
                                } else {
                                    console.log(`✗ ${result.serverType || 'Unknown'} server does not support 206 (status: ${validation.statusCode || 'unknown'})`);
                                    return null;
                                }
                            } catch (err) {
                                console.log(`✗ ${result.serverType || 'Unknown'} server validation failed: ${err.message}`);
                                return null;
                            }
                        }));

                        validatedUrls.push(...validationResults.filter(r => r !== null));
                    }

                    console.log(`Validation complete: ${validatedUrls.length}/${filteredResults.length} URLs support 206`);
