...
export async function get4KHDHubStreams(tmdbId, type, season = null, episode = null, config) {
    try {
        ...
        if (isLazyLoadEnabled()) {
            console.log(`[4KHDHub] Lazy-load enabled: returning ${downloadLinks.length} preview streams without extraction/validation`);

            // Create preview streams from download links with metadata
            const previewStreams = downloadLinks.map(link => {
                // Handle both old format (string URLs) and new format (objects with metadata)
                const url = typeof link === 'string' ? link : link.url;
                const size = typeof link === 'object' ? link.size : null;
                const quality = typeof link === 'object' ? link.quality : null;
                const label = typeof link === 'object' ? link.label : 'Download';
            ...
            // Sort by URL to maintain consistent order
            previewStreams.sort((a, b) => {
                // Prioritize certain patterns
                const aPriority = a.url.toLowerCase().includes('hubcloud') || a.url.toLowerCase().includes('pixel') ? 0 :
                    a.url.toLowerCase().includes('hubdrive') ? 1 :
                        a.url.toLowerCase().includes('workers.dev') ? 2 : 3;
                const bPriority = b.url.toLowerCase().includes('hubcloud') || b.url.toLowerCase().includes('pixel') ? 0 :
                    b.url.toLowerCase().includes('hubdrive') ? 1 :
                        b.url.toLowerCase().includes('workers.dev') ? 2 : 3;
            ...
            // Limit number of preview streams
            const MAX_PREVIEW_STREAMS = parseInt(process.env.MAX_4KHDHUB_PREVIEW_LINKS) || 25;
            const limitedStreams = streams.slice(0, MAX_PREVIEW_STREAMS);

            console.log(`[4KHDHub] Returning ${limitedStreams.length} preview streams (lazy-load mode)`);
            return limitedStreams;
        }

        // LEGACY MODE: Full extraction and validation (slow but thorough)
        console.log(`[4KHDHub] Lazy-load disabled: extracting and validating all streams (legacy mode)`);
        ...
        const prioritizedLinks = [...downloadLinks].sort((a, b) => {
            ...
            const aPriority = aUrl.toLowerCase().includes('hubcloud') || aUrl.toLowerCase().includes('pixel') ? 0 :
                aUrl.toLowerCase().includes('hubdrive') ? 1 :
                    aUrl.toLowerCase().includes('workers.dev') ? 2 : 3;
            const bPriority = bUrl.toLowerCase().includes('hubcloud') || bUrl.toLowerCase().includes('pixel') ? 0 :
                bUrl.toLowerCase().includes('hubdrive') ? 1 :
                    bUrl.toLowerCase().includes('workers.dev') ? 2 : 3;
            return aPriority - bPriority;
        });

        console.log(`[4KHDHub] Link prioritization: HubDrive (only), as requested`);
        ...
        for (let i = 0; i < prioritizedLinks.length && streamingLinks.length < MAX_WORKING_LINKS; i += PARALLEL_BATCH_SIZE) {
            ...
            const batchPromises = batch.map(async (link) => {
                try {
                    ...
                    const extracted = await extractStreamingLinks([linkUrl]);
