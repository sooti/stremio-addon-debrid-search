...
function shouldIncludeLink(url) {
    ...
    // Check if URL contains any of our hosting patterns
    ...
    try {
        ...
        // Only accept if there's a meaningful path
        if (pathname === '/' || pathname === '') {
            // Exception: redirect services with query params are OK (gadgetsweb, hblinks)
            if (urlObj.search && (hostname.includes('gadgetsweb') || hostname.includes('hblinks') || hostname.includes('gamerxyt'))) {
                return true;
            }
            // Homepage without query params is not a file
            return false;
        }

        // For file hosting services, require /file/ or /drive/ or /d/ in path
        ...
        // For stream services, require /file/ or /e/ (embed)
        if (hostname.includes('hdstream4u') || hostname.includes('ninjastream') || hostname.includes('dood')) {
            return pathname.includes('/file/') || pathname.includes('/e/') || pathname.includes('/d/');
        }

        // For direct file services (workers.dev, r2.dev, pixeldrain)
        if (hostname.includes('workers.dev') || hostname.includes('r2.dev') || hostname.includes('googleusercontent')) {
            return true; // These are direct file URLs
        }

        if (hostname.includes('pixeldrain')) {
            return pathname.includes('/u/') || pathname.includes('/api/');
        }

        // For other services, require a meaningful path (not just /)
        return pathname.length > 3;
    } catch (error) {
        // If URL parsing fails, fall back to basic check
        return hasHostPattern;
    }
}
