...
export async function getHDHub4uStreams(imdbId, type, season = null, episode = null) {
    try {
        ...
        if (type !== 'movie' && normalizedTarget && !normalizedContentTitle.includes(normalizedTarget)) {
            console.log(`[HDHub4u] Skipping content due to title mismatch: "${content.title}" vs target "${normalizedTarget}"`);
            return [];
        }
