...
export async function getHDHub4uStreams(imdbId, type, season = null, episode = null) {
    try {
        ...
        // Filter out googleusercontent.com if it's the ONLY option
        const hasGoogleusercontent = validated.some(link => link.url?.includes('googleusercontent.com'));
        const hasOtherLinks = validated.some(link => !link.url?.includes('googleusercontent.com'));

        if (hasGoogleusercontent && !hasOtherLinks) {
            console.log('[HDHub4u] Skipping results - only googleusercontent.com links found (user requested to skip these)');
            return [];
        }

        // If we have other links, filter out googleusercontent
        const finalValidated = hasOtherLinks
            ? validated.filter(link => !link.url?.includes('googleusercontent.com'))
            : validated;

        if (hasGoogleusercontent && hasOtherLinks) {
            console.log(`[HDHub4u] Filtered out ${validated.length - finalValidated.length} googleusercontent.com link(s), keeping ${finalValidated.length} other links`);
        }
