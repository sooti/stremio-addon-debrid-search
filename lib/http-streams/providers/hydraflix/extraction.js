...
async function extractVidSrc(embedUrl) {
    try {
        ...
        // We'll use vidsrc.to's rip API as a fallback
        ...
        // Use vidsrc.to rip API
        let ripUrl;
        if (contentType === 'tv' && season && episode) {
            ripUrl = `https://vidsrc.to/embed/tv/${id}/${season}/${episode}`;
        } else {
            ripUrl = `https://vidsrc.to/embed/movie/${id}`;
        }
...
export async function extractEmbedUrl(embedUrl, serverName) {
    ...
    try {
        ...
        if (tmdbId && season && episode) {
            ...
            // Use vidsrc.to which is more reliable and widely supported
