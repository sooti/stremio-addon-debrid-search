...
const STREMSRC_SOURCE_URL = "https://vidsrc-embed.ru/embed";
...
/**
 * Grabs RCP data from HTML or JavaScript
 * @param {string} html - HTML or JavaScript content
 * @returns {Object|null} Object with metadata and data, or null
 */
export function rcpGrabber(html) {
    // Try multiple patterns to extract stream URL

    const patterns = [
        { name: "src: 'url'", regex: /src:\s*'([^']*)'/ },
        { name: "file: 'url'", regex: /file:\s*['"]([^'"]+)['"]/ },
        { name: "iframe src", regex: /<iframe[^>]+src=["']([^"']+)["']/i },
        { name: "source: 'url'", regex: /source:\s*["']([^"']+)["']/ },
        { name: "direct m3u8", regex: /(https?:\/\/[^\s'"<>]+\.m3u8[^\s'"<>]*)/ },
        { name: "sources array", regex: /sources:\s*\[\s*["']([^"']+)["']/ },
        { name: "url: m3u8", regex: /url:\s*["']([^"']+\.m3u8[^"']*)["']/ },
    ];

    for (const pattern of patterns) {
        const match = html.match(pattern.regex);
        if (match) {
            console.log(`[RCP Grabber] Matched pattern: ${pattern.name}`);
            console.log(`[RCP Grabber] Extracted URL: ${match[1].substring(0, 100)}...`);
            return {
                metadata: { image: "" },
                data: match[1],
            };
    ...
    console.log('[RCP Grabber] No pattern matched. Response preview:', html.substring(0, 300));
    return null;
}
