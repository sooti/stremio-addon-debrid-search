 ...
 * @param {boolean} [options.requirePartialContent=false] - Require HTTP 206 response
 * @returns {Promise<{isValid: boolean, filename: string|null, statusCode: number|null}>} Validation details
 */
export function validateSeekableUrl(url, options = {}) {
    ...
    if (disableSeekValidation) {
        if (requirePartialContent) {
            console.log(`[4KHDHub] Seek validation disabled but 206 confirmation requested for ${url}`);
            return Promise.resolve({ isValid: false, filename: null, statusCode: null });
        }
        return validateUrl(url).then(isValid => ({ isValid, filename: null, statusCode: null }));
    }
        ...
        try {
            ...
            if (isTrustedHost && !requirePartialContent) {
                ...
                const req = protocol.request(url, options, (res) => {
                    ...
                    res.destroy();
                    resolve({ isValid: true, filename, statusCode: res.statusCode });
                });

                req.on('error', () => {
                    req.destroy(); // Ensure request is destroyed on error
                    resolve({ isValid: true, filename: null, statusCode: null });
                });
                req.on('timeout', () => {
                    req.destroy();
                    resolve({ isValid: true, filename: null, statusCode: null });
            ...
            const req = protocol.request(url, options, (res) => {
                ...
                if (urlObj.hostname.includes('googleusercontent.com')) {
                    ...
                    if (res.statusCode === 206) {
                        ...
                        res.destroy();
                        resolve({ isValid: false, filename, statusCode: res.statusCode });
                        return;
                    }
                }
                ...
                res.destroy(); // Close connection immediately
                resolve({ isValid: finalValidity, filename, statusCode: res.statusCode });
            });

            req.on('error', (err) => {
                console.log(`[4KHDHub] Seek validation error for ${url}: ${err.message}`);
                req.destroy(); // Ensure request is destroyed on error
                resolve({ isValid: false, filename: null, statusCode: null });
            });
