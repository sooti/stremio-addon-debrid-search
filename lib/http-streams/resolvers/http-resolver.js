...
const HTTP_RESOLVE_CACHE_TTL_MS = parseInt(process.env.HTTP_RESOLVE_CACHE_TTL_MS || '600000', 10);

function buildCacheKey(decodedUrl) {
    const hash = crypto.createHash('md5').update(decodedUrl).digest('hex');
    return `httpstreaming:${hash}`;
}

async function getCachedResolveUrl(cacheKey) {
    if (!sqliteCache?.isEnabled()) return null;
    try {
        const cached = await sqliteCache.getCachedHttpResolveUrl(cacheKey);
        if (cached?.url) {
            console.log(`[HTTP-RESOLVE] Using SQLite cached link for ${cacheKey.substring(0, 10)}...`);
            return cached.url;
        }
    } catch (error) {
        console.error(`[HTTP-RESOLVE] Failed to read SQLite cache for ${cacheKey.substring(0, 10)}: ${error.message}`);
    }
    return null;
}

async function persistResolveUrl(cacheKey, finalUrl) {
    if (!sqliteCache?.isEnabled() || !finalUrl) return;
    try {
        await sqliteCache.setCachedHttpResolveUrl(cacheKey, finalUrl, HTTP_RESOLVE_CACHE_TTL_MS);
        console.log(`[HTTP-RESOLVE] Stored SQLite cache entry for ${cacheKey.substring(0, 10)}...`);
    } catch (error) {
        console.error(`[HTTP-RESOLVE] Failed to persist SQLite cache for ${cacheKey.substring(0, 10)}: ${error.message}`);
    }
}
...
export async function resolveHttpStreamUrl(redirectUrl) {
    try {
        ...
        const cacheKey = buildCacheKey(decodedUrl);

        const cachedUrl = await getCachedResolveUrl(cacheKey);
        if (cachedUrl) {
            return cachedUrl;
        }
        ...
        if (seekableLink) {
            ...
            await persistResolveUrl(cacheKey, seekableLink);
            return seekableLink;
        }
