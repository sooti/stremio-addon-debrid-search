...
function makeRequest(url, options = {}) {
    ...
    const DEFAULT_TIMEOUT = parseInt(process.env.REQUEST_TIMEOUT) || 5000; // AGGRESSIVE: 5s timeout for ultra-fast requests
...
async function get4KHDHubStreams(tmdbId, type, season = null, episode = null, config) {
    try {
        ...
        if (cachedStreams && cachedStreams.length > 0) {
            ...
            const MAX_DOWNLOAD_LINKS = parseInt(process.env.MAX_DOWNLOAD_LINKS) || 6; // Only process first 6 download links
            ...
            const MIN_4K_STREAMS = parseInt(process.env.MIN_4K_STREAMS) || 1;
