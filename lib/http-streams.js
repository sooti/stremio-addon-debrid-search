 ...
 * Note: Dizipall24 primarily serves Turkish content
 *
...
async function getDizipall24Streams(tmdbId, type, season = null, episode = null, config) {
    try {
        ...
        // Dizipall24 only supports series for now
        if (type !== 'series') {
            console.log(`[DIZIPALL24] Only series are supported, skipping type: ${type}`);
            return [];
        }
        ...
        // Get metadata to find series name
        const cinemetaDetails = await Cinemeta.getMeta(type, tmdbId);
        if (!cinemetaDetails) {
        ...
        const seriesName = cinemetaDetails.name;
        console.log(`[DIZIPALL24] Series name: ${seriesName}`);
        ...
        const streamInfo = await dizipall24.searchAndGetStream(seriesName, season, episode);

        if (!streamInfo) {
            console.log(`[DIZIPALL24] No stream found for ${seriesName} S${season}E${episode}`);
            return [];
        }
        ...
        const formattedStream = {
            ...
            behaviorHints: {
                ...
                bingeGroup: `dizipall24-${seriesName.toLowerCase().replace(/[^a-z0-9]/g, '-')}`
            }
        };
