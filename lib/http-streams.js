...
const MEMORY_CACHE_TTL = 5 * 60 * 1000; // 5 minutes in-memory cache

// URL validation cache to avoid re-validating same URLs
const URL_VALIDATION_CACHE = new Map();
const URL_VALIDATION_CACHE_TTL = 30 * 60 * 1000; // 30 minutes cache for validation results
...
setInterval(() => {
    ...
    // Clean up URL validation cache
    for (const [key, value] of URL_VALIDATION_CACHE.entries()) {
        if (now - value.timestamp > URL_VALIDATION_CACHE_TTL) {
            URL_VALIDATION_CACHE.delete(key);
            removedCount++;
        }
    }
    ...
    if (removedCount > 0) {
        console.log(`[4KHDHub] Cache cleanup: removed ${removedCount} expired entries`);
    }
...
const getFromCache = async (key) => {
  ...
  const memoryCacheKey = `memory_${key}`;
  const memoryCached = MEMORY_CACHE.get(memoryCacheKey);
  if (memoryCached) {
    const now = Date.now();
    if (now - memoryCached.timestamp < MEMORY_CACHE_TTL) {
      console.log(`[4KHDHub] Memory cache HIT for ${key}`);
      return memoryCached.data;
    } else {
      // Remove expired entry
      MEMORY_CACHE.delete(memoryCached);
    }
  }
  ...
  try {
    ...
    // Check if cache is expired (configurable expiration, default 24 hours)
    const cacheTime = parsedData.timestamp || 0;
    const now = Date.now();
    const expiryTime = (process.env.CACHE_EXPIRY_HOURS || 24) * 60 * 60 * 1000; // Configurable expiry in hours
    ...
    MEMORY_CACHE.set(memoryCacheKey, {
...
function validateUrl(url) {
    ...
    const timeout = parseInt(process.env.VALIDATION_TIMEOUT) || 600; // AGGRESSIVE: 600ms for ultra-fast validation
...
function validateSeekableUrl(url) {
    ...
    if (disableSeekValidation) {
        return validateUrl(url).then(isValid => {
            const result = { isValid, filename: null };
            URL_VALIDATION_CACHE.set(cacheKey, { result, timestamp: Date.now() });
            return result;
        });
    }

    // Wrap the validation promise to cache the result
...
function makeRequest(url, options = {}) {
    ...
    const DEFAULT_TIMEOUT = parseInt(process.env.REQUEST_TIMEOUT) || 2500; // AGGRESSIVE: 5s timeout for ultra-fast requests
    const MAX_RETRIES = parseInt(process.env.REQUEST_MAX_RETRIES) || 0; // AGGRESSIVE: No retries for maximum speed
    const RETRY_DELAY = parseInt(process.env.REQUEST_RETRY_DELAY) || 0; // No delay needed if no retries
...
async function scrape4KHDHubSearch(searchQuery) {
    try {
        ...
        // SPEED OPTIMIZATION: Reduced search strategies to just 2 for faster results
...
async function get4KHDHubStreams(tmdbId, type, season = null, episode = null, config) {
    try {
        ...
        // Create cache key for extracted streams
        const cacheKey = `4khdhub_streams_v7_${tmdbId}_${type}${season ? `_s${season}e${episode}` : ''}`;

        let streamingLinks = [];
        let pageLanguages = []; // Store languages from the page badge - must be declared here for scope

        // 1. Check cache for extracted streams first
        let cachedStreams = await getFromCache(cacheKey);
        if (cachedStreams && cachedStreams.length > 0) {
            console.log(`[4KHDHub] Cache HIT for ${cacheKey}. Using ${cachedStreams.length} cached stream(s).`);
            streamingLinks = cachedStreams;
        } else {
            if (cachedStreams && cachedStreams.length === 0) {
            ...
            // SPEED OPTIMIZATION: Simplified search with only 1 fallback
            let searchResults = [];
            let bestMatch = null;
            ...
            if (searchResults.length > 0) {
                sortedMatches = getSortedMatches(searchResults, cinemetaDetails.name);
                bestMatch = sortedMatches[0];
            }

            // SPEED OPTIMIZATION: Only try ONE fallback if primary search fails
            if (!bestMatch && searchResults.length === 0) {
                console.log(`[4KHDHub] ⚡ No results from primary search, trying one fallback...`);

                // Try search without year as the only fallback
                const titleWithoutYear = removeYear(cinemetaDetails.name);
                if (titleWithoutYear !== cinemetaDetails.name) {
                    console.log(`[4KHDHub] Fallback search: "${titleWithoutYear}"`);
                    const fallbackResults = await scrape4KHDHubSearch(titleWithoutYear);
                    if (fallbackResults.length > 0) {
                        const fallbackSorted = getSortedMatches(fallbackResults, cinemetaDetails.name);
                        bestMatch = fallbackSorted[0];
                        sortedMatches = fallbackSorted;
            ...
            if (type === 'movie') {
                // Try sorted matches in order until we find one with valid year
                let validMatch = null;
                ...
                if (!validMatch) {
                    console.log(`[4KHDHub] No match passed year validation`);
                    return [];
                }
            ...
            // AGGRESSIVE OPTIMIZATION: Limit download links to process for faster results
            const MAX_DOWNLOAD_LINKS = parseInt(process.env.MAX_DOWNLOAD_LINKS) || 4; // Only process first 6 download links
            downloadLinks = downloadLinks.slice(0, MAX_DOWNLOAD_LINKS);
            console.log(`[4KHDHub] ⚡ Speed optimization: Processing only ${downloadLinks.length} download links (out of original set)`);

            // AGGRESSIVE OPTIMIZATION: Stop at first 4K stream for ultra-fast results
            const MAX_WORKING_LINKS = parseInt(process.env.MAX_4KHDHUB_LINKS) || 6;
            // OPTIMIZATION: Increased batch size to 6 to process all links in one batch
            const PARALLEL_BATCH_SIZE = parseInt(process.env.BATCH_SIZE) || 6;
            // AGGRESSIVE OPTIMIZATION: Stop at first 4K stream
            const MIN_4K_STREAMS = parseInt(process.env.MIN_4K_STREAMS) || 2;
            ...
            console.log(`[4KHDHub] ⚡ SPEED MODE: Will stop after ${MIN_4K_STREAMS} 4K stream(s) or ${MAX_WORKING_LINKS} total streams`);

            // Process in batches for parallel execution with early exit
            let earlyExit = false;
            for (let i = 0; i < prioritizedLinks.length && !earlyExit; i += PARALLEL_BATCH_SIZE) {
                ...
                for (const result of batchResults) {
                    ...
                    // OPTIMIZATION: Early exit if we have enough quality streams
                    const count4K = streamingLinks.filter(s => {
                        const title = (s.title || s.name || '').toLowerCase();
                        return title.includes('2160p') || title.includes('4k');
                    }).length;

                    // Exit early if we have minimum 4K streams OR reached max working links
                    if (count4K >= MIN_4K_STREAMS) {
                        console.log(`[4KHDHub] ⚡ Early exit: Found ${count4K} 4K streams (target: ${MIN_4K_STREAMS})`);
                        earlyExit = true;
                        break;
                    }

                    if (streamingLinks.length >= MAX_WORKING_LINKS) {
                        console.log(`[4KHDHub] ⚡ Early exit: Reached ${MAX_WORKING_LINKS} working links`);
                        earlyExit = true;
                        break;
                    }
                }
            ...
            console.log(`[4KHDHub] Extracted ${streamingLinks.length} working stream(s) (${streamingLinks.filter(s => {
                const title = (s.title || s.name || '').toLowerCase();
                return title.includes('2160p') || title.includes('4k');
            }).length} in 4K)`);

            // Cache the extracted streams
            await saveToCache(cacheKey, streamingLinks);
        }
        ...
        // Smart deduplication: Group by quality + size, keep only best CDN per group
        // This prevents multiple CDN links for the same quality (e.g., Pixeldrain + FSL for same 2160p file)
        const qualityGroups = new Map();

        for (const link of uniqueLinks) {
            // Extract resolution from title/name
            const titleLower = (link.title || link.name || '').toLowerCase();
            const resolutionMatch = titleLower.match(/(\d{3,4})p/);
            const resolution = resolutionMatch ? resolutionMatch[1] : 'unknown';

            // Parse size to bytes for comparison
            let sizeBytes = 0;
            if (link.size) {
                const sizeMatch = link.size.match(/([\d.]+)\s*(GB|MB|KB)/i);
                if (sizeMatch) {
            ...
            // Create a key based on resolution and size range
            // Group by size buckets: <500MB, 500MB-1GB, 1-2GB, 2-5GB, 5-10GB, 10GB+
            let sizeBucket = 'unknown';
            if (sizeBytes > 0) {
                const sizeGB = sizeBytes / (1024 * 1024 * 1024);
                if (sizeGB < 0.5) sizeBucket = '<500MB';
                else if (sizeGB < 1) sizeBucket = '500MB-1GB';
                else if (sizeGB < 2) sizeBucket = '1-2GB';
                else if (sizeGB < 5) sizeBucket = '2-5GB';
                else if (sizeGB < 10) sizeBucket = '5-10GB';
                else sizeBucket = '10GB+';
            }
            const groupKey = `${resolution}_${sizeBucket}`;

            // Determine CDN priority (lower is better)
            const url = link.url.toLowerCase();
            let cdnPriority = 99;
            if (url.includes('pixeldrain')) cdnPriority = 0;
            else if (url.includes('hubcloud')) cdnPriority = 1;
            else if (url.includes('fsl')) cdnPriority = 2;
            else if (url.includes('workers.dev') || url.includes('hubcdn')) cdnPriority = 3;
            else if (url.includes('googleusercontent')) cdnPriority = 4;

            // Check if we already have a stream in this quality group
            const existing = qualityGroups.get(groupKey);
            if (!existing || cdnPriority < existing.cdnPriority) {
                qualityGroups.set(groupKey, { link, cdnPriority });
            }
        }

        // Extract deduplicated links
        const deduplicatedLinks = Array.from(qualityGroups.values()).map(item => item.link);

        console.log(`[4KHDHub] After quality dedup: ${deduplicatedLinks.length} unique quality levels (removed ${uniqueLinks.length - deduplicatedLinks.length} CDN duplicates)`);

        // Validate URLs if DISABLE_4KHDHUB_URL_VALIDATION is false
        let validatedLinks = deduplicatedLinks;
        ...
        if (!disableValidation) {
            ...
            console.log(`[4KHDHub] URL validation enabled, validating ${deduplicatedLinks.length} links...`);
            console.log(`[4KHDHub] Seek validation ${enableSeekValidation ? 'enabled' : 'disabled'}`);

            // OPTIMIZATION: Track validated filenames to skip duplicate mirrors
            const validatedFilenames = new Map(); // filename -> true (if valid)

            // Helper function to extract filename from URL for deduplication
            const extractFilenameForDedup = (url) => {
                try {
                    const urlObj = new URL(url);
                    const pathname = decodeURIComponent(urlObj.pathname);
                    let filename = pathname.split('/').pop();

                    // For Google User Content URLs, try query parameters
                    if (urlObj.hostname.includes('googleusercontent.com')) {
                        const searchParams = urlObj.searchParams;
                        if (searchParams.has('file')) filename = searchParams.get('file');
                        else if (searchParams.has('name')) filename = searchParams.get('name');
                        else if (searchParams.has('title')) filename = searchParams.get('title');
                    }

                    // Remove file extension and normalize
                    filename = filename.replace(/\.(mkv|mp4|avi|webm)$/i, '').toLowerCase();

                    // For very short filenames (likely hashes), use full path
                    if (filename.length < 10) {
                        filename = pathname.toLowerCase();
                    }
            ...
            for (const link of deduplicatedLinks) {
                try {
                    ...
                    // Check if this host is in the trusted list (expanded for speed)
                    const isTrustedHost = [
                        'pixeldrain.dev',
                        'pixeldrain.com',
                        'pixeldrain.net',
                    ...
                    if (isTrustedHost) {
                        ...
                        // Track trusted host filenames as validated
                        const filename = extractFilenameForDedup(link.url);
                        if (filename) {
                            validatedFilenames.set(filename, true);
                        }
                    } else {
            ...
            console.log(`[4KHDHub] Trusted hosts: ${trustedHosts.length} links (auto-validated)`);
            ...
            if (otherLinks.length > 0) {
                ...
                const chunkSize = parseInt(process.env.VALIDATION_CHUNK_SIZE) || 30; // AGGRESSIVE: Validate 30 at once for maximum speed
                ...
                for (let i = 0; i < otherLinks.length; i += chunkSize) {
                    ...
                    const validationPromises = chunk.map(async (link) => {
                        // OPTIMIZATION: Check if we already validated this filename
                        const filename = extractFilenameForDedup(link.url);
                        if (filename && validatedFilenames.has(filename)) {
                            console.log(`[4KHDHub] ⚡ Skipping validation for duplicate filename: ${filename.substring(0, 50)}...`);
                            skippedDuplicates++;
                            return link; // Return link as valid without validation
                        }
                        ...
                        // Mark this filename as validated
                        if (filename) {
                            validatedFilenames.set(filename, true);
                        }
                ...
                if (enableSeekValidation) {
                    console.log(`[4KHDHub] Seek validation complete: ${validatedOtherLinks.length}/${otherLinks.length} non-trusted links are seekable (${skippedDuplicates} duplicates skipped)`);
                } else {
                    console.log(`[4KHDHub] Basic validation complete: ${validatedOtherLinks.length}/${otherLinks.length} non-trusted links are valid (${skippedDuplicates} duplicates skipped)`);
                }
            } else {
        ...
        const filteredStreams = filterStreamsByLanguage(streams, config.Languages);
        ...
        if ((type === 'series' || type === 'tv') && season && episode) {
            ...
            const episodeFilteredStreams = filteredStreams.filter(stream => {
            ...
            console.log(`[4KHDHub] Episode filtering: ${filteredStreams.length} -> ${episodeFilteredStreams.length} streams after filtering`);
...
async function getStreamSrcStreams(tmdbId, type, season = null, episode = null, config) {
  try {
    ...
    // Filter by language if config has language requirements
    let filteredStreams = filterStreamsByLanguage(streams, config?.Languages || []);
    ...
    console.log(`[StreamSrc] Returning ${filteredStreams.length} streams`);
    return filteredStreams;
  } catch (error) {
...
// Export functions
export { get4KHDHubStreams, getStreamSrcStreams, validateSeekableUrl, resolveHttpStreamUrl };
