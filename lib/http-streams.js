...
// In-memory cache for frequently accessed items with LRU eviction
const MEMORY_CACHE = new Map();
const MEMORY_CACHE_TTL = 5 * 60 * 1000; // 5 minutes in-memory cache
const MEMORY_CACHE_MAX_SIZE = parseInt(process.env.MEMORY_CACHE_MAX_SIZE || '1000', 10); // Max entries to prevent unbounded growth

// LRU eviction for MEMORY_CACHE
const evictOldestCacheEntry = () => {
  if (MEMORY_CACHE.size >= MEMORY_CACHE_MAX_SIZE) {
    // Find oldest entry (first key in insertion order)
    const oldestKey = MEMORY_CACHE.keys().next().value;
    if (oldestKey) {
      MEMORY_CACHE.delete(oldestKey);
      console.log(`[4KHDHub] Evicted oldest cache entry, cache size: ${MEMORY_CACHE.size}/${MEMORY_CACHE_MAX_SIZE}`);
    }
  }
};
...
const getFromCache = async (key) => {
  ...
  if (memoryCached) {
    ...
    if (now - memoryCached.timestamp < MEMORY_CACHE_TTL) {
      ...
      MEMORY_CACHE.delete(memoryCacheKey);
      MEMORY_CACHE.set(memoryCacheKey, memoryCached);
  ...
  try {
    ...
    // Add to in-memory cache for faster subsequent access (with LRU eviction)
            ...
            if (!bestMatch && searchResults.length > 0) {
                ...
                if (titleWithoutYear !== cinemetaDetails.name) {
                    ...
                    if (fallbackResults.length > 0) {
                        ...
                        const fallbackMatch = fallbackSorted[0];
                        if (fallbackMatch && (!bestMatch || (fallbackMatch.score || 0) > (bestMatch.score || 0))) {
                ...
                if (!bestMatch) {
                    ...
                    for (const altQuery of alternativeQueries) {
                        ...
                        if (altResults.length > 0) {
                            const altSorted = getSortedMatches(altResults, cinemetaDetails.name);
                            const altMatch = altSorted[0];
                            if (altMatch && (!bestMatch || (altMatch.score || 0) > (bestMatch.score || 0))) {
                                bestMatch = altMatch;
                                sortedMatches = altSorted;
                                searchResults = altResults;
                                const scoreDisplay = (altMatch.score !== undefined && altMatch.score !== null) ? altMatch.score.toFixed(1) : 'N/A';
                                console.log(`[4KHDHub] Found better match with query: "${altQuery}" (score: ${scoreDisplay})`);
                                break;
                            }
                        }
                    }
