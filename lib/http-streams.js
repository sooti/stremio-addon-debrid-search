 ...
 */
async function getVidLinkProStreams(tmdbId, type, season = null, episode = null, config = {}) {
    try {
        console.log(`[VidLink] Starting search for TMDB ID: ${tmdbId}, Type: ${type}${season ? `, Season: ${season}` : ''}${episode ? `, Episode: ${episode}` : ''}`);
        ...
        try {
            const cinemetaDetails = await Cinemeta.getMeta(type, tmdbId);
            if (cinemetaDetails) {
                mediaTitle = cinemetaDetails.name || 'Unknown';
            }
        } catch (error) {
            console.warn('[VidLink] Could not fetch Cinemeta details:', error.message);
        }
