...
async function scrape4KHDHubSearch(searchQuery) {
    try {
        ...
        // Try each search strategy in parallel but with a delay to avoid rate limiting
        let allResults = [];
        for (const strategy of uniqueStrategies) {
            try {
                ...
                if (response.statusCode === 200) {
                    ...
                    // If we found good results with primary search, we might not need others
                    if (strategy.name === 'primary' && results.length > 2) {
...
function extractStreamingLinks(downloadLinks) {
    ...
    return Promise.all(downloadLinks.map((link, index) => processLink(link, index)))
        .then(results => {
            const validResults = results.filter(result => result !== null);
...
export { get4KHDHubStreams, getStreamSrcStreams };
