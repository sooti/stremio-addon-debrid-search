...
function filterByLanguage(streams, languages) {
    ...
    return streams.filter(stream => {
        const streamLangs = stream.title.match(/\b(English|French|Spanish|German|Italian|Portuguese|Russian|Hindi|Japanese|Korean|Chinese|Arabic|Turkish|Dutch|Swedish|Norwegian|Danish|Finnish|Polish|Czech|Hungarian|Romanian|Greek|Hebrew|Thai|Tamil|Malayalam|Telugu|Kannada)\b/gi) || [];
...
async function get4KHDHubStreams(tmdbId, type, season = null, episode = null, config) {
    try {
        ...
        if (!disableValidation) {
            ...
            const validationPromises = uniqueLinks.map(async (link) => {
                ...
                if (result.filename) {
                    ...
                    const originalLangs = link.title.match(/\b(English|French|Spanish|German|Italian|Portuguese|Russian|Hindi|Japanese|Korean|Chinese|Arabic|Turkish|Dutch|Swedish|Norwegian|Danish|Finnish|Polish|Czech|Hungarian|Romanian|Greek|Hebrew|Thai|Tamil|Malayalam|Telugu|Kannada)\b/gi) || [];
        ...
        const streams = validatedLinks.map(link => {
            ...
            const langs = link.title.match(/\b(English|French|Spanish|German|Italian|Portuguese|Russian|Hindi|Japanese|Korean|Chinese|Arabic|Turkish|Dutch|Swedish|Norwegian|Danish|Finnish|Polish|Czech|Hungarian|Romanian|Greek|Hebrew|Thai|Tamil|Malayalam|Telugu|Kannada)\b/gi) || [];
