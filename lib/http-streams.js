...
/**
 * Get Dizipall24 streams (Turkish streaming site)
 * Note: Dizipall24 primarily serves Turkish content (movies & series)
 *
 * @param {string} tmdbId - TMDB/IMDB ID (will be used to get metadata)
 ...
 */
async function getDizipall24Streams(tmdbId, type, season = null, episode = null, config) {
    try {
        console.log(`[DIZIPALL24] Starting search for TMDB ID: ${tmdbId}, Type: ${type}${season ? `, Season: ${season}` : ''}${episode ? `, Episode: ${episode}` : ''}`);

        // Get metadata to find content name
        const cinemetaDetails = await Cinemeta.getMeta(type, tmdbId);
        if (!cinemetaDetails) {
            console.log(`[DIZIPALL24] Could not fetch metadata for ID: ${tmdbId}`);
            return [];
        }

        const contentName = cinemetaDetails.name;
        console.log(`[DIZIPALL24] Content name: ${contentName}`);

        // Validate required parameters for series
        if (type === 'series' && (!season || !episode)) {
            console.log(`[DIZIPALL24] Season and episode required for series`);
            return [];
        }

        // Import dizipall24 module dynamically
        let dizipall24;
        try {
            const dizipall24Module = await import('./dizipall24.js');
            dizipall24 = dizipall24Module.default;
        } catch (importError) {
            console.error(`[DIZIPALL24] Failed to import dizipall24 module:`, importError);
            return [];
        }

        // Search and get stream
        const streamInfo = await dizipall24.searchAndGetStream(contentName, type, season, episode);

        if (!streamInfo) {
            const contentId = type === 'movie' ? contentName : `${contentName} S${season}E${episode}`;
            console.log(`[DIZIPALL24] No stream found for ${contentId}`);
            return [];
        }

        // Check if stream is expired or about to expire (less than 5 minutes)
        if (streamInfo.expiresIn && streamInfo.expiresIn < 300000) {
            console.warn(`[DIZIPALL24] Stream expires in ${Math.round(streamInfo.expiresIn / 1000)} seconds, may need refresh`);
        }

        console.log(`[DIZIPALL24] Found stream: ${streamInfo.title}`);
        console.log(`[DIZIPALL24] Stream expires in: ${streamInfo.expiresIn ? Math.round(streamInfo.expiresIn / 60000) : 'unknown'} minutes`);

        // Format stream in the same format as other HTTP streams
        const formattedStream = {
            name: 'Dizipall24 🇹🇷',
            title: streamInfo.title + (streamInfo.expiresIn ? ` (expires in ${Math.round(streamInfo.expiresIn / 60000)}min)` : ''),
            url: streamInfo.url,
            behaviorHints: {
                notWebReady: true, // HLS streams work better in native players
                bingeGroup: `dizipall24-${contentName.toLowerCase().replace(/[^a-z0-9]/g, '-')}`
            }
        };

        console.log(`[DIZIPALL24] Returning 1 stream result`);
...
// Export functions
export { get4KHDHubStreams, getStreamSrcStreams, getDizipall24Streams, validateSeekableUrl, resolveHttpStreamUrl, extractHubCloudLinks };
