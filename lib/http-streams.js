...
function extractHubCloudLinks(url, referer) {
    ...
    return makeRequest(url, { parseHTML: true })
        .then(response => {
            ...
            // Check if this is already a hubcloud.php or hubcloud.one page
            let href;
            if (url.includes('hubcloud.php') || url.includes('gamerxyt.com') || url.includes('hubcloud.one')) {
                // If it's already a direct hubcloud link, check if we should extract from it directly
                if (url.includes('hubcloud.one')) {
                    console.log(`Already a hubcloud.one URL, will process directly: ${url}`);
                    // For hubcloud.one pages, skip the download button search and go straight to extraction
                    href = url;
                } else {
                    href = url;
        ...
        .then(response => {
            ...
            let downloadButtons = $('a.btn[href*="workers.dev"], a.btn[href*="hubcdn.fans"]');
        ...
        const streams = validatedLinks.map(link => {
            ...
            if (resolution === 'other') {
                ...
                if (titleLower.includes('h265') || titleLower.includes('hevc')) {
                    resolution = '2160p'; // assume 4K for H265/HEVC
                } else if (titleLower.includes('h264')) {
                    resolution = '1080p'; // assume 1080p for H264
                } else {
