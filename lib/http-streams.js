...
async function get4KHDHubStreams(tmdbId, type, season = null, episode = null, config) {
    try {
        ...
        if (cachedStreams && cachedStreams.length > 0) {
            ...
            // Optimized extraction: parallel processing with early stopping
            console.log(`[4KHDHub] Found ${downloadLinks.length} redirect URLs, extracting final streams...`);

            const MAX_WORKING_LINKS = 10; // Stop after finding this many working links
            const PARALLEL_BATCH_SIZE = 5; // Process this many links at once

            streamingLinks = [];

            // Process in batches for parallel execution
            for (let i = 0; i < downloadLinks.length && streamingLinks.length < MAX_WORKING_LINKS; i += PARALLEL_BATCH_SIZE) {
                const batch = downloadLinks.slice(i, i + PARALLEL_BATCH_SIZE);
                ...
                for (const result of batchResults) {
                    ...
                    if (streamingLinks.length >= MAX_WORKING_LINKS) {
                        console.log(`[4KHDHub] Reached ${MAX_WORKING_LINKS} working links, stopping early`);
                        break;
                    }
                }
            }
        ...
        const streams = validatedLinks.map(link => {
            ...
            // Detect languages from the title using centralized language mapping
            const detectedLanguages = detectLanguagesFromTitle(link.title);
