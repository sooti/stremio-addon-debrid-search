...
async function resolveStream(apiKey, urlToResolve, type, id) {
    ...
    let history = await OCClient.cloud.history();
    let torrents = await processTorrents(OCClient, history);
    ...
    console.log(`[OC RESOLVER] Item added (requestId: ${addedItem.requestId}). Waiting 8 seconds...`);
    await delay(8000);
    ...
    if (newItemInHistory) {
        ...
        if (processedFiles.length > 0) {
            ...
            const searchKey = type === 'series'
                ? `${cinemetaDetails.name} s${id.split(':')[1]}e${id.split(':')[2]}`
