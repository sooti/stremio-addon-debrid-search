...
const BTDIG_URL = 'https://btdig.com';
...
const BTDIG_ENABLED = process.env.BTDIG_ENABLED === 'true';
// ===================================================================================
...
async function searchOffcloudTorrents(apiKey, type, id) {
    ...
    if (BTDIG_ENABLED) scraperPromises.push(searchBtdig(specificSearchKey, signal));
...
async function combineAndMarkResults(apiKey, personalFiles, externalSources) {
    const sourceNames = ['Jackett', 'Torrentio', 'Zilean', 'Comet', 'StremThru', 'AIOStreams', 'BTDigg'];
    const enabledFlags = [JACKETT_ENABLED, TORRENTIO_ENABLED, ZILEAN_ENABLED, COMET_ENABLED, STREMTHRU_ENABLED, AIOSTREAMS_ENABLED, BTDIG_ENABLED];
...
// ===== BTDig config toggles =====
const BTDIG_DEBUG = process.env.BTDIG_DEBUG === 'true';
const BTDIG_MAX_PAGES = parseInt(process.env.BTDIG_MAX_PAGES || '3', 10);
const BTDIG_MAX_RETRIES = parseInt(process.env.BTDIG_MAX_RETRIES || '4', 10);
const BTDIG_MIN_BACKOFF_MS = parseInt(process.env.BTDIG_MIN_BACKOFF_MS || '400', 10);
const BTDIG_MAX_BACKOFF_MS = parseInt(process.env.BTDIG_MAX_BACKOFF_MS || '4000', 10);
const BTDIG_COOKIE = process.env.BTDIG_COOKIE || '';
const BTDIG_UA_OVERRIDE = process.env.BTDIG_USER_AGENT || '';

// ===== Helper: UA rotation + retry/backoff =====
const UA_POOL = [
  BTDIG_UA_OVERRIDE,
...
function jitter(ms) {
  ...
  return ms + Math.floor(Math.random() * (delta * 2) - delta);
}

function backoffDelay(attempt) {
  const base = Math.min(BTDIG_MAX_BACKOFF_MS, BTDIG_MIN_BACKOFF_MS * Math.pow(2, attempt));
  return jitter(base);
}
...
function sleep(ms, signal) {
  return new Promise((resolve, reject) => {
    ...
    if (signal) {
      const onAbort = () => { clearTimeout(t); reject(new axios.Cancel('aborted')); };
      if (signal.aborted) { clearTimeout(t); return reject(new axios.Cancel('aborted')); }
      signal.addEventListener('abort', onAbort, { once: true });
    }
  });
}

function looksLikeRateLimitHTML(html = '') {
  const lower = html.toLowerCase();
  return lower.includes('just a moment') || lower.includes('checking your browser') || lower.includes('/cdn-cgi/');
}

async function getWithRetry(url, { timeout, signal }) {
  let lastErr;
  for (let attempt = 0; attempt <= BTDIG_MAX_RETRIES; attempt++) {
    ...
    try {
      const headers = {
        ...
        ...(BTDIG_COOKIE ? { 'Cookie': BTDIG_COOKIE } : {}),
        'Referer': 'https://btdig.com/',
      };

      const resp = await axios.get(url, { timeout, signal, headers });
      if (BTDIG_DEBUG) console.log(`[BTDig] Attempt ${attempt} status=${resp.status} bytes=${String(resp.data || '').length}`);
      ...
      if (resp.status === 429 || resp.status === 503) {
        ...
        if (BTDIG_DEBUG) console.log(`[BTDig] Got ${resp.status}. Backing off ${delayMs}ms...`);
        await sleep(delayMs, signal);
        continue;
      }

      const html = String(resp.data || '');
      if (looksLikeRateLimitHTML(html)) {
        const delayMs = backoffDelay(attempt);
        if (BTDIG_DEBUG) console.log(`[BTDig] Anti-bot HTML detected. Backing off ${delayMs}ms...`);
        await sleep(delayMs, signal);
        continue;
      }
      ...
      if (BTDIG_DEBUG) console.log(`[BTDig] Attempt ${attempt} failed (${status || err.code || err.message}).`);
      if (!retriable || attempt === BTDIG_MAX_RETRIES) break;

      const delayMs = backoffDelay(attempt);
      if (BTDIG_DEBUG) console.log(`[BTDig] Retrying after ${delayMs}ms...`);
      await sleep(delayMs, signal);
    }
  }
  throw lastErr || new Error('BTDig request failed');
}

// ===== Regex helpers inspired by your Python snippet =====
...
function parseTotalPages(html) {
  // <span style="color:rgb(100, 100, 100);padding:2px 10px">123 results found
  const m = html.match(/>(\d+)\s+results\s+found/i);
  if (!m) return 1;
  const totalResults = parseInt(m[1], 10);
  return Math.max(1, Math.ceil(totalResults / 10));
}

function parseOnePage(html) {
  const results = [];
  // split on one_result blocks
  const blocks = html.match(/<div class="one_result"[\s\S]*?(?=<div class="one_result"|$)/g) || [];
  if (BTDIG_DEBUG) console.log(`[BTDig] one_result blocks on page: ${blocks.length}`);

  for (const block of blocks) {
    const magnetMatch = block.match(/<a href="(magnet:\?xt=urn:btih:[^"]+)"/i);
    const nameMatch = block.match(/<div class="torrent_name"[^>]*>\s*<a[^>]*>([\s\S]*?)<\/a>/i);
    const sizeMatch = block.match(/<span class="torrent_size"[^>]*>([^<]+)<\/span>/i);

    if (!magnetMatch || !nameMatch) continue;
    const magnet = decodeEntities(magnetMatch[1]);
    const infoMatch = magnet.match(/btih:([a-fA-F0-9]{40})/);
    if (!infoMatch) continue;
    const infoHash = infoMatch[1].toLowerCase();

    const title = decodeEntities(stripTags(nameMatch[1])).trim();
    const sizeLabel = sizeMatch ? decodeEntities(sizeMatch[1]).trim() : null;

    results.push({
      Title: title || `BTDigg`,
      InfoHash: infoHash,
      Size: sizeLabel ? sizeToBytes(sizeLabel) : 0, // uses your existing sizeToBytes
      Seeders: null,
      Tracker: 'BTDigg'
    });
  }
...
// ===== The scraper itself =====
async function searchBtdig(query, signal) {
  console.time('[OC TIMER] Btdig');
  try {
    const base = `${BTDIG_URL}/search?order=0&q=${encodeURIComponent(query)}`;
    if (BTDIG_DEBUG) console.log(`[BTDig] Fetching first page: ${base}`);
    const firstResp = await getWithRetry(base, { timeout: SCRAPER_TIMEOUT, signal });
    const firstHtml = String(firstResp.data || '');
    const totalPages = Math.min(parseTotalPages(firstHtml), BTDIG_MAX_PAGES);
    if (BTDIG_DEBUG) console.log(`[BTDig] totalPages (capped): ${totalPages}`);

    const all = [];
    // page 0
    all.push(...parseOnePage(firstHtml));

    // subsequent pages ?p=1..N-1
    for (let p = 1; p < totalPages; p++) {
      if (signal?.aborted) break;
      const pageUrl = `${BTDIG_URL}/search?order=0&q=${encodeURIComponent(query)}&p=${p}`;
      if (BTDIG_DEBUG) console.log(`[BTDig] Fetching page ${p + 1}: ${pageUrl}`);
      const resp = await getWithRetry(pageUrl, { timeout: SCRAPER_TIMEOUT, signal });
      all.push(...parseOnePage(String(resp.data || '')));

      // be nice to the site
      await sleep(500 + Math.floor(Math.random() * 300), signal);
    }

    // De-dupe by infohash
    const dedup = Array.from(new Map(all.map(r => [r.InfoHash, r])).values());
    if (BTDIG_DEBUG) console.log(`[BTDig] Found ${all.length} raw results, ${dedup.length} unique.`);

    console.timeEnd('[OC TIMER] Btdig');
    return dedup;
  } catch (err) {
    console.timeEnd('[OC TIMER] Btdig');
    if (!axios.isCancel(err)) {
      console.error(`[BTDig] Search failed: ${err.message}`);
    }
    return [];
  }
}
