    ...
    const ocHandler = {
        ...
        checkCachedHashes: async (hashes) => {
            ...
            try {
                ...
                const pre = await cacheDb.checkHashesCached('offcloud', lowered);
                const toQuery = lowered.filter(h => !pre.has(h));
                if (toQuery.length === 0) return pre;
                const url = `${OFFCLOUD_API_URL}/cache?key=${apiKey}`;
                const response = await axios.post(url, { hashes: toQuery });
                const fromApi = new Set(response.data.cachedItems || []);
                return new Set([...pre, ...fromApi]);
            } catch (error) {
        ...
        cleanup: async () => {
            if (mongoUpserts.length > 0) {
                try { await cacheDb.upsertHashes('offcloud', mongoUpserts); } catch {}
            }
        }
    };
