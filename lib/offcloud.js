...
async function searchPersonalFiles(apiKey, searchKey) {
    ...
    try {
        ...
        const torrents = await processTorrents(OCClient, history);
...
async function resolveStream(apiKey, urlToResolve) {
    console.log(`[OC RESOLVER] Starting resolution for: ${urlToResolve.substring(0, 100)}...`);
    const isMagnet = urlToResolve.startsWith('magnet:');
    if (!isMagnet) {
        console.log(`[OC RESOLVER] This is a direct link, returning.`);
        return urlToResolve;
    }
    ...
    console.log(`[OC RESOLVER] Step 1: Checking for existing file (hash: ${hash})...`);
    const history = await OCClient.cloud.history();
    const torrents = await processTorrents(OCClient, history);
    let existingFile = torrents.find(file => file.hash === hash);
    if (existingFile) {
        console.log(`[OC RESOLVER] SUCCESS: Found existing file. No need to add.`);
        return existingFile.url;
    }

    console.log(`[OC RESOLVER] Step 2: File not in cloud. Adding to OffCloud...`);
    const addedItem = await addToOffcloud(apiKey, urlToResolve);
    if (!addedItem || !addedItem.requestId) {
        console.error(`[OC RESOLVER] FAILED: Could not add item to Offcloud.`);
        return null;
    }
    
    console.log(`[OC RESOLVER] Step 3: Item added (requestId: ${addedItem.requestId}). Checking immediately...`);
    // VVVV FASTER POLLING LOGIC VVVV
    // Check immediately, then start polling if not found
    for (let i = 0; i <= 4; i++) { // Poll 5 times (0 is immediate, then 4 waits)
        if (i > 0) {
            console.log(`[OC RESOLVER] File not found. Waiting 2 seconds (Attempt ${i})...`);
            await delay(500);
        }
        const newHistory = await OCClient.cloud.history();
        const newTorrents = await processTorrents(OCClient, newHistory);
        existingFile = newTorrents.find(file => file.requestId === addedItem.requestId || file.hash === hash);
        if (existingFile) {
            console.log(`[OC RESOLVER] SUCCESS: Found file on attempt ${i}.`);
            return existingFile.url;
        }
    }

    console.error(`[OC RESOLVER] FAILED: Could not find file after all polling attempts.`);
    return null;
}
...
async function addToOffcloud(apiKey, magnetLink) {
    ...
    try {
        console.log(`[OC API] ==> Sending request to add magnet.`);
        const response = await axios.post(url, { url: magnetLink });
        console.log(`[OC API] <== Successfully added magnet.`);
        return response.data;
    } catch (error) { 
        console.error(`[OC API] <== FAILED to add magnet: ${error.message}`);
        return null; 
    }
}
...
function getKeywords(str) {
    return str.toLowerCase().replace(/[':()]/g, '').split(' ').filter(word => word.length > 2 && !['the', 'and'].includes(word));
}
