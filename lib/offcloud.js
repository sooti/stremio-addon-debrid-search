...
async function searchPersonalFiles(apiKey, searchKeys, specificSearchKey, type, season, episode) {
    ...
    try {
        ...
        // **FIX 1 - Prevent crash on empty search keys**
        if (!primaryKey) {
            console.log(`[${LOG_PREFIX}] No valid search key found. Skipping personal files search.`);
...
async function combineAndMarkResults(apiKey, personalFiles, externalSources, episodeInfo = null) {
    ...
    const bypassPersonalFiles = enrichedPersonalFiles.filter(file => file.bypassFiltering === true);
