...
async function combineAndMarkResults(apiKey, personalFiles, externalSources) {
    const sourceNames = ['Bitmagnet', 'Jackett', 'Torrentio', 'Zilean', 'Comet', 'StremThru'];
    const enabledFlags = [BITMAGNET_ENABLED, JACKETT_ENABLED, TORRENTIO_ENABLED, ZILEAN_ENABLED, COMET_ENABLED, STREMTHRU_ENABLED];
...
async function processHistoryEntry(client, item) {
    ...
    // If the filename has a video extension, skip exploration and treat as a single file.
    if (isLikelySingleVideo) {
        ...
        // If it's not a valid video (e.g., a sample based on its name), return null.
        return null;
    }

    // --- Original logic for all other cases (folders, archives, or ambiguously named torrents) ---
    try {
...
// *** THIS FUNCTION REMAINS UNCHANGED (for context) ***
