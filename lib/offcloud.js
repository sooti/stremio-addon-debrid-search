...
async function searchOffcloudTorrents(apiKey, searchKey, threshold = 0.6) {
    console.log(`Searching torrents on Offloud with key: ${searchKey}`);
                ...
                return Promise.all(
                    encodedUrlsArray.map(async (encodedUrl) => {
                        ...
                        try {
                            ...
                            // Filter filenames containing 'sample' and size less than 700MB
                            if (fileName.toLowerCase().includes('sample') && fileSize < 700 * 1024 * 1024) {
                                return null;
                            }
...
async function getEncodedUrls(client, torrentId, item) {
    try {
        ...
        if (error === 'Bad archive') {
            return [
                {
                    url: `https://${item.server}.offcloud.com/cloud/download/${item.requestId}/${item.fileName}`,
                },
            ];
        }
