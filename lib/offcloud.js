...
async function searchOffcloudTorrents(apiKey, type, id) {
    ...
    // **START: FIX**
    // Create the episodeInfo object to pass to the cache processor
    ...
    try {
        ...
        // **START: FIX**
        // Pass episodeInfo down to the combine function
        ...
        if (bypassStreams.length > 0) {
            console.log(`[${LOG_PREFIX} FINAL] Found ${bypassStreams.length} bypass streams - returning ONLY these direct cloud links`);
        ...
        // **START: FIX**
        // Pass episodeInfo down to the combine function in the catch block too
...
async function searchPersonalFiles(apiKey, searchKeys, specificSearchKey, type, season, episode) {
    ...
    try {
        ...
        if (primarySearchTerm && type === 'series' && season && episode) {
            console.log(`[${LOG_PREFIX} EXACT] Checking for exact archive matches for: "${primarySearchTerm}"`);
            ...
            if (exactArchiveMatch) {
                console.log(`[${LOG_PREFIX} EXACT] Found exact archive match, returning ONLY this direct cloud link.`);
        ...
        const fuse = new Fuse(torrents, {
            keys: [{ name: 'searchableName', weight: 0.7 }, { name: 'name', weight: 0.3 }],
            threshold: 0.6, minMatchCharLength: 2, includeScore: true,
        });
        ...
        console.log(`[${LOG_PREFIX}] Found ${uniqueResults.length} personal files after search.`);
        console.timeEnd(`[${LOG_PREFIX} TIMER] Personal Cloud`);
...
// **START: FIX**
// Update the function signature to accept episodeInfo
...
// **END: FIX**
    ...
    if (bypassPersonalFiles.length > 0) {
        console.log(`[${LOG_PREFIX} COMBINE] Found ${bypassPersonalFiles.length} bypass personal files, returning ONLY these.`);
        return bypassPersonalFiles.map(file => ({ ...file, source: 'offcloud', isPersonal: true, tracker: 'Personal' }));
    }
    ...
    const markedPersonal = personalFiles.map(file => ({ ...file, source: 'offcloud', isPersonal: true, tracker: 'Personal' }));
    ...
    const ocHandler = {
        ...
        checkCachedHashes: async (hashes) => {
            ...
            try {
                const response = await axios.post(url, { hashes: hashes.map(h => h.toLowerCase()) });
                return new Set(response.data.cachedItems || []);
            } catch (error) {
    ...
    // **START: FIX**
    // Pass episodeInfo to the processAndFilterTorrents function
