...
async function searchOffcloudTorrents(apiKey, type, id) {
    ...
    try {
        ...
        const [personalFiles, ...scraperResults] = await Promise.all([
            searchPersonalFiles(apiKey, allSearchKeys, specificSearchKey),
            ...scraperPromises
        ]);
        ...
        const personalFiles = await searchPersonalFiles(apiKey, allSearchKeys, specificSearchKey); // Pass key here too for error case
...
// ===================================================================================
async function searchPersonalFiles(apiKey, searchKeys, specificSearchKey) {
    ...
    try {
        ...
        console.log(`[OC] Searching for: "${primarySearchTerm}"`);
        const results = fuse.search(primarySearchTerm);
        console.log(`[OC] Strategy "${primarySearchTerm}": found ${results.length} matches`);
