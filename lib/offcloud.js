...
const ZILEAN_LIMIT = parseInt(process.env.ZILEAN_LIMIT) || 75; // Add this line
...
async function combineAndMarkResults(apiKey, personalFiles, externalSources) {
    ...
    // **CRITICAL: If we have personal files with a bypass flag, return them immediately**
    ...
    // --- Always process and combine both personal and external sources ---
    ...
    // Mark personal files and update their size if a better source is found externally
    ...
    // Filter out any external torrents that are already in personal files
    ...
    // Return the combined list, with personal files always appearing first
...
async function searchZilean(title, season, episode, signal) {
    // FIX 1: Changed logging prefixes from [RD] to [OC]
    ...
    try {
        ...
        // FIX 3: Made the episode filtering logic more robust
        if (episode) {
            const targetEpisode = parseInt(episode);
            results = results.filter(result => {
                // Safely check if result.episodes is an array before using it
                const episodes = Array.isArray(result.episodes) ? result.episodes : [];
                
                const isSeasonPack = episodes.length === 0 || result.complete === true;
                if (isSeasonPack) {
                    return true;
                }
        ...
        // FIX 2: Use the ZILEAN_LIMIT variable
