...
export async function searchBluDV(query, signal, logPrefix, config) {
    ...
    try {
        ...
        const minBluDvTimeout = config?.BLUDV_TIMEOUT ?? ENV.BLUDV_TIMEOUT ?? 8000;
        // BluDV origin is frequently slow; ensure we never go below the min timeout
        const requestTimeout = Math.max(timeout ?? 0, minBluDvTimeout);
        ...
        for (let i = 0; i < accumulated.length; i += batchSize) {
            ...
            for (let j = 0; j < responses.length; j++) {
                ...
                try {
                    ...
                    // NOTE: We only process direct magnet links. systemads.org redirect links are skipped
                    // because they don't provide real infohashes that work with debrid services.
                    for (const magnetLink of magnetLinks) {
                        // Only process direct magnet links
                        if (!magnetLink.startsWith('magnet:')) {
                            // Skip systemads.org and other redirect links
                            continue;
                        }
