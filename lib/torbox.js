...
async function combineAndMarkResults(apiKey, personalFiles, externalSources, episodeInfo = null, cinemetaDetails) {
    ...
    const validTitleTorrents = uniqueExternalTorrents.filter(t => isValidTorrentTitle(t.Title, LOG_PREFIX));
    
    const newExternalTorrents = validTitleTorrents.filter(t => t.InfoHash && !personalHashes.has(t.InfoHash.toLowerCase()));
    ...
    if (episodeInfo && cinemetaDetails) {
        ...
        if (rejectedCount > 0) console.log(`[${LOG_PREFIX}] Franchise filter rejected ${rejectedCount} torrents from other series.`);
        torrentsToProcess = franchiseFiltered;
    }

    const hashes = torrentsToProcess.map(t => t.InfoHash.toLowerCase());
    console.log('checking these hashes: ', hashes)
    const cachedHashes = await checkTorboxCache(apiKey, hashes);
    console.log('Found these hashes: ', cachedHashes)
    ...
    const cachedSingleFiles = torrentsToProcess
        .filter(t => !isPack(t) && cachedHashes.has(t.InfoHash.toLowerCase()))
        .filter(t => isValidVideo(t.Title, t.Size, undefined, LOG_PREFIX));
