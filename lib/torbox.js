...
// Convert base32 hash to hex (TorBox expects hex format)
function base32ToHex(base32Hash) {
    try {
        // Base32 alphabet (RFC 4648)
        const alphabet = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ234567';
        const base32 = base32Hash.toUpperCase().replace(/=+$/, '');
...
async function searchTorboxTorrents(apiKey, type, id, userConfig = {}) {
	...
	console.log('combined results: ', obfuscateSensitive(JSON.stringify(combinedResults), apiKey))
        ...
        const combined = await combineAndMarkResults(apiKey, personalFiles, completedResults, episodeInfo, cinemetaDetails);
        if (type === 'movie') {
...
async function checkTorboxCache(apiKey, hashes) {
    ...
    try {
        console.log(`[${LOG_PREFIX}] Checking cache for ${hashes.length} hashes (will convert to hex).`);
        console.log(`[${LOG_PREFIX}] Sample base32 hashes:`, hashes.slice(0, 3));

        const hexToBase32Map = new Map();
        const hexHashes = hashes.map(h => {
            const hex = base32ToHex(h);
            if (hex) {
                hexToBase32Map.set(hex, h);
            }
            return hex;
        }).filter(Boolean);

        if (hexHashes.length === 0) {
            console.log(`[${LOG_PREFIX}] No valid hex hashes to check after conversion.`);
            return new Set();
        }
        
        console.log(`[${LOG_PREFIX}] Converted to ${hexHashes.length} hex hashes. Sample:`, hexHashes.slice(0, 3));
        const response = await axios.post(url, { hashes: hexHashes }, { headers });

        if (response.data?.success && typeof response.data.data === 'object') {
            const cachedHex = Object.keys(response.data.data);
            console.log(`[${LOG_PREFIX}] TorBox API returned ${cachedHex.length} cached hex hashes`);
            console.log(`[${LOG_PREFIX}] Sample cached hex:`, cachedHex.slice(0, 3));
            
            const cachedBase32 = new Set();
            for (const hex of cachedHex) {
                const lowerHex = hex.toLowerCase();
                if (hexToBase32Map.has(lowerHex)) {
                    cachedBase32.add(hexToBase32Map.get(lowerHex));
                }
            }
            console.log(`[${LOG_PREFIX}] Matched ${cachedBase32.size} hashes back to base32.`);
            return cachedBase32;
        }
        return new Set();
    } catch (error) {
...
async function getTorrentList(apiKey, bypassCache = false) {
    ...
    try {
        ...
        // Handle TorBox API errors gracefully (e.g., 500 server errors)
