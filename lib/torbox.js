...
async function searchTorboxTorrents(apiKey, type, id) {
    ...
    if (config.SNOWFL_ENABLED) scraperPromises.push(scrapers.searchSnowfl(specificSearchKey, signal, LOG_PREFIX));
...
async function combineAndMarkResults(apiKey, personalFiles, externalSources, episodeInfo = null, cinemetaDetails) {
    ...
    const uniqueExternalTorrents = [...new Map(externalTorrentsRaw.map(t => [t.InfoHash, t])).values()];
    console.log(`[TB DEBUG] Started with ${uniqueExternalTorrents.length} unique external torrents.`);

    // --- Filter 1: Episode Matching ---
    let episodeFiltered = uniqueExternalTorrents;
    if (episodeInfo) {
        episodeFiltered = uniqueExternalTorrents.filter(torrent => {
            try {
                const parsed = PTT.parse(torrent.Title);
                const isEpisodeMatch = parsed.season === episodeInfo.season && parsed.episode === episodeInfo.episode;
                const isSeasonPack = parsed.season === episodeInfo.season && parsed.episode === undefined;
                if (isEpisodeMatch || isSeasonPack) return true;
                if (parsed.season === episodeInfo.season && parsed.episode !== undefined) return false;
                return true;
            } catch (e) {
                return true;
            }
        });
        console.log(`[TB DEBUG] ${episodeFiltered.length} torrents remain after initial episode filter.`);
    }
    ...
    // --- Filter 3: Franchise/Series Name Matching ---
    ...
    if (episodeInfo && cinemetaDetails) {
        ...
        franchiseFiltered = validTitleTorrents.filter(torrent => {
            try {
                const torrentTitle = normalize(PTT.parse(torrent.Title).title);
                return torrentTitle.includes(seriesTitle);
            } catch (e) { return true; }
        });
    }
    console.log(`[TB DEBUG] ${franchiseFiltered.length} torrents remain after franchise filter.`);
    
    // --- Filter 4: Check TorBox Cache ---
    const hashesToCheck = franchiseFiltered.map(t => t.InfoHash.toLowerCase());
    const cachedHashes = await checkTorboxCache(apiKey, hashesToCheck);
    console.log(`[TB] Found ${cachedHashes.size} cached torrents from ${hashesToCheck.length} unique candidates.`);

    const cachedTorrents = franchiseFiltered.filter(t => cachedHashes.has(t.InfoHash.toLowerCase()));
