...
async function searchTorboxTorrents(apiKey, type, id, userConfig = {}) {
    ...
    try {
        console.time(`[${LOG_PREFIX}] Total search time ${Date.now()}`);
        ...
        console.timeEnd(`[${LOG_PREFIX}] Total search time ${Date.now()}`);
...
async function searchPersonalFiles(apiKey, searchKey) {
    console.time(`[${LOG_PREFIX} TIMER] Personal Cloud ${Date.now()}`);
    try {
        ...
        console.timeEnd(`[${LOG_PREFIX} TIMER] Personal Cloud ${Date.now()}`);
    }
}
