...
async function extractMDriveLinks(mdriveUrl) {
  ...
  try {
    ...
    // Look for hubcloud links (accept any hubcloud domain)
...
async function getMoviesDriveStreams(imdbId, tmdbId, mediaType = 'movie', season = null, episode = null, config = {}) {
  ...
  try {
    ...
    // Process all quality variants (no limit - we want all resolutions including 4K)
    ...
    const streamPromises = limitedLinks.map(async (link) => {
      try {
        ...
        const nonGdflixLinks = mdriveLinks.filter(mdriveLink => mdriveLink.type !== 'gdflix');

        const hubcloudPromises = nonGdflixLinks
          .map(async (mdriveLink) => {
            try {
              ...
              return filteredStreams.map(stream => {
                ...
                // Remove DS4k from title for resolution detection (DS4k doesn't indicate 4K resolution)
                // This prevents false 4K detection when actual resolution is in formats like 480P, 720P, 1080P, 2160P
                const titleForResolution = rawTitle.replace(/DS4k/gi, '');

                // Extract resolution from cleaned title (must be done AFTER cleaning)
                const resolution = getResolutionFromName(titleForResolution);
