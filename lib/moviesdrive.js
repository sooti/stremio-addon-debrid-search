  ...
  try {
    ...
    const streamPromises = limitedLinks.map(async (link) => {
      try {
        ...
        // Process ALL hosting links (hubcloud AND gdflix) in parallel
        // GDFlix links are also compatible with extractHubCloudLinks
        console.log(`[MoviesDrive] Processing ${mdriveLinks.length} hosting links (HubCloud + GDFlix)`);

        const hubcloudPromises = mdriveLinks
          .map(async (mdriveLink) => {
            try {
              const linkType = mdriveLink.type === 'gdflix' ? 'GDFlix' : 'HubCloud';
    ...
    console.log(`[MoviesDrive] Returning ${filteredStreams.length} streams (filtered ${beforeFilter - filteredStreams.length} archive files, sorted by resolution, then size)`);
