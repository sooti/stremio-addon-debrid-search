...
function toTorrentDetails(item) {
    ...
    if (item.type === 'folder' && Array.isArray(item.content)) {
        ...
        videos.push({
            name: item.name,
            url: `${process.env.ADDON_URL}/resolve/Premiumize/null/${item.id}/${encode(item.stream_link)}`,
            size: item.size,
        });
    }
        ...
        const cachedTorrents = torrents
            ...
            .map(t => {
                ...
                return {
                    ...
                    source: 'premiumize',
                    hash: t.InfoHash,
                    url: `magnet:?xt=urn:btih:${t.InfoHash}&dn=${encode(cacheInfo.name || t.Title || t.name || '')}`, // Keep magnet link for resolver
                };
            });
