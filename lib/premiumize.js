...
const addHashToMongo = (hash, fileName = null, size = null, data = null) => debridHelpers.addHashToMongo(hash, fileName, size, data, 'premiumize');
    ...
    try {
        ...
        try {
            if (mongoCache?.isEnabled()) {
                const mongoHashes = await mongoCache.getCachedHashes('premiumize', hashes);
        ...
        try {
            if (mongoCache?.isEnabled() && cachedTorrents.length > 0) {
                const upserts = [];
