...
export function formatStreamsWithFlags(cachedLinks) {
  ...
  const streams = cachedLinks.map((streamInfo) => {
    try {
      ...
      // Clean up rawQuality to remove size in brackets (e.g., " [10.04 GB]")
      const cleanedFilename = rawQuality.replace(/\s*\[[^\]]+\]\s*$/, '').trim();

      // Use cleaned filename instead of rawQuality with size
      const title = `${cleanedFilename}${flagsSuffix}\n💾 ${size} | UHDMovies`;
  ...
  // Also filter out zip/rar/pack files - we only want direct video files
  const validStreams = streams.filter(stream => {
    ...
    // Check if the stream is an archive file (zip, rar, 7z, pack)
    const title = (stream.fullTitle || '').toLowerCase();
    const isArchive = /\.(zip|rar|7z|tar|gz|pack)(\s|$)/i.test(title);
    if (isArchive) {
      console.log(`[UHDMovies] Filtering out archive file: ${stream.fullTitle}`);
      return false;
    }
  ...
  console.log(`[UHDMovies] Formatted ${validStreams.length} stream(s) (filtered ${streams.length - validStreams.length} unknown quality/archive streams)`);
