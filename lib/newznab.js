...
async function search(serverUrl, apiKey, query, options = {}) {
  try {
    ...
    const parsed = await parseStringPromise(responseData, {
      explicitArray: false,
      mergeAttrs: true
    });
    ...
    const results = items.map(item => {
      ...
      // Extract guid value - xml2js can parse guid as object if it has attributes
      const guidValue = typeof item.guid === 'object' && item.guid._
        ? item.guid._
        : item.guid;

      return {
        id: guidValue || item.link,
...
async function getNzbContent(nzbUrl, apiKey) {
  try {
    ...
    // Fetch from URL (SQLite cache doesn't store NZB content)
